...
**为什么 `StubRoutines` 用全局函数指针表而不是虚函数接口？** stub 的调用在解释器和 JIT 编译代码的热路径上（每次方法调用、每次 Safepoint 检查都走）。虚函数调用需要两次间接跳转（读 vtable → 跳函数地址），而函数指针调用只需要一次间接跳转（直接读函数指针地址并跳转）。在一个性能敏感的路径上，两次 vs 一次间接跳转的区别在高频调用中会被放大。而且 stub 的初始化是一次性的（启动时填充一次，运行时只读取），函数指针表正好匹配这种 `write-once, read-many` 模式。
