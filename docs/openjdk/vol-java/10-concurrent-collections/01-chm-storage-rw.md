...
## 五个最容易混掉的边界：读无锁不是无同步，CAS 不是整图并发答案，锁桶不是锁表，MOVED 不是坏桶，弱一致也不是乱读

第一，读无锁不是无同步。`get()` 不进入 `synchronized`，不等于它不依赖同步语义；`volatile table`、桶位原子读写和节点字段的发布协议，正是读线程敢不加锁仍能看到合法结构的前提。

第二，CAS 不是整图并发答案。CAS 很适合“空桶占坑”这种单槽位竞争，但一旦进入链表遍历、旧值覆盖、尾插新节点、树化判断这类复合结构修改，单个 CAS 不足以表达整段原子语义，最终仍要回到桶内独占修改。

第三，锁桶不是锁表。JDK 11 的 `ConcurrentHashMap` 在冲突写时锁的是当前桶首节点，不是整个 map，也不是旧版分段锁的 segment；它真正想压缩的是同步范围，而不是完全消灭锁。

第四，`MOVED` 不是坏桶。它表示这个桶正处在迁移协议里，当前线程不该把它当普通链表继续本地修改，而应转向协助扩容或去新表继续查找；这是状态切换信号，不是结构损坏信号。

第五，弱一致也不是乱读。CHM 没有给 `get()` 承诺全局最新快照，但它依然努力保证“结构合法、查找可继续、结果符合某个并发时刻的可见状态”；弱一致说的是快照语义较弱，不是说返回值毫无规则。

把这五条边界记稳，ConcurrentHashMap 就不会再被误解成“比 HashMap 多点 CAS 和关键字”的黑箱。它真正想讲的是：先按桶状态划分访问路径，再按冲突等级选择同步强度，把最贵的独占修改严格推迟到桶内结构真的发生竞争的那一刻。
