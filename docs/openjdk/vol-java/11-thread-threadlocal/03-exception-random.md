...
## 五个最容易混掉的边界：子线程异常不是回主线程，默认处理器不是日志框架，stderr 不是异常消失，ThreadLocalRandom 不是 ThreadLocal<Random>，线程私有也不是只有一种实现

第一，子线程异常不是回主线程。线程一旦分叉，异常就不会再沿创建者线程的调用栈往回冒；未捕获异常最终走的是当前线程自己的退出责任链，而不是启动它的那条主线程栈。

第二，默认处理器不是日志框架。`UncaughtExceptionHandler` 责任链只负责决定谁最后接手这个异常，至于是否进统一日志、告警或观测系统，要看你有没有把默认出口显式接进这些基础设施。

第三，stderr 不是异常消失。很多“线程炸了但日志没有”的现象，不是 JVM 吞掉了异常，而是最后兜底动作只是把堆栈打到标准错误流；如果进程环境没收集这条流，问题就会看起来像没发生过。

第四，`ThreadLocalRandom` 不是 `ThreadLocal<Random>`。它没有走 `ThreadLocalMap` 的通用值容器路线，而是把种子、probe 等高频状态直接挂到 `Thread` 字段上，把共享竞争压缩到初始化阶段。

第五，线程私有也不是只有一种实现。通用业务值适合用 `ThreadLocalMap` 这种可索引、可清理的容器；固定用途、高频访问的状态则可能更适合线程裸字段。两者目标都叫线程私有，但生命周期、成本和风险完全不同。

把这五条边界记稳，域 11 的三篇文章就不会散成“线程 API”“ThreadLocal”“随机数优化”三堆碎知识。它们真正共同讲的是：Thread 对象既托管生命周期和异常出口，也托管不同形态的线程私有状态；关键永远不是名字，而是这份状态在失败时如何收容、在高频时如何避开共享竞争。
