...
> 基于 JDK 11 `java.base` 的 `Double`、`Math`、`StrictMath` 与 `FloatingDecimal` 实现。本文讨论的是 JDK 11 当前的浮点打印调用链、Math/StrictMath 委托关系和包装类型比较语义，不把这些实现路径或性能表现外推成所有 JDK 版本、平台或语言库的统一规范；JDK 19+ 的浮点打印实现差异只作为版本边界说明。
...
## 五个最容易混掉的边界：double 不是十进制容器，打印不是存储还原，epsilon 不是万能比较，Math 不是必然更快，NaN 也不是普通数字

第一，double 不是十进制容器。它保存的是有限位宽的二进制浮点值，`0.1` 这样的十进制小数可能只能以近似值进入；这不等于 double 的二进制运算规则不精确，而是表示目标不同。

第二，打印不是存储还原。`Double.toString(0.1)` 输出 `0.1`，说明这个字符串足以把同一个 double 往返解析回来，不说明内部位模式等于数学上的十进制 0.1，也不能把日志字符串直接当成金额事实。

第三，epsilon 不是万能比较。固定阈值无法同时适配极小值、极大值、接近零和不同量纲；浮点比较必须先定义误差模型，需要十进制金额语义时应更换表示，而不是继续调一个常数。

第四，Math 不是必然更快。JDK 11 中许多 `Math` 方法直接委托给 `StrictMath`，具体性能还受版本、平台和调用路径影响；两个类的关键区别首先是结果承诺与实现自由度，不是一个快、一个慢的口诀。

第五，NaN 不是普通数字。它不等于自身，Infinity 也不是“足够大的正常值”，`+0.0` 与 `-0.0` 还会在除法、比较、排序和包装对象语义中表现出差异。业务规则若不先规定这些值如何进入、传播和落库，浮点异常就会在边界处变成数据异常。

把这五条边界记稳，浮点问题就不会重新塌缩成“精度不够”四个字。它真正想讲的是：先识别位模式与舍入，再区分打印展示和内部存储，最后为比较、排序、数学函数和业务金额分别选择匹配的语义；工具没有错，错的是把不同层的承诺当成了同一层。
