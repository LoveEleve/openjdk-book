...
> 基于 JDK 11 `java.base` 的包装类实现，并区分 JLS 装箱最低保证与 JDK 当前缓存策略。本文讨论的是 JDK 11 包装类缓存与对象身份边界，不把这里的缓存上限、装箱命中路径和解析算法外推成所有 JDK 版本或所有数值容器的统一规范。
...
## 五、五个最容易混掉的边界：== 不是比值，-128..127 不是固定上限，缓存命中不是业务保证，parseInt 不是溢出后报错，Long 缓存也不是 Integer 复制

在收网之前，先把这一篇最容易记错的五条边界压实。

第一，`==` 对包装类不是比值。它比较的是对象身份；缓存命中时恰巧返回同一个引用，值相等但引用不同时 `==` 就可能返回 false。业务比较数值必须用 `equals` 或拆箱。

第二，`-128..127` 也不是固定的缓存上限。这是 JLS 对装箱缓存的最低保证，JDK 11 可以通过 `-XX:AutoBoxCacheMax` 或 `IntegerCache.high` 扩大上限，但不能因此依赖 `==` 来做值比较。

第三，缓存命中更不是业务正确性的保证。数据库、反序列化、反射或显式 `new Integer` 产生的对象可能不经过缓存路径，`==` 的偶然正确性就会消失。

第四，`parseInt` 也不是先算出结果再检查溢出。溢出发生后信息已经丢失，实现必须在累加前判断下一步是否还能安全继续，这是边界检查必须先于危险算术的原则。

第五，`Long` 的缓存也不是 `Integer` 缓存的复制。`LongCache` 有它自己的上限和范围，`Short/Byte` 也有各自的缓存实现；不能只凭 Integer 推导所有包装类的缓存行为。

把这五条边界记稳，包装类与缓存这一篇就不会重新塌回“Integer 100 为什么 == 100”的面试口诀印象。它真正想讲的是：包装类把数值变成了对象，对象身份和数值相等是两套不能混用的语义。
