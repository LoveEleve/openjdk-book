...
## 六、五个最容易混掉的边界：final 不等于内容不可变，byte[]+coder 不等于规范布局，构造不都复制，substring 不总是复制，被共享也不等于可变安全

在收网之前，先把这一篇最容易记错的五条边界压实。

第一，`final byte[]` 不等于数组内容不可变。`final` 只约束引用不能换指向，`value[0] = ...` 仍然可能成立；String 真正做不到被外部改写，靠的是 private、不可信输入隔离和内部方法不原地修改这几道边界叠加。

第二，`byte[] + coder` 也不是 Java 规范要求的唯一布局。它是 JDK 9 引入 Compact Strings 后，HotSpot/JDK 11 当前实现选择的存储方式；换成 `char[]` 的 JVM 在语义上照样可以存在。

第三，String 的所有构造也不都复制数组。公开构造必须防御性复制外部输入，但包内可信路径（如 `String(byte[], coder)`）允许直接共享已受控数组。拿包内路径去证明“String 构造从不复制”是错的。

第四，`substring` 也不总是复制。`beginIndex == 0`、结果与当前对象完全一致时，它会直接返回当前对象；只有真正截取子区间时才走编码专用路径创建新数组。

第五，一个 String 被多个地方共享，也不代表它天然安全可改。共享是安全的，前提正是这个对象真的遵守了不可变边界；一旦有人能拿到内部数组并修改，共享者会一起被污染。所以边界靠的是防线本身，不是“被共享”这个事实。

把这五条边界记稳，String 不可变这一篇就不会重新塌回“String 有 final 所以不可变”的单点印象。它真正想讲的是：不可变是一整套类边界、字段边界、数组边界和操作边界共同完成的约定。
