...
## 五个最容易混掉的边界：引用 CAS 不是对象内部全原子，ABA 不是值没变就没事，Stamped 不是历史日志，Markable 不是简化 Stamped，FieldUpdater 也不是白拿的轻量原子类

第一，引用 CAS 不是对象内部全原子。`AtomicReference` 保证的是“整个引用值的替换”这一步原子，而不是帮你把被引用对象内部多个字段的修改也自动粘成一个原子整体。

第二，ABA 不是“值没变就没事”。对引用结构来说，A→B→A 仍然可能意味着链路摘下又放回、节点被处理过、历史已经改变；CAS 只看当前值相等，不会天然知道中间发生过什么。

第三，Stamped 不是历史日志。`AtomicStampedReference` 的 stamp 首先是和当前引用一起参与 CAS 校验的附加状态，最常见用法是版本戳，但它并不负责保存完整演化历史。

第四，Markable 不是简化版 Stamped。它不是“版本号不够用时少给一点”，而是当你真正只关心一位逻辑状态时，用布尔标记把引用和状态一起 CAS；适用问题类型本来就不同。

第五，`FieldUpdater` 也不是白拿的轻量原子类。它省掉的是每对象一个包装实例的内存，但前提是字段类型、`volatile` 约束和访问控制都得满足，而且它的封装边界与正确性纪律比直接 Atomic 包装更脆弱。

把这五条边界记稳，CAS 的扩展路线就不会再被看成“把 AtomicInteger 平移到别的类型”那么简单。它真正想讲的是：一旦从数字走向引用和现有字段，你得到的是更强的适用范围，但也必须同时承担历史语义、ABA 风险和封装约束这些新增成本。
