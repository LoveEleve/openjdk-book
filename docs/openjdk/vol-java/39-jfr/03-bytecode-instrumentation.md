...
这里最容易掉进的有三种失败理解。第一种，是把 `Event` 基类源码当成最终执行真相，于是看到 `commit()` 很薄就误判“JFR 其实没做什么”。第二种，是把字节码增强理解成单纯替换几个方法体的小补丁，看不见它真正编译的是整条事件提交协议。第三种，是把注入后的事件类想成脱离系统自己写缓冲的独立角色，而忽略 `EventHandler` 这条统一控制链。
