...
更具体一点，线程池最容易被误判的其实有三个失败方案。

第一种失败方案，是把线程池理解成“线程数量配置题”。于是排查问题时只盯着 `corePoolSize`、`maximumPoolSize`，好像把数字改对了，行为自然就对了。但只要线程池已经进入 `SHUTDOWN`，或者队列里还有一堆旧任务没清，你就会发现“线程数”根本解释不了它下一步为何这么做。

第二种失败方案，是把它理解成“线程 + 队列”的松耦合组合。这样看时，很容易觉得状态放一个字段、工人数放一个字段、workers 再单独放一个集合就够了。问题在于线程池的大多数关键判断，偏偏都要同时读取这些事实：状态是不是还允许扩工，工人数是不是已经归零，最后一个 worker 退出时是否该进入终止态。只要这些事实分散得太松，就很容易在并发窗口里读到拼不起来的中间态。

第三种失败方案，是把 Worker 当成普通 `Thread` 包装壳。这样你就会忽略线程池真正管理的是“工人生命周期”：它带着什么首任务出生、当前是不是忙碌、是否适合被中断、已经完成了多少任务。这些都不是裸线程对象自己会替线程池维护好的语义。
...
## 五个最容易混掉的边界：ctl 不是普通计数器，状态不是标签，Worker 不是线程壳，core/max 不是独立旋钮，shutdown 也不是立刻停工

第一，`ctl` 不是普通计数器。它不是单纯记“当前有几个线程”，而是把运行状态和 workerCount 打包成同一份联合事实，供扩线程、回收和终止判断一起使用。

第二，状态不是标签。`RUNNING`、`SHUTDOWN`、`STOP`、`TIDYING`、`TERMINATED` 不是给生命周期贴名字，而是在实时改变“接不接新任务、清不清旧任务、该不该中断工人”的策略。

第三，`Worker` 不是线程壳。它不仅包着一个真实线程，还承载首任务、完成任务计数以及工人级的 AQS 控制状态；线程池真正管理的是这种可被统计、可被区分忙闲、可被中断的工人实体。

第四，`corePoolSize` / `maximumPoolSize` 不是独立旋钮。它们只有放回 `workQueue`、`keepAliveTime` 和拒绝策略的同一条决策链里才有意义；单独调大一个数字，不代表线程池行为就会按直觉变化。

第五，`shutdown()` 也不是立刻停工。它切断的是“接收新任务”的入口，而不是马上让已有 worker 和队列任务全部消失；线程池真正收尾还要继续走到 workerCount 归零以及 `TIDYING → TERMINATED` 的终态切换。

把这五条边界记稳，ThreadPoolExecutor 就不会再被误解成“线程数配置题”。它真正想讲的是：线程池始终在联合管理状态、工人数、队列积压和工人生命周期，任何提交、扩容、回收或关闭动作，都是在这套联合状态机上做出的决策。
