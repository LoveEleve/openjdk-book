...
## 五个最容易混掉的边界：FutureTask 不是结果壳，完成不是只有成功，get 不是 join，cancel 不是回滚，定时调度也不是线程先睡着

第一，`FutureTask` 不是结果壳。它不是线程池执行别的任务、调用方再拿一个外部结果对象观察，而是同一个对象同时承担执行实体和结果句柄两种角色。

第二，完成不是只有成功。`NORMAL`、`EXCEPTIONAL`、`CANCELLED`、`INTERRUPTED` 都是不同的最终结局；FutureTask 的状态机要区分的不只是“有没有结束”，而是“这次结束到底是哪一种命运”。

第三，`get()` 不是 `join()`。它等待的不是线程字面意义上的结束，而是 FutureTask 状态稳定到一个最终可对外解释的结局；这也是为什么异常和取消都会在 `get()` 这里重新表现出来。

第四，`cancel()` 不是回滚。它只能在任务尚未定局时抢占 `NEW` 状态控制权，并在需要时尝试发出协作式中断请求；已经正常完成或异常完成的历史结果，不会被它撤销。

第五，定时调度也不是线程先睡着。真正持有时间顺序的是 `DelayedWorkQueue` 和任务自己的触发时间字段，worker 只在任务真正到点时才被占用，而不是提前抱着未来任务长期休眠。

把这五条边界记稳，FutureTask 和 ScheduledThreadPoolExecutor 就不会再被误解成“一个拿结果的包装类”和“一个能 delay 的普通线程池”。它们真正想讲的是：执行、结果、取消和时间都必须被编进同一套任务状态机与排序协议里，否则调用方看到的结局和线程池内部实际发生的时序就会脱节。
