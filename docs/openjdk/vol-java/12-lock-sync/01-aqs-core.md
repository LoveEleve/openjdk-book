...
## 五个最容易混掉的边界：state 不是锁本身，队列不是资格来源，AQS 不是具体同步器，模板方法不是装饰，release 也不是改完 state 就结束

第一，`state` 不是锁本身。它只是同步资格的共享事实载体，至于 0/1、重入计数、许可余额还是倒计时步数，要由具体子类赋义；把它当成固定的“锁标志位”，就看不见 AQS 为什么能复用到那么多同步器上。

第二，队列不是资格来源。等待队列只负责收容失败线程和维持后续推进秩序，并不能独立决定“现在谁该通过”；真正决定资格是否成立的，仍然是当下的 `state` 与子类语义钩子。

第三，AQS 不是具体同步器。它不等于 `ReentrantLock` 的别名，也不是某种独占锁实现，而是一套把获取失败、排队等待、状态变化和后继推进统一起来的骨架框架。

第四，模板方法不是装饰。`tryAcquire`、`tryRelease` 这些钩子不是为了让子类“顺便自定义一点逻辑”，而是 AQS 故意留给子类去解释 state 业务含义的真正接口；没有这层钩子，AQS 就只能服务一种固定同步语义。

第五，`release()` 也不是改完 state 就结束。释放动作一旦让资格事实发生变化，后续还必须考虑等待队列里的线程是否该被推进；否则 state 虽然空出来了，后继线程却可能永远睡在队列里。

把这五条边界记稳，AQS 就不会再被误解成“一个 int 加一条链表”的实现技巧。它真正想讲的是：同步器要同时拥有资格事实、失败收容和可扩展语义，只有这三层拼在一起，`ReentrantLock`、`Semaphore`、`CountDownLatch` 这类完全不同的工具才能共用同一套等待骨架。
