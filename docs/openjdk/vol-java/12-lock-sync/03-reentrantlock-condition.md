...
## 五个最容易混掉的边界：可重入不是锁自动认主，unlock 不是一定真释放，公平不是绝不插队，Condition 不是第二把锁，signal 也不是立刻继续执行

第一，可重入不是锁自动认主。`ReentrantLock` 之所以允许同一线程再次进入，不是 JVM 神奇识别“这是自己人”，而是锁内部同时记录了 owner 身份和 `state` 重入计数，并把同 owner 的再次获取解释成计数累加。

第二，`unlock()` 不是一定真释放。对可重入锁来说，单次释放通常只是在退一层持有计数；只有计数真正归零，owner 才会被清掉，后继线程才有机会获得这把锁。

第三，公平不是绝不插队。公平锁主要约束的是正常阻塞获取路径上，队列外新线程是否还能先试一次资格；像无超时版 `tryLock()` 这种显式“我先赌一把”的路径，本来就接受非公平语义。

第四，Condition 不是第二把锁。它解决的是“已经拿到锁但条件不满足时去哪等”，所以必须依附于原来的独占锁和 AQS 同步队列；没有持锁前提、没有 fullyRelease 和重新 acquire 的闭环，它就无法单独成立。

第五，`signal()` 也不是立刻继续执行。它首先做的是把节点从条件队列转回主同步队列，等待线程随后还得重新参与锁竞争、重新拿回原先层数的锁，条件满足不等于当前持锁者已经让出执行权。

把这五条边界记稳，ReentrantLock 与 Condition 就不会再被拆成“可重入”“公平锁”“await/signal”几块孤立知识。它们真正共同维护的是一条完整协议：锁负责持有资格与重入账本，Condition 负责把业务条件等待从锁竞争等待里分拆出来，再在条件成立后把线程送回主赛道继续争取执行权。
