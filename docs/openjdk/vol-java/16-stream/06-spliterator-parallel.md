...
## 五个最容易混掉的边界：Spliterator 不是 Iterator 换名，能 split 不等于适合并行，特性位不是注释，parallel 不是专属线程池，数据大也不是自动更快

第一，`Spliterator` 不是 `Iterator` 换名。它多出来的关键不是另一个遍历接口，而是 `trySplit`、`estimateSize` 和 `characteristics` 这套可切分执行契约，为并行拆块提供入口。

第二，能 split 不等于适合并行。数据源就算实现了 `trySplit`，如果切分很不均衡、切分成本很高，或者后续顺序约束太重，并行流照样可能得不偿失。

第三，特性位不是注释。`SIZED`、`SUBSIZED`、`ORDERED`、`CONCURRENT`、`IMMUTABLE` 这些标志会直接改变框架对切分质量、顺序语义和执行安全性的假设。

第四，`parallel()` 不是专属线程池。默认并行流最终仍会落到共享的 `ForkJoinPool.commonPool()`，所以阻塞式回调和长时间等待会把影响扩散到 JVM 里其他共用 commonPool 的任务。

第五，数据大也不是自动更快。并行收益永远要同时满足切块靠谱、单元素处理够重、顺序约束不重、执行逻辑不阻塞这几条前提；缺哪一条，parallel 都可能只是把额外开销堆上去。

把这五条边界记稳，并行流就不会再被讲成“加个 parallel 开关 JVM 就自动帮你多线程提速”的黑箱优化。它真正想讲的是：先由 Spliterator 证明数据源切得值不值得，再由 commonPool 去执行这些切块，而最终效果则取决于切分质量、任务成本和执行环境能否真的承受这条并行路径。
