...
## 五个最容易混掉的边界：终端不是最后顺手取值，TerminalOp 不是统一壳，collect 不是简单装容器，短路不是方法自己 return，count 也不是必然全量数一遍

第一，终端不是最后顺手取值。它首先要把前面积累的惰性描述切到真正执行阶段，再决定结果如何落地，而不是在所有工作都做完之后随便读一个返回值。

第二，`TerminalOp` 不是统一壳。不同终端操作真正关心的收口策略并不相同：有的要归约合并，有的要短路停流，有的甚至能直接凭上游特征得出答案。

第三，`collect` 不是简单装容器。它真正提供的是 supplier / accumulator / combiner 这套收口协议，让串行与并行路径都知道该怎样累加和合并结果。

第四，短路不是方法自己 `return`。`anyMatch`、`findFirst` 这些终端之所以能提早结束，是因为终端 sink 通过 `cancellationRequested()` 改变了整条执行链的停流控制，而不是在链尾孤立地提前返回。

第五，`count()` 也不是必然全量数一遍。在 `SIZED` 等特征明确的场景下，终端操作可以直接利用上游尺寸事实返回结果，甚至不必驱动整条管道逐元素消费。

把这五条边界记稳，终端求值就不会再被误讲成“链子最后拿个结果”的尾声。它真正想讲的是：终端操作既负责把流水线点火，也负责选择收口策略、决定是否短路、是否并行合并，甚至判断有没有必要真正把所有元素都放过这条管道。
