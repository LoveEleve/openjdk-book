...
## 五个最容易混掉的边界：Pipeline 不是执行链，Sink 不是普通 Consumer，终端操作不是只收结果，短路不是构建期裁剪，单遍消费也不是每步各扫一遍

第一，Pipeline 不是执行链。它在构建期主要承担的是节点登记、前后链接和标志合成，不会因为 `filter()`、`map()` 已经被调用就立刻触碰数据源。

第二，Sink 不是普通 Consumer。它不只接 `accept()`，还带着 `begin/end/cancellationRequested()` 这一整套执行协议，用来承载运行时逐元素推进和停流控制。

第三，终端操作不是只收结果。它真正先做的是调用 `evaluate()` 按下执行开关，把前面积累的静态 Pipeline 描述翻译成可运行的 Sink 链。

第四，短路不是构建期裁剪。`limit`、`takeWhile`、`anyMatch` 这些语义并不会在链刚搭好时就神奇截断数据，而是在执行期通过取消协议决定是否继续往后消费元素。

第五，单遍消费也不是“每步各扫一遍”。无状态流水线的关键效率来自同一个源 Spliterator 只被驱动一次，而元素在这一次推进中连续穿过整条 Sink 管道。

把这五条边界记稳，Stream 的惰性机制就不会再被误讲成“JVM 先帮你忍着不执行”的黑箱优化。它真正想讲的是：先用 Pipeline 记录图纸，再在终端操作来临时反向焊出 Sink 管道，最后用一次源遍历把所有节点逻辑串成真正的逐元素执行链。
