...
> 基于 JDK 11 `java.transaction.xa` 的 XA 协议接口语义，并结合 `javax.sql.XAConnection`、XA 驱动和事务管理器的协作边界。本文讨论的是 JDK 11 接口签名与 2PC 理论流程；当前源码树裁剪了 `java.transaction.xa` 模块，具体驱动、中间件和 Seata 行为不属于 JDK 实现事实，不能外推成所有分布式事务框架的统一规范。
...
> 关联: 本地 Connection 事务边界;XA 实现参考数据库驱动(域外)

## 两个库,怎么原子提交

本地事务好办: 一个 Connection,commit/rollback 划边界。但**跨库事务**一进来,问题就立刻变了。订单库扣款成功了,库存库减库存失败怎么办?两个库要么都提交要么都回滚,这句话说出来很轻松,真正困难的地方在于: 这两个参与者并不共享一个本地事务,也不共享一个锁管理器,中间还隔着网络、协调者和恢复日志。

最常见的三种失败方案也都很直觉。第一种失败方案,是把跨库事务想成本地事务的横向放大版,好像只要拿到两条 Connection,最后一起 `commit()` 就行。这样看时,你会忽略“谁来统一裁决”“谁先承诺自己能提交”“其中一个提交不了时另一个为什么必须跟着回滚”。第二种失败方案,是把 2PC 理解成“先问一圈意见,再发个 commit”这么简单,从而看不见 prepare 阶段真正锁住了哪些资源,以及为什么协调者一旦崩溃,参与者会陷在不确定状态里。第三种失败方案,是把 XA 和 Seata AT 统统归成“分布式事务框架”,只按快慢比较,却不先区分它们在一致性、锁持有时间和恢复语义上的根本差异。

所以这一篇讲的不是“再多背几个接口名”,而是分布式事务最核心的一条主线: **当一个本地事务边界不够用了,系统必须先定义参与者契约,再定义统一裁决流程,最后承担强一致承诺带来的阻塞与可用性代价。**
...
## 五个最容易混掉的边界：XA 不是实现本身，prepare 不是提交成功，2PC 不是没有阻塞，recover 不是自动修复，Seata AT 也不是 XA 换个名字

第一，XA 不是实现本身。XAResource 只定义事务管理器与参与者之间的协议接口，真正执行本地事务、加锁、记录恢复状态的是数据库驱动和资源管理器；JDK 不会凭一个接口替你完成跨库原子提交。

第二，`prepare` 不是提交成功。它代表参与者投票并进入“可以提交但尚未最终提交”的状态，资源可能已经被锁住；只有协调者在所有参与者都准备成功后发出 commit，分支才真正完成提交。

第三，2PC 不是没有阻塞。它用准备阶段换取统一决策，但参与者可能在协调者故障或网络异常期间保持不确定状态并继续占用资源；强一致承诺正是用等待和可用性代价换来的。

第四，`recover()` 不是自动修复。它为协调者恢复后重新发现未完成分支提供入口，但恢复仍依赖事务日志、参与者状态和协调者决策，接口本身不会替系统猜出应该提交还是回滚。

第五，Seata AT 也不是 XA 换个名字。XA/2PC 依赖参与者 prepare、全局决策和同步资源约束，Seata AT 则通过 undo log 与异步补偿换取不同的一致性和吞吐取舍；二者不能只按“谁更快”做替换判断。

把这五条边界记稳，分布式事务就不会被简化成“给多个 Connection 外面套一个 commit”。它真正想讲的是：协议接口、协调者、资源参与者、本地锁和恢复日志共同决定了跨库事务能承诺什么，而强一致性从来不是没有代价的开关。
