...
> 下一层:在此引擎之上叠加"源码学习领域层"(docs/product/04-*,待写)
...
| (领域层) | 书级组织(书籍/章节) | docs/product/02-* | 书架/章节实体 = 源码学习领域层设计(非引擎层;引擎提供事件过滤+检索能力) |
...
- [ ] 用户确认后:在 31 个能力域上做 **MVP 取舍决策**(docs/product/02-mvp-scope.md)
- [ ] 然后:源码学习领域层设计(docs/product/03-*)
- [ ] 最后:JD 映射(面试附带层)
