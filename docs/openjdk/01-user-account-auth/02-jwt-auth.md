...
但当前实现显然不满足于此。`GatewayAuthFilter.isBlacklisted()` 在 `my-xhs-gateway/src/main/java/com/myxhs/gateway/filter/GatewayAuthFilter.java:203` 到 `:227` 中，会额外查 Redis 黑名单。
