...
机制一目了然:**目标已绑定 → 当场算偏移,还能选 2 字节短跳;目标未绑定 → 登记 `add_patch_at` + 先发 6 字节长格式(rel32 槽位填 0)**。等 `bind(L)` 时(assembler.cpp:111-119,`bind_loc(locator())` + 调 `patch_instructions`),遍历补丁数组(169 起:溢出时从 GrowableArray 弹出,否则从 `_patches` 数组取),逐个调 `pd_patch_instruction`(汇编器平台实现)回填真实偏移。`jccb`(2138)是显式短跳版本,同样登记后回填。
