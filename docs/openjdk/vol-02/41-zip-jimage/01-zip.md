...
> → **后续**:[02 — JIMAGE 模块镜像](02-jimage.md)(JDK 自己的镜像格式)
...
- `entries[]` 是**扁平数组**,每个单元是 16 字节的 `jzcell`(zip_util.h:183-187),只存三样: `hash`(名字的 32 位哈希)、`next`(链式哈希的下一跳)、`cenpos`(该条目在 CEN 里的偏移,名字本体读到文件里再按需解析);
- `table[]` 是哈希桶数组,`tablelen = (total/2) | 1`——**表大小只有条目数的一半**(注释原文 "Odd -> fewer collisions"): 每个桶平均约 2 个条目(负载因子≈2),链式冲突下摊还仍是 O(1),还省一半索引内存;强制奇数避免与 2 的幂取模产生低位聚集;
...
**关键设计 (斜体)**: *哈希表只存"偏移 + 哈希"不存名字,是"空间换延迟"的权衡: 每个条目只占一个 16 字节的 `jzcell`,打开时遍历目录区一遍就全部建好;名字的完整内容(可能几 KB)留到命中时再按需从文件读。目录区是 ZIP 的"地图",哈希表是这张地图的索引。*
