...
前几行是魔术数分级:`andl` 用 0x7FFF0000 掩掉符号位,只留指数(11 位)和 4 位尾数高位,减 0x30300000,与 0x10C50000 比较。一个 `cmpl` 的 flags 被两个跳转共享:418 行的无符号 `above` 把"指数超出 [771, 1039]"整体甩去 L_2TAG_PACKET_0_0_1(指数 < 771 时无符号下溢成大数,也满足 above);进入大标签后,502 行再用**有符号** `greater` 从剩下的里分出真正的巨大参数(指数 > 1039,跳第 2 节的 Payne-Hanek);留在原地的极小参数(指数 < 771,有符号为负)继续走第 3 节的两条捷径。一次比较、两个跳转,三种去向。`PI32INV` 是 32/π(常量在 336-339 行),`ONEHALF` 是 0.5。接下来:y = x·32/π,`por` 把 x 的符号并进 0.5,加完再 `cvttsd2sil`(截断转 int)——**加 ±0.5 再截断 = 四舍五入**,这就是 N = round(32/π·x)。注意这里的 64 位常量 `_PI32INV = 0x40245F306DC9C883`,指数 0x402 = 2³,数值正好是 32/π ≈ 10.186。代码块里的 `SHIFTER`(2⁵²)是头注释描述的"加 2⁵² 再减"舍入方案的常量——本实现改用 ±0.5 舍入,xmm2 在 457 行被表数据覆盖,这行加载不再被使用,属于方案切换的残留。
...
**关键设计 (斜体)**: *为什么表步长选 π/32 而不是 π/4?步长减半,表体积翻倍、多项式需要的项数减半。π/32 让 |r| ≤ π/64 ≈ 0.049,4 项多项式(下一小节)就够把修正压进 double 末位(ULP 量级);表项只有 32 字节,查表就一次对齐读取。对比纯多项式方案(fdlibm 的 `__kernel_sin` 要 6 项 13 次多项式),"查一次表 + 4 项多项式"在 x86 上更快——用 2 KB 数据换掉一半的浮点指令。*
...
53 位尾数被拆成"高 21 位 + 低 32 位"两段(531-534 行),与 2/π 的连续 7 个 32 位表字交叉相乘——每个 `imulq` 的两个操作数都 ≤ 2³²,64 位乘积恰好无截断,再按 2³² 错位累加(541-547 行),2/π 的有效数字用了 224 位。乘积的高位是"x·2/π 的整数部分",低位决定小数和象限;余数(650 行起)换算成以 π/4 为单位的约减结果。这就是 Payne-Hanek(1983 年论文)的核心:**先用 2/π 的扩展精度数字做整数乘法,只关心小数部分,再反乘 π/4**——精度只受乘法链长度限制,1e100 也不怕。
