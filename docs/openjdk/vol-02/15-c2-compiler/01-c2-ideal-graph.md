...
前面整个 C1 域，我们都在强调同一件事：C1 追求的是低延迟，所以它选择块式 HIR、轻量优化、LinearScan 和 Runtime1 这种“够用就好”的编译组织方式。
...
## 2. `Node`：为什么 C2 要把控制、数据和内存都画成边

C2 先换掉的，是程序的形状。

`Node` 最核心的两个字段是 `_in` 和 `_out`。源码注释写得非常直接：`_in` 是 use-def 引用数组，`_out` 是 def-use 引用数组。也就是说，一个节点既知道“我依赖谁”，也知道“谁依赖我”。`node.hpp:282-283`

更重要的是，输入边还分成两类：required edges 和 precedence edges。required edges 是语义正确性必须要有的输入，顺序有意义，也允许 `NULL`；precedence edges 则主要帮助决定执行顺序，不允许重复和内嵌 `NULL`。`node.hpp:285-291`
...
`Node` 的寿命管理也呼应了这张图的规模。它们统一从 `Compile::current()->node_arena()` 分配，`operator delete` 是 NOP。也就是说，C2 根本没打算让节点各自做细粒度释放；它依赖的是 Arena 批量回收，而死节点清理由优化阶段在图逻辑上处理。`node.hpp:231-238`

---

## 3. `Node` 不是被动数据结构：`Identity / Value / Ideal` 三钩子

C2 里每个节点都不是“等着 pass 来改”的被动物体。`Node` 基类直接定义了三个统一钩子：

- `Identity(PhaseGVN*)`
- `Value(PhaseGVN*)`
- `Ideal(PhaseGVN*, bool can_reshape)`。`node.cpp:1081-1145`

默认行为本身就很有意思：

- `Identity()` 默认返回 `this`，表示“没找到更好的既有节点”；
- `Value()` 默认返回 `bottom_type()`，也就是最坏情况类型；
- `Ideal()` 默认返回 `NULL`，表示“这节点已经够理想了，没有图改写”。

这三个钩子把三类优化动作拆开了：

- `Identity` 负责说“我其实等价于某个已有节点”；
- `Value` 负责说“看我的输入类型，我运行时可能值的集合更精确了”；
- `Ideal` 负责说“我可以把自己或周围子图重写成更好的形状”。

尤其是 `Ideal()` 的返回契约，非常容易被误解。源码注释写得极细：

- 只要做了任何图改写，就必须返回改写后子图的根，即使根还是 `this`；
- 不能从 `Ideal()` 返回一个“旧节点”，想返回旧节点要走 `Identity()`；
- 除了 `this` 指针本身，不能去修改旧节点，因为旧节点可能被别的用户共享。

这里最该记住的不是规则条文本身，而是它揭示了 C2 的工作方式：**节点优化不是一个 pass 拿着剪刀在外面乱改图，而是每个节点自己暴露“我能否折叠、能否变窄、能否重写”的统一接口。**

---

## 4. `Type`：为什么必须把“可能值集合”刻在图上

光有节点和边还不够。优化器还必须知道：某个节点在运行时可能取什么值。

C2 用 `Type` 做这件事。`Type` 不是“调试信息里的类型注释”，而是一套真正参与优化的格。基类直接提供 `meet`、`join`、`dual`。`type.hpp:224-247`

对整数区间来说，`TypeInt::xmeet()` 会把两个区间扩成能同时覆盖二者的最小大区间。比如 `[0,10]` 和 `[5,15]` meet 之后得到 `[0,15]`。这不是求交集，而是给 Phi 和其他合流节点一份同时覆盖多条路径的安全上界。`type.cpp:1455-1495`

指针类型更能看出格为什么重要。`TypePtr::ptr_meet` 那张表里，`Null` 和 `NotNull` 的 meet 不是“丢掉 null 信息”，而是直接变成 `BotPTR`。也就是说，**“既是 null 又非 null”在格里代表矛盾，不是模糊。** `type.cpp:2460-2466`
...
5. 最后做 `hash_find_insert()`，执行全局值编号。`phaseX.cpp:1283-1390`

整段最关键的不是“它做了这五件事”，而是每一步几乎都会把旧节点的用户重新放回 worklist。也就是说，**一个节点的局部变化不会停在自己身上，而会沿 def-use 边继续逼着整张图重新考虑。**

这就是为什么 IGVN 不能用“固定跑三趟”“固定跑五趟”来替代。因为你根本不知道一次局部改写最终会触发多少层后续变化：图改写可能暴露新的常量；类型变窄可能让别的 `If` 或 `Phi` 可折叠；常量化又可能触发新的 `Identity` 或 hash CSE；两个节点一旦被合并，所有用户都得重新审视。

---

## 6. 这不是全图暴力重扫：worklist 的稀疏传播

看到“不动点迭代”，很容易以为 C2 在做的是“全图一遍遍重扫”。如果真这样，它早就慢得不可接受了。

C2 真实做法更稀疏。`Compile` 在 parse 期就准备了一份 `_for_igvn` 初始 worklist，注释直接说了：这是 Iterative GVN 的起始节点列表。`record_for_igvn()` 本身也只是把节点 push 进这张表。`compile.cpp:757-759`

真正运行时，`optimize()` 只是不断从 `_worklist` 里弹出节点：

- 若节点还有用户，就对它做 `transform_old()`；
- 若它已经没用户且不是 top，就把它当死节点清掉；
- 同时用节点数上限和 `K * live_nodes()` 这种守卫防止优化发散。`phaseX.cpp:1223-1247`
...
## 7. 把三件事收回到同一个闭环：`Node` 给路径，`Type` 给精度，`IGVN` 给传播机制
...
1. **C2 只是“优化更多的 C1”吗?** 不是。它先换世界观——统一图 + 类型格 + 不动点传播。
2. **IGVN 不就是多跑几遍规范化 pass 吗?** 不是。IGVN 是 worklist 驱动的传播机制，局部变化会沿 def-use 边继续扩散。
3. **`Node` 只是被动数据结构吗?** 不是。每个节点都通过 `Identity / Value / Ideal` 暴露自己的优化接口。
4. **`Type` 只是调试标签吗?** 不是。它是优化的精度来源：区间、null 性质、singleton 常量化都靠它。
5. **不动点迭代是不是等于全图反复重扫?** 不是。C2 只重看受影响的节点，靠的是 worklist 稀疏传播。

把这一篇压成三句话：

- **C2 不是“更多 pass”，而是“统一图 + 类型 + 不动点迭代”**。
- **`Node` 给依赖路径，`Type` 给值域精度，`IGVN` 给传播机制**，三者形成闭环。
- **C2 的全局性不来自“多花时间”本身，而来自这套表示与传播方式本来就适合把局部变化推到全局。**

下一篇: `Parse + GraphKit`——既然世界观已经换成 Ideal Graph，那字节码到底怎么一条条灌进这张图里。
