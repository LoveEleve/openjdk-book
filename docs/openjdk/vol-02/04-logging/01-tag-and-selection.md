...
所有标签集中在 `LOG_TAG_LIST` 宏里(logTag.hpp:34-174),gc、heap、region、task、phases 都是**同一层的枚举值**:
...
**关键设计 (斜体)**: *为什么用扁平枚举而不是模块树?因为一条日志**可以属于多个分类**——G1 的并发标记事件同时是 `gc+task`(调度)和 `gc+phases`(阶段统计),树形分类只能选一个父节点,标签可以叠。扁平 + 多标签 = "一个事件从多个维度可被过滤"。5 个上限是"够用就好":绝大多数日志 1-3 个标签,第 6 个大概率是凑数。*
