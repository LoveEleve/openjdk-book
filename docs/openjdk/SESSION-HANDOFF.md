...
> **状态**: 2026-08-15 | 卷 2 写作中: **115/152 篇完成**(第 1 批 12 + 第 2 批 26 + 第 3 批 14 + 第 4 批 21 + 第 5 批 32 + 第 6 批 10) | 第 1-5 批**全部完结**;第 6 批(JIT/GC)进行中,**本会话 32 篇: 20-02 + 27-jni(3) + 30-jvm-entry(3) + 32-jfr(6) + 34-nmt(2) + 36-attach(2) + 37-heap-dumper(2) + 39-runtime-monitoring(2) + 46-sa(1) + 14-c1(4,14 域完结) + 15-c2(6)**,下一篇 15-c2-compiler/07 | **上下文已满,本文件为非常详细交接版**——新 AI 只读本文件即可继续,不要依赖旧会话记忆
...
**下一步(唯一,无选择)**: 15-c2-compiler/07(PhaseMacroExpand: 高层抽象→低层 MachNode 展开,大纲 `planning/outlines/15-c2-compiler/07-c2-macro-intrinsics.md`;15 域 01-06 篇已完结,悬念已指向 07)。
...
第 6 批(JIT/GC): 14 ✅ → **15 ✅(6/8)** → 21 → 25 → 28 → 29 → 33 → 43
第 7 批(上层): 22 → 26 → 35 → 40 → 47
```

**已完成 115 篇**(全部在 `docs/openjdk/vol-02/`):
...
| **15-c2-compiler** | 1-6 | `01-c2-ideal-graph.md`(253)+`02-c2-parse-graphkit.md`(230)+`03-c2-optimizations.md`(138)+`04-c2-loops.md`(140)+`05-c2-register-alloc.md`(143)+`06-c2-codegen.md`(129) | ✅ **15 域 6/8(本会话)** |
...
**本会话新增素材 30 个**(全部 gitignore 不入库,在 materials/commands/):
...
## 六、本会话实战经验(最重要,新 AI 必读;6.1-6.51 旧会话沉淀,6.52-6.70 本会话 19 篇新沉淀)
...
- **章节维护教训(连续 5 次犯,写在教训里还是犯——根因确认)**: 追加新节的错误模式=`new=新节全部内容+"### 6.xx 旧节标题"` 去替换 `old="### 6.xx 旧节标题"`——**new 里带着旧节标题,导致新节块插到旧节标题前面**;已 5 次发生(6.65/6.66、6.66/6.67、6.67/6.68、6.68/6.69、6.69/6.70),每次都要二次交换修复;**正确做法: 锚点必须用旧节最后一条内容(如 "- 实证: xxx-demo.txt"),且 new 里不得包含旧节标题**;插入后**必须立即运行 `grep -n "### 6.6"` 校验顺序**——这是提交前固定步骤
...
- [ ] **15-c2-compiler/07**(PhaseMacroExpand: 高层抽象→低层 MachNode 展开)——**下一篇**;大纲 `planning/outlines/15-c2-compiler/07-c2-macro-intrinsics.md`;15-c2-compiler/06 悬念已指向 15-c2-compiler/07
...
```
1. 读 planning/outlines/15-c2-compiler/07-*.md(注意 ⚠️ 块——01-06 篇已回填(3 块 13/15/13/17/15/15 条),07 篇大概率同样漂移;15-c2/07 重点: PhaseMacroExpand——macro.cpp/hpp(PhaseMacroExpand: expand_macro_nodes/eliminate_macro_nodes 已在 03 篇见过一面的落地;Allocate/AllocateArray/Lock/Unlock/ArrayCopy 的展开;scalar_replacement 与 EA 衔接)、macroArrayCopy.cpp(arraycopy 展开)、library_call.cpp(intrinsic 的大本营,6991 行——本篇主要讲 intrinsic 还是 macro?按大纲标题"高层抽象→低层 MachNode 展开"主 macro;注意 03 篇已讲 eliminate_macro_nodes(EA 后),07 讲 expand_macro_nodes(最终展开);与 01-06 篇衔接: 宏节点在 IGVN/EA 挂起→循环优化后展开→Matcher 匹配;flag: PrintMacroExpansion 之类 notproduct 边界)
...
5. 15-c2 后 → 21-shared-runtime → 25-gc → 28-jvmti → 29-mh → 33-jmx → 43-nio-net(第 6 批剩余 1 域)
```
