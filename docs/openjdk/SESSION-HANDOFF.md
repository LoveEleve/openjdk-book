...
> **状态**: 2026-08-15 | 卷 2 写作中: **114/152 篇完成**(第 1 批 12 + 第 2 批 26 + 第 3 批 14 + 第 4 批 21 + 第 5 批 32 + 第 6 批 9) | 第 1-5 批**全部完结**;第 6 批(JIT/GC)进行中,**本会话 31 篇: 20-02 + 27-jni(3) + 30-jvm-entry(3) + 32-jfr(6) + 34-nmt(2) + 36-attach(2) + 37-heap-dumper(2) + 39-runtime-monitoring(2) + 46-sa(1) + 14-c1(4,14 域完结) + 15-c2(5)**,下一篇 15-c2-compiler/06 | **上下文已满,本文件为非常详细交接版**——新 AI 只读本文件即可继续,不要依赖旧会话记忆
...
**下一步(唯一,无选择)**: 15-c2-compiler/06(Matcher + Code Generation: DFA 指令选择 → x86 机码,大纲 `planning/outlines/15-c2-compiler/06-c2-codegen.md`;15 域 01-05 篇已完结,悬念已指向 06)。
...
第 6 批(JIT/GC): 14 ✅ → **15 ✅(5/8)** → 21 → 25 → 28 → 29 → 33 → 43
第 7 批(上层): 22 → 26 → 35 → 40 → 47
```

**已完成 114 篇**(全部在 `docs/openjdk/vol-02/`):
...
| **15-c2-compiler** | 1-5 | `01-c2-ideal-graph.md`(253)+`02-c2-parse-graphkit.md`(230)+`03-c2-optimizations.md`(138)+`04-c2-loops.md`(140)+`05-c2-register-alloc.md`(143) | ✅ **15 域 5/8(本会话)** |
...
**本会话新增素材 29 个**(全部 gitignore 不入库,在 materials/commands/):
...
## 六、本会话实战经验(最重要,新 AI 必读;6.1-6.51 旧会话沉淀,6.52-6.69 本会话 18 篇新沉淀)
...
- **章节维护教训(连续 4 次犯,写在教训里还是犯)**: 在本文件 §6 追加新节时,用 `old=旧节标题` 作锚点会把新节插在**旧节标题之前**(新节块+旧节标题连在一起),导致编号倒序——已 4 次发生(6.65/6.66、6.66/6.67、6.67/6.68、6.68/6.69),每次都要二次交换修复;**正确做法(写进教训后第 4 次仍然犯,说明必须机械化)**: 锚点必须用**旧节最后一条内容**(如 "- 实证: xxx-demo.txt"),新节整体插在其后;**插入后必须立即运行 `grep -n "### 6.6"` 校验顺序**——把校验固化成提交前的一步
...
- [ ] **15-c2-compiler/06**(Matcher + Code Generation: DFA 指令选择 → x86 机码)——**下一篇**;大纲 `planning/outlines/15-c2-compiler/06-c2-codegen.md`;15-c2-compiler/05 悬念已指向 15-c2-compiler/06
...
```
1. 读 planning/outlines/15-c2-compiler/06-*.md(注意 ⚠️ 块——01-05 篇已回填(3 块 13/15/13/17/15 条),06 篇大概率同样漂移;15-c2/06 重点: Matcher 指令选择——matcher.cpp/hpp(Matcher::match/ReduceInst/DFA 状态机、x86.ad 的 instruct 规则与 match 子树、opto/classes.hpp 的机器节点)、output.cpp(发码: PhaseOutput/emit/relocation)、block.cpp(块序)、gcm.cpp(全局调度);与 01-05 篇衔接: 理想图经 Matcher→机器节点→RA(chaitin)→发码;注意 Matcher 的 DFA=adlc 生成物(不在源码树),x86.ad 是唯一可读的指令定义源;与 04 篇衔接: 向量节点(LoadV/AddVI)的 .ad 匹配已在 04 篇实证;flag: PrintOptoAssembly diagnostic 但实现 NOT_PRODUCT(01 篇已探明))
...
5. 15-c2 后 → 21-shared-runtime → 25-gc → 28-jvmti → 29-mh → 33-jmx → 43-nio-net(第 6 批剩余 2 域)
```
