...
> **状态**: 2026-08-15 | 卷 2 写作中: **118/152 篇完成**(第 1 批 12 + 第 2 批 26 + 第 3 批 14 + 第 4 批 21 + 第 5 批 32 + 第 6 批 13) | 第 1-5 批**全部完结**;第 6 批(JIT/GC)进行中,**本会话 35 篇: 20-02 + 27-jni(3) + 30-jvm-entry(3) + 32-jfr(6) + 34-nmt(2) + 36-attach(2) + 37-heap-dumper(2) + 39-runtime-monitoring(2) + 46-sa(1) + 14-c1(4,14 域完结) + 15-c2(8,15 域完结) + 21-shared-runtime(1)**;下一篇 21-shared-runtime/02 | **上下文已满,本文件为非常详细交接版**——新 AI 只读本文件即可继续,不要依赖旧会话记忆
...
**下一步(唯一,无选择)**: 21-shared-runtime/02(c2i/i2c Adapter,大纲 `planning/outlines/21-shared-runtime/02-c2i-i2c-adapter.md`;21 域 01 篇已完结,悬念已指向 02)。
...
第 6 批(JIT/GC): 14 ✅ → 15 ✅(8/8) → **21 ✅(1/3)** → 25 → 28 → 29 → 33 → 43
第 7 批(上层): 22 → 26 → 35 → 40 → 47
```

**已完成 118 篇**(全部在 `docs/openjdk/vol-02/`):
...
| **21-shared-runtime** | 1 | `21-shared-runtime/01-runtime-stubs.md`(96) | ✅ **21 域 1/3(本会话)** |
...
## 六、本会话实战经验(最重要,新 AI 必读;6.1-6.51 旧会话沉淀,6.52-6.73 本会话 22 篇新沉淀)
...
- **章节维护教训(连续 8 次犯——模板已明示仍犯,从 6.73 起强制两步执行)**: 错误模式=`new=新节内容+"### 6.xx 旧节标题"` 替换 `old="### 6.xx 旧节标题"`;已 8 次(6.65-6.73 每次);**强制操作顺序(违反即事故)**: ①写正文阶段先在文件末尾(## 七、之前)追加 `### 6.xx+1 新节...`(用文件追加/插入到 ## 七、行前,完全不碰旧节内容);②**立即 `grep -n "### 6.6"` 校验连续递增**;③再做其他任何编辑。禁止用"新节+旧节标题"整体替换模式
...
- [ ] **21-shared-runtime/02**(c2i/i2c Adapter)——**下一篇**;大纲 `planning/outlines/21-shared-runtime/02-c2i-i2c-adapter.md`;21-01 悬念已指向 02
...
```
1. 读 planning/outlines/21-shared-runtime/02-*.md(注意 ⚠️ 块——01 篇已回填 3 块 14 条,02 篇大概率同样漂移;21-02 重点: c2i/i2c adapter——sharedRuntime_x86_64.cpp 的 generate_c2i_adapter/generate_i2c_adapter、方法入口形态(解释器入口 from_interpreted_entry vs 编译入口)、i2c 的栈帧切换与参数搬运;与 01 篇衔接: 01 讲桩(IC miss/deopt 逃生口),02 讲编译↔解释双向入口;与 15-c2/02 衔接: from_interpreted_entry 字段(i2c/i2i);与 16-code-cache 衔接: nmethod 入口)
...
5. 21 域后 → 25-gc → 28-jvmti → 29-mh → 33-jmx → 43-nio-net(第 6 批剩余 5 域)
```
