...
> **状态**: 2026-08-14 | 卷 2 写作中: **91/152 篇完成**(第 1 批 12 + 第 2 批 26 + 第 3 批 14 + 第 4 批 21 + 第 5 批 18) | 第 1-4 批**全部完结**(12 个域),第 5 批(VM 核心)进行中 18/18,**11-cds/12-ci/13-jit-framework/18-safepoint/20-vm-operations/27-jni/30-jvm-entry 域完结,32-jfr 1/4(本会话)**,下一篇 32-jfr/02 | **上下文已满,本文件为非常详细交接版**——新 AI 只读本文件即可继续,不要依赖旧会话记忆
...
第 5 批(VM 核心): **11 ✅ → 12 ✅ → 13 ✅ → 18 ✅ → 20 ✅(2/2 完结) → 27 ✅(3/3 完结) → 30 ✅(3/3 完结) → 32(1/4)** → 34 → 36 → 37 → 39 → 46   🚧 进行中
第 6 批(JIT/GC): 14 → 15 → 21 → 25 → 28 → 29 → 33 → 43
第 7 批(上层): 22 → 26 → 35 → 40 → 47
```

**已完成 91 篇**(全部在 `docs/openjdk/vol-02/`):
...
| **32-jfr** | 1-4 | `32-jfr/01-recorder-engine.md`(139) | 🚧 32 域 1/4(本会话) |
...
- 实证: 30-reflection-stackwalk-demo.txt / 32-jfr-recorder-demo.txt(素材清单见 §五)
...
- 实证: 30-java-calls-demo.txt / 30-reflection-stackwalk-demo.txt / 32-jfr-recorder-demo.txt(素材清单见 §五)
...
- 实证: 27-jni-check-demo.txt / 30-jvm-entry-demo.txt / 30-java-calls-demo.txt / 30-reflection-stackwalk-demo.txt / 32-jfr-recorder-demo.txt(素材清单见 §五)
...
- 实证: 27-jni-fastpath-demo.txt / 27-jni-check-demo.txt / 30-jvm-entry-demo.txt / 30-java-calls-demo.txt / 30-reflection-stackwalk-demo.txt / 32-jfr-recorder-demo.txt(素材清单见 §五)
...
- 实证: 27-jni-handles-demo.txt / 27-jni-fastpath-demo.txt / 27-jni-check-demo.txt / 30-jvm-entry-demo.txt / 30-java-calls-demo.txt / 30-reflection-stackwalk-demo.txt / 32-jfr-recorder-demo.txt(素材清单见 §五)
...
- [ ] **32-jfr/02**(事件类型与元数据)——**下一篇**;大纲 `planning/outlines/32-jfr/02-event-metadata.md`;32-01 悬念指向它
