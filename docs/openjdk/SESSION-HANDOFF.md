...
**本会话覆盖**: 21-shared-runtime/03(异常,21 域收官)+ **25-gc-framework 六篇全完结**(01 BarrierSet+Access API/02 CollectedHeap+分配/03 Reference Processing/04 WorkGang+TaskQueue/05 CardTable+DirtyCardQueue/06 OopStorage+StringDedup+GC Stats)。25 域是第 6 批最大域,**已收官**;下一篇 28-jvmti/01(第 6 批剩余 4 域: 28→29→33→43,然后第 7 批 22/26/35/40/47)。
...
| JVMTI 源码(28-01 用) | 预判: `share/prims/jvmti*.cpp/hpp`(jvmtiEnv.cpp 接口实现/jvmtiEventController 事件分发/jvmtiExport 事件发布)、`share/prims/jvmti.xml`(接口定义源)+`jvmtiEnter.cpp` 生成物;36-attach/02 已讲 agent 加载链,本篇从 JVM 内部角度——写作时按大纲实际定位 |
