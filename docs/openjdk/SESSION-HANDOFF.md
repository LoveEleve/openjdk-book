...
**下一步(唯一,无选择)**: 08-interpreter(解释器——字节码怎么执行,24 域三个域的消费方)。
...
| 24-frame | 1-3 | `24-frame/01-physical-frame.md`(238 行)/`02-virtual-frame.md`(151 行)/`03-deopt-gc-scan.md`(268 行) | ✅ **24 域完结** |
...
- [x] **24-frame/03**(deopt-gc-scan)——✅ 完成,commit 见 §二(正文 8850989+大纲回填 6ae67c0);**24 域 3/3 完结**
- [ ] **08-interpreter**(字节码执行)——**下一篇**;24 域完结后接续
...
```
1. 读 planning/outlines/08-interpreter/ 大纲(注意 ⚠️ 块——24 域三篇大纲均已回填(01 十条/02 六条/03 九条),08 大概率同样漂移;24-03 篇悬念指向它: 解释器怎么执行字节码)
2. 验证大纲所有 file:line 与专有名词(按 §6.5-1 的规律;重点: templateTable.cpp/bytecodeInterpreter 的分派、解释器帧布局(24-01 已铺垫 offsets)、Interpreter::_code codelet(24-01 实证 271 个)、oopMapCache 计算器(24-01 已铺垫 per-Klass);与 24 域呼应: deopt 的 deopt_reexecute_entry/continue_after_entry 就是 08 域的入口模板;实证可用 -XX:+PrintInterpreter 布局+字节码跟踪)
