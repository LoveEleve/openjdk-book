...
> **状态**: 2026-08-15 | 卷 2 写作中: **113/152 篇完成**(第 1 批 12 + 第 2 批 26 + 第 3 批 14 + 第 4 批 21 + 第 5 批 32 + 第 6 批 8) | 第 1-5 批**全部完结**;第 6 批(JIT/GC)进行中,**本会话 30 篇: 20-02 + 27-jni(3) + 30-jvm-entry(3) + 32-jfr(6) + 34-nmt(2) + 36-attach(2) + 37-heap-dumper(2) + 39-runtime-monitoring(2) + 46-sa(1) + 14-c1(4,14 域完结) + 15-c2(4)**,下一篇 15-c2-compiler/05 | **上下文已满,本文件为非常详细交接版**——新 AI 只读本文件即可继续,不要依赖旧会话记忆
...
**下一步(唯一,无选择)**: 15-c2-compiler/05(Chaitin: 图着色寄存器分配 O(n²),大纲 `planning/outlines/15-c2-compiler/05-c2-register-alloc.md`;15 域 01-04 篇已完结,悬念已指向 05)。
...
第 6 批(JIT/GC): 14 ✅ → **15 ✅(4/8)** → 21 → 25 → 28 → 29 → 33 → 43
第 7 批(上层): 22 → 26 → 35 → 40 → 47
```

**已完成 113 篇**(全部在 `docs/openjdk/vol-02/`):
...
| **15-c2-compiler** | 1-4 | `01-c2-ideal-graph.md`(253)+`02-c2-parse-graphkit.md`(230)+`03-c2-optimizations.md`(138)+`04-c2-loops.md`(140) | ✅ **15 域 4/8(本会话)** |
...
**本会话新增素材 28 个**(全部 gitignore 不入库,在 materials/commands/):
...
## 六、本会话实战经验(最重要,新 AI 必读;6.1-6.51 旧会话沉淀,6.52-6.68 本会话 17 篇新沉淀)
...
- **实证方法论(重要)**: ①**容器性能对照噪声 ±20%**——短运行不可靠,需 -Xbatch+超长运行(reps=2000000)才稳定;②C1 vs C2 对照最稳(C2 循环体系整体效果: level 3 慢 3.7 倍);③flag 拆分: -XX:-UseSuperWord +59%/-XX:LoopUnrollLimit=1 +70%(展开贡献>向量化,因 pack 依赖展开后的相邻指令);④OSR 事件(1 % 3 run @ 6)=回边触发编译的存在性证据;⑤CITime IdealLoop 阶段计时=循环优化耗时的直接读数;⑥TraceLoopOpts/TraceSuperWord notproduct 不可用——循环变换本身无法直接打印
- **章节维护教训(连续 3 次犯)**: 在本文件 §6 追加新节时,用 `old=旧节标题` 作锚点会把新节插在**旧节标题之前**(新节块+旧节标题连在一起),导致编号倒序(6.68 在 6.67 前)——已 3 次发生(6.65/6.66、6.66/6.67、6.67/6.68),每次都要二次交换修复;**正确做法: 锚点用旧节最后一条内容(如 "- 实证: xxx-demo.txt"),新节整体插在其后**,或插入后立即 grep 顺序校验
...
- [ ] **15-c2-compiler/05**(Chaitin: 图着色寄存器分配 O(n²))——**下一篇**;大纲 `planning/outlines/15-c2-compiler/05-c2-register-alloc.md`;15-c2-compiler/04 悬念已指向 15-c2-compiler/05
...
```
1. 读 planning/outlines/15-c2-compiler/05-*.md(注意 ⚠️ 块——01-04 篇已回填(3 块 13/15/13/17 条),05 篇大概率同样漂移;15-c2/05 重点: Chaitin 图着色——chaitin.cpp/hpp(Chaitin::register_alloc: build_ifg_virtual/simplify/select/coalesce/IFG 干扰图)、ifg.cpp(IFG 构建)、coalesce.cpp(合并)、reg_split.cpp(spill/split)、live.cpp(活跃性)、与 14-c1 LinearScan 对照(图着色 O(n²) vs 线性扫描 O(n));与 01-04 篇衔接: Node/IGVN/循环优化已铺底,05 讲机器码前的最后一关;注意 Chaitin 对理想图的消费=Matcher 之后(compile.cpp Code_Gen 阶段);flag: VerifyGraphEdges/PrintOptoAssembly 已探明 notproduct/diagnostic 边界)
...
5. 15-c2 后 → 21-shared-runtime → 25-gc → 28-jvmti → 29-mh → 33-jmx → 43-nio-net(第 6 批剩余 3 域)
```
