...
> **状态**: 2026-08-15 | 卷 2 写作中: **111/152 篇完成**(第 1 批 12 + 第 2 批 26 + 第 3 批 14 + 第 4 批 21 + 第 5 批 32 + 第 6 批 6) | 第 1-5 批**全部完结**;第 6 批(JIT/GC)进行中,**本会话 28 篇: 20-02 + 27-jni(3) + 30-jvm-entry(3) + 32-jfr(6) + 34-nmt(2) + 36-attach(2) + 37-heap-dumper(2) + 39-runtime-monitoring(2) + 46-sa(1) + 14-c1(4,14 域完结) + 15-c2(2)**,下一篇 15-c2-compiler/03 | **上下文已满,本文件为非常详细交接版**——新 AI 只读本文件即可继续,不要依赖旧会话记忆
...
**下一步(唯一,无选择)**: 15-c2-compiler/03(IGVN + CCP + Escape Analysis,大纲 `planning/outlines/15-c2-compiler/03-c2-optimizations.md`;15 域 01/02 篇已完结,悬念已指向 03)。
...
第 6 批(JIT/GC): 14 ✅ → **15 ✅(2/8)** → 21 → 25 → 28 → 29 → 33 → 43
第 7 批(上层): 22 → 26 → 35 → 40 → 47
```

**已完成 111 篇**(全部在 `docs/openjdk/vol-02/`):
...
| **15-c2-compiler** | 1-2 | `15-c2-compiler/01-c2-ideal-graph.md`(253)+`02-c2-parse-graphkit.md`(230) | ✅ **15 域 2/8(本会话)** |
...
**本会话新增素材 26 个**(全部 gitignore 不入库,在 materials/commands/):
...
## 六、本会话实战经验(最重要,新 AI 必读;6.1-6.51 旧会话沉淀,6.52-6.66 本会话 15 篇新沉淀)
...
### 6.66 15-c2-compiler/02(Parse + GraphKit,15 域 2/8,大纲 19 处漂移含 4 处机制编造 + 深审 2 轮,2026-08-15)
...
- **"C1 只 inline tiny methods(size<35, depth≤2)" 错(重要)**: C1 同样用 MaxInlineSize/MaxInlineLevel/MaxRecursiveInlineLevel(c1_GraphBuilder.cpp:3801-3803)+**NestedInliningSizeRatio=90% 逐层衰减**(c1_globals.hpp:177,:700-705)——实证 C1 树 7 字节 d6 也 "callee is too large"(35×0.9^13≈6<7)
...
- [x] 15-c2-compiler/02——✅ 完结(正文 5e82ad9 含回填 ⚠️ 3 块 19 条/README 5fe6151);**15 域 2/8,第 6 批 3/8**
- [ ] **15-c2-compiler/03**(IGVN + CCP + Escape Analysis)——**下一篇**;大纲 `planning/outlines/15-c2-compiler/03-c2-optimizations.md`;15-c2-compiler/02 悬念已指向 15-c2-compiler/03
...
```
1. 读 planning/outlines/15-c2-compiler/03-*.md(注意 ⚠️ 块——01 篇 3 块 13 条、02 篇 3 块 19 条,03 篇大概率同样漂移;15-c2/03 重点: 优化三引擎——IGVN 深化(gvn.cpp/phaseX.cpp 的 PhaseCCP:ccp.do_transform+iterGVN2)、Escape Analysis(escape.cpp ConnectionGraph: 标量替换 scalar_replaceable/macro.cpp 的 PhaseMacroExpand eliminate_macro_nodes、12-02 域 bcEscapeAnalyzer 的对照)、Compile::Optimize 阶段编排(compile.cpp:2220+,PHASE_* 枚举 phasetype.hpp);与 01/02 篇衔接: transform_old 五步与 Parse 期单遍 GVN 已铺底,03 讲全局优化引擎;与 12-ci/02 衔接: ConnectionGraph 与 bcEscapeAnalyzer 的精度对照)
...
5. 15-c2 后 → 21-shared-runtime → 25-gc → 28-jvmti → 29-mh → 33-jmx → 43-nio-net(第 6 批剩余 5 域)
```
