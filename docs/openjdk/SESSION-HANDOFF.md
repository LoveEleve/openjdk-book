...
**下一步(唯一,无选择)**: 19-sync/03(enter/exit/wait——多线程怎么抢锁、怎么睡、怎么醒)。
...
第 4 批(执行/帧): 10(3/3) → 19(2/4) → 23 → 24 → 08 → 31 → 44   🚧 进行中
第 5 批(VM 核心): 11 → 12 → 13 → 18 → 20 → 27 → 30 → 32 → 34 → 36 → 37 → 39 → 46
第 6 批(JIT/GC): 14 → 15 → 21 → 25 → 28 → 29 → 33 → 43
第 7 批(上层): 22 → 26 → 35 → 40 → 47
```

**已完成 57 篇**(全部在 `docs/openjdk/vol-02/`,第 1 批 12 + 第 2 批 26 + 第 3 批 14 + 第 4 批 5):
...
| 19-sync | 1-2 | `19-sync/01`(157 行)/02(107 行) | 🚧 19 域进行中(2/4) |
...
- [x] **19-sync/01-02**(lock-hierarchy/objectmonitor-structure)——✅ 完成,commit 见 §二
- [ ] **19-sync/03**(enter-exit-wait,多线程怎么抢锁/睡/醒)——大纲在 `planning/outlines/19-synchronization/03-enter-exit-wait.md`;02 篇悬念指向它
...
```
1. 读 planning/outlines/19-synchronization/03-enter-exit-wait.md(大纲,注意 ⚠️ 块——19-01/02 已回填 15 条,03 大概率同样漂移;02 篇的悬念指向它: enter 排队/exit 唤醒/wait 流转/自旋与 _succ)
2. 验证大纲所有 file:line 与专有名词(按 §6.5-1 的规律;重点: objectMonitor.cpp(2455 行)的 enter/exit/wait/notify/notifyAll/EnterI/ExitEpilog/自旋 TrySpin/AdaptiveSpinning、synchronizer.cpp 的 inflate/deflate_idle_monitors/omAlloc;02 篇已确认字段行号,enter/exit 的行号大概率漂移需重验)
3. 按第三节流程写 → 自查(脚本 /data/tmp/opencode/check.py,新引用文件先加 MAPPINGS/HS_MAP;注意 ART 变量改回当前文件;星号检查已修复——代码块先剔再剔 span,正文裸转义星号须加反引号)→ 深审 2 轮 → 回填大纲 → 提交 → 更新 README
4. 19 域 4 篇完结后 → 23-stub
```
