...
> **状态**: 2026-08-12 | 卷 2 写作中: **40/152 篇完成**(第 1 批 12 ✅ + 第 2 批 26 ✅ 收官;第 3 批进行中: 07 域 2/7) | 上下文已满,本文件为**非常详细交接版**(350+ 行)——新 AI 只读本文件即可继续
...
**下一步(唯一,无选择)**: 07-classfile-classloader/03(Symbol 与 StringTable)。
...
**已完成 40 篇**(全部在 `docs/openjdk/vol-02/`,第 1 批 12 + 第 2 批 26 + 第 3 批 2):
...
- [ ] **07-classfile-classloader/03**(symbol-string-table)——大纲在 `planning/outlines/07-classfile-classloader/03-symbol-string-table.md`
...
```
1. 读 planning/outlines/07-classfile-classloader/03-symbol-string-table.md(大纲,注意 ⚠️ 块——02 篇 11 处漂移已回填)
2. 验证大纲所有 file:line 与专有名词(按 §6.5-1 的规律;重点: SymbolTable/StringTable 实际行号与并发哈希/锁机制,07-01 已铺垫 Utf8→SymbolTable 批量分配;文件名先 find 验证——symbolTable.hpp/stringTable.hpp 在 share/classfile/)
