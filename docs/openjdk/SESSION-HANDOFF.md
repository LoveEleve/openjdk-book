...
> **状态**: 2026-08-15 | 卷 2 写作中: **117/152 篇完成**(第 1 批 12 + 第 2 批 26 + 第 3 批 14 + 第 4 批 21 + 第 5 批 32 + 第 6 批 12) | 第 1-5 批**全部完结**;第 6 批(JIT/GC)进行中,**本会话 34 篇: 20-02 + 27-jni(3) + 30-jvm-entry(3) + 32-jfr(6) + 34-nmt(2) + 36-attach(2) + 37-heap-dumper(2) + 39-runtime-monitoring(2) + 46-sa(1) + 14-c1(4,14 域完结) + 15-c2(8,**15 域完结**)**;下一篇 21-shared-runtime(第 6 批第 3 个域) | **上下文已满,本文件为非常详细交接版**——新 AI 只读本文件即可继续,不要依赖旧会话记忆
...
**下一步(唯一,无选择)**: 21-shared-runtime(大纲 `planning/outlines/21-shared-runtime/`;**15 域已收官**,16-code-cache 等第 1-5 批域早完结,按 writing-order 第 6 批剩余: 21 → 25 → 28 → 29 → 33 → 43)。
...
第 6 批(JIT/GC): 14 ✅ → **15 ✅(8/8,15 域完结)** → 21 → 25 → 28 → 29 → 33 → 43
第 7 批(上层): 22 → 26 → 35 → 40 → 47
```

**已完成 117 篇**(全部在 `docs/openjdk/vol-02/`):
...
## 六、本会话实战经验(最重要,新 AI 必读;6.1-6.51 旧会话沉淀,6.52-6.72 本会话 21 篇新沉淀)
...
- **章节维护教训(连续 7 次犯,根因确认为"new 里带旧节标题"的替换模式,写进教训 6 次仍犯——必须改为一次性脚本)**: 错误模式=`new=新节内容+"### 6.xx 旧节标题"` 替换 `old="### 6.xx 旧节标题"`——new 带旧节标题导致新节插到标题前;已 7 次(6.65-6.72 每次);**正确操作模板(必须严格执行)**: `old = 旧节末条("- 实证: xxx-demo.txt")`, `new = old + "\n\n### 6.xx+1 新节..."`(new 不重复旧节标题);**插入后立即 `grep -n "### 6.6"` 校验连续递增,再做任何其他步骤**——本次(6.72)已在教训明示后仍犯,说明下一步必须把"插入+校验"合成一个脚本原子执行
...
- [ ] **21-shared-runtime**——**下一篇**;大纲 `planning/outlines/21-shared-runtime/`;15 域完结后按 writing-order 进入第 6 批第 3 个域
...
```
1. 读 planning/outlines/21-shared-runtime/*.md(注意 ⚠️ 块——15-c2 域 8 篇已回填(3 块 13/15/13/17/15/15/14/17 条),21 域大概率同样漂移;21-shared-runtime 域内容建议先读 KP(knowledge-planning/21-*.md)与 outlines 清单定篇数;与 15-c2 衔接: C2 编译产物(nmethod/CodeCache=16 域已完结)与 runtime 的交互;第 6 批剩余 6 域: 21 → 25 → 28 → 29 → 33 → 43,每域完成后按 §三 流程回填 ⚠️+提交+README+HANDOFF;HANDOFF §6 追加新节用正确操作模板(锚点=旧节末条实证行,new 不重复旧节标题,插入后立即 grep -n "### 6.6" 校验——教训 6.68)
...
5. 21-shared-runtime → 25-gc → 28-jvmti → 29-mh → 33-jmx → 43-nio-net(第 6 批剩余 6 域)
```
