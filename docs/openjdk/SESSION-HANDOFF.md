...
- **span 语义**: span=_total_allocated - priority_queue->total()(:167,**分配增量**,非"分配字节数");样本 span=该次分配的 allocated(:193);队满 quick reject(:171-175 peek->span()>span)+reuse(:176);样本队列默认 **256**(jfrOptionSet.cpp:173)
- **双锁**: sample 入口 **JfrTryLock 非阻塞**(:147,失败跳过);acquire/release(:94-104)是**自旋锁**(checkpoint 用)——大纲只提 tryLock
- **启用**: LeakProfiler::start(:41-77: 0 禁用/ZGC+Shenandoah 不支持 :53-62/StartOperation VM 操作 safepoint 安装 :68);jfrJniMethod.cpp:109;GC 集成 oops_do(:227-246)+scavenge(:201-225 span 转前驱)
