...
> **状态**: 2026-08-15 | 卷 2 写作中: **112/152 篇完成**(第 1 批 12 + 第 2 批 26 + 第 3 批 14 + 第 4 批 21 + 第 5 批 32 + 第 6 批 7) | 第 1-5 批**全部完结**;第 6 批(JIT/GC)进行中,**本会话 29 篇: 20-02 + 27-jni(3) + 30-jvm-entry(3) + 32-jfr(6) + 34-nmt(2) + 36-attach(2) + 37-heap-dumper(2) + 39-runtime-monitoring(2) + 46-sa(1) + 14-c1(4,14 域完结) + 15-c2(3)**,下一篇 15-c2-compiler/04 | **上下文已满,本文件为非常详细交接版**——新 AI 只读本文件即可继续,不要依赖旧会话记忆
...
**下一步(唯一,无选择)**: 15-c2-compiler/04(Loop Optimization + SuperWord: 循环变换与向量化,大纲 `planning/outlines/15-c2-compiler/04-c2-loops.md`;15 域 01-03 篇已完结,悬念已指向 04)。
...
第 6 批(JIT/GC): 14 ✅ → **15 ✅(3/8)** → 21 → 25 → 28 → 29 → 33 → 43
第 7 批(上层): 22 → 26 → 35 → 40 → 47
```

**已完成 112 篇**(全部在 `docs/openjdk/vol-02/`):
...
| **15-c2-compiler** | 1-3 | `15-c2-compiler/01-c2-ideal-graph.md`(253)+`02-c2-parse-graphkit.md`(230)+`03-c2-optimizations.md`(138) | ✅ **15 域 3/8(本会话)** |
...
**本会话新增素材 27 个**(全部 gitignore 不入库,在 materials/commands/):
...
## 六、本会话实战经验(最重要,新 AI 必读;6.1-6.51 旧会话沉淀,6.52-6.67 本会话 16 篇新沉淀)
...
- [ ] **15-c2-compiler/04**(Loop Optimization + SuperWord: 循环变换与向量化)——**下一篇**;大纲 `planning/outlines/15-c2-compiler/04-c2-loops.md`;15-c2-compiler/03 悬念已指向 15-c2-compiler/04
...
```
1. 读 planning/outlines/15-c2-compiler/04-*.md(注意 ⚠️ 块——01 篇 3 块 13 条、02 篇 3 块 15 条、03 篇 3 块 13 条,04 篇大概率同样漂移;15-c2/04 重点: 循环体系——IdealLoopTree 构建(loopnode.cpp build_loop_tree/is_counted_loop 判定)、PhaseIdealLoop(loopopts.cpp: 不变量外提/剥皮/展开 loopTransform.cpp unroll/loopUnswitch.cpp/loopPredicate.cpp 范围检查谓词)、SuperWord 向量化(superword.cpp 5218 行: 依赖分析/打包/pack/unpack);与 01-03 篇衔接: Node/Type/IGVN/CCP/EA 已铺底,04 讲循环专属优化;与 13-jit/02 衔接: OSR 在回边、Tier3BackEdge 阈值;注意范围检查谓词与 01 篇 remove_range_check_casts/opaque4 的关联)
...
5. 15-c2 后 → 21-shared-runtime → 25-gc → 28-jvmti → 29-mh → 33-jmx → 43-nio-net(第 6 批剩余 4 域)
```
