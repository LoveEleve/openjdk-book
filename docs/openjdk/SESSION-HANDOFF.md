...
> **状态**: 2026-08-15 | 卷 2 写作中: **116/152 篇完成**(第 1 批 12 + 第 2 批 26 + 第 3 批 14 + 第 4 批 21 + 第 5 批 32 + 第 6 批 11) | 第 1-5 批**全部完结**;第 6 批(JIT/GC)进行中,**本会话 33 篇: 20-02 + 27-jni(3) + 30-jvm-entry(3) + 32-jfr(6) + 34-nmt(2) + 36-attach(2) + 37-heap-dumper(2) + 39-runtime-monitoring(2) + 46-sa(1) + 14-c1(4,14 域完结) + 15-c2(7)**,下一篇 15-c2-compiler/08(15 域收官篇) | **上下文已满,本文件为非常详细交接版**——新 AI 只读本文件即可继续,不要依赖旧会话记忆
...
**下一步(唯一,无选择)**: 15-c2-compiler/08(library_call.cpp: 6991 行的 intrinsic 世界,大纲 `planning/outlines/15-c2-compiler/08-c2-library-calls.md`;**15 域收官篇**;15 域 01-07 篇已完结,悬念已指向 08)。
...
第 6 批(JIT/GC): 14 ✅ → **15 ✅(7/8,剩 08)** → 21 → 25 → 28 → 29 → 33 → 43
第 7 批(上层): 22 → 26 → 35 → 40 → 47
```

**已完成 116 篇**(全部在 `docs/openjdk/vol-02/`):
...
| **15-c2-compiler** | 1-7 | `01-c2-ideal-graph.md`(253)+`02-c2-parse-graphkit.md`(230)+`03-c2-optimizations.md`(138)+`04-c2-loops.md`(140)+`05-c2-register-alloc.md`(143)+`06-c2-codegen.md`(129)+`07-c2-macro-intrinsics.md`(74) | ✅ **15 域 7/8(本会话),剩 08** |
...
**本会话新增素材 31 个**(全部 gitignore 不入库,在 materials/commands/):
...
## 六、本会话实战经验(最重要,新 AI 必读;6.1-6.51 旧会话沉淀,6.52-6.71 本会话 20 篇新沉淀)
...
- **章节维护教训(连续 6 次犯,根因已确认仍犯——必须用工具固化)**: 错误模式=`new=新节内容+"### 6.xx 旧节标题"` 替换 `old="### 6.xx 旧节标题"`——new 带旧节标题导致新节插到标题前;已 6 次(6.65-6.71 每次),**正确操作模板**: `old = 旧节末条("- 实证: xxx-demo.txt")`, `new = old + "\n\n### 6.xx+1 新节..."`(new 不重复旧节标题,旧节标题保持在原位);**提交前必须运行 `grep -n "### 6.6"` 校验连续递增**——本会话每次都在写完 python 脚本后忽略校验直接进入下一步,导致修复滞后一轮
...
- [ ] **15-c2-compiler/08**(library_call.cpp: 6991 行的 intrinsic 世界)——**下一篇,15 域收官篇**;大纲 `planning/outlines/15-c2-compiler/08-c2-library-calls.md`;15-c2-compiler/07 悬念已指向 15-c2-compiler/08
...
```
1. 读 planning/outlines/15-c2-compiler/08-*.md(注意 ⚠️ 块——01-07 篇已回填(3 块 13/15/13/17/15/15/14 条),08 篇大概率同样漂移;**15-c2/08 是 15 域收官篇,写完后 15 域完结、第 6 批 2/8**;15-c2/08 重点: library_call.cpp(6991 行)intrinsic 机制——LibraryCallKit(generate_call/generate_arraycopy 之外: inline_string/inline_math/inline_unsafe 等)、callGenerator.cpp 的 find_intrinsic(vmIntrinsics::ID 分派)、intrinsicnode.cpp;与 02 篇衔接: do_call→call_generator→find_intrinsic(02 篇已见: intrinsic 优先于内联决策);与 07 篇衔接: arraycopy intrinsic→ArrayCopyNode→macroArrayCopy 展开;flag: PrintIntrinsics diagnostic(c2_globals.hpp:657 已在 03 篇素材)可用=实证主通道;注意 library_call.cpp 大而散,选 2-3 个代表性 intrinsic(如 System.arraycopy/StringBuilder/Math)深挖,勿全篇铺开)
...
5. 15-c2 后 → 21-shared-runtime → 25-gc → 28-jvmti → 29-mh → 33-jmx → 43-nio-net(第 6 批剩余 6 域)
```
