...
**下一步(唯一,无选择)**: 第 4 批 10-metaspace/01(类的元数据怎么分配+回收)——第 3 批三个域全部收官,进入第 4 批(执行/帧)。
...
**已完成 52 篇**(全部在 `docs/openjdk/vol-02/`,第 1 批 12 + 第 2 批 26 + 第 3 批 14):
...
| 17-threads | 1-4 | `17-threads/01`(191 行)/02(192 行)/03(165 行)/04(188 行) | ✅ **17 域完结(第 3 批第 3 个域收官)** |
...
- [x] **17-threads/01-04**——✅ **17 域完结(第 3 批第 3 个域收官,第 3 批 14 篇全部完成)**,commit 见 §二
- [ ] **10-metaspace/01**(第 4 批第一个域)——大纲在 `planning/outlines/10-metaspace/`;09 域悬念与 17 域 04 的悬念都指向第 4 批
...
```
1. 读 planning/outlines/10-metaspace/01-metaspace-overview.md(第 4 批第一个域,注意 ⚠️ 块——前 3 批已回填大量经验,第 4 批同样要全部重验;09 域悬念指向它: VirtualSpaceNode→ChunkManager→Metablock)
2. 验证大纲所有 file:line 与专有名词(按 §6.5-1 的规律;重点: metaspace.cpp/hpp 的 Metaspace::initialize/allocate、virtualSpaceList.hpp(09-02 已验证 :39)、chunkManager.hpp(:44)、metablock.hpp(:42)、metachunk.hpp;文件名先 find 验证——share/memory/metaspace/ 目录)
3. 按第三节流程写 → 自查(脚本 /data/tmp/opencode/check.py,新引用文件先加 MAPPINGS/HS_MAP;注意 ART 变量改回当前文件)→ 深审 2 轮 → 回填大纲 → 提交 → 更新 README
4. 第 4 批按拓扑: 10 → 19 → 23 → 24 → 08 → 31 → 44
```
