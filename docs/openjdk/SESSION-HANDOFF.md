...
> **状态**: 2026-08-12 | 卷 2 写作中: **44/152 篇完成**(第 1 批 12 ✅ + 第 2 批 26 ✅ 收官;第 3 批进行中: 07 域 6/7) | 上下文已满,本文件为**非常详细交接版**(350+ 行)——新 AI 只读本文件即可继续
...
**下一步(唯一,无选择)**: 07-classfile-classloader/07(javaClasses 核心类镜像,07 域收官)。
...
- [x] **07-classfile-classloader/06**(jpms-modules)——已完结,4a23fde
- [ ] **07-classfile-classloader/07**(javaclasses-core-mirrors,07 域收官)——大纲在 `planning/outlines/07-classfile-classloader/07-javaclasses-core-mirrors.md`
...
```
1. 读 planning/outlines/07-classfile-classloader/07-javaclasses-core-mirrors.md(大纲,注意 ⚠️ 块——06 篇 10 处漂移已回填;本篇是 07 域收官篇)
2. 验证大纲所有 file:line 与专有名词(按 §6.5-1 的规律;重点: javaClasses.cpp 的 compute_offsets/静态偏移字段(06-03 已见过 java.lang.Class 的 injected fields),String/Class/Thread 镜像字段;文件名先 find 验证——javaClasses.cpp/hpp 在 share/classfile/)
3. 按第三节流程写 → 自查(脚本)→ 深审 2 轮 → 回填大纲 → 提交 → 更新 README
4. 07 域 7 篇完结后 → 第 3 批 09-memory-core
```
