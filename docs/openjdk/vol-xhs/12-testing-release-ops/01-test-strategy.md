...
这轮进一步沉淀出的纪律是：**文档结论也必须和代码修复状态同步收口。** 如果代码已经修掉了 `FOR UPDATE` 路由、批量 Feign、支付补偿语义、推荐精排偏好维度，而测试与交接文档仍沿用旧口径，那么测试结论本身也会变成另一种“假成功”或“假风险”。因此在 `my-xhs` 这套方法里，测试策略不只验证代码，也要验证“代码状态、文档口径、运行态证据”三者有没有继续对齐。

同样地，对尚未落代码的容量热点也不能因为“看起来像问题”就写成“确定性 bug 已定位”。像 `ContentRecallStrategy` 这类 MySQL `LIKE` 标签召回，更适合在测试策略里被标成“运行态容量风险待专项压测”，而不是在没有新实现前就下过满结论。
...
## 验证强度如何反过来约束结论强度：这套策略不只是测试方法，也是项目表达纪律

前面已经讲了三层验证法和 15 层证据链，但还有一个特别值得单独点出来的价值：**这套策略不只是在约束"怎么测"，还在约束"怎么讲"。**

也就是说，它不只是测试方法论，还是项目表达纪律。因为 `my-xhs` 的历史已经反复证明，分布式系统最容易出问题的地方，不是"接口跑不通"，而是"结论说得比证据满"。

当前这套验证纪律至少在三个地方直接约束了项目表达：

### 第一层：L0/L1/L2 决定了你能说多满

- 只做到 L0（配置存在、文件存在、语法正确），只能说"静态成立"；
- 做到 L1（框架语义、端口语义、配置覆盖关系），才能说"按当前实现理解应如此"；
- 做到 L2（远程平台探测、真实请求、真实指标、真实 trace），才允许说"当前在线可用"。

这意味着在面试或交接里，你不能把"compose 里配了 Prometheus"说成"监控已上线"，也不能把"代码里有补偿任务"说成"补偿链已验证闭环"。验证层级直接约束了结论层级。

### 第二层：逐用例执行纪律约束了"成功"这个词的可信度

297/297 这类回归结果之所以有分量，不只是因为全绿，而是因为每一条用例都是单独执行、单独验证、单独归因的。也就是说，"全绿"在这里不是批量跑完后的统计结果，而是每一条证据都还能被追溯到单个动作。

这在面试表达里也很有价值：你不是说"我们测了很多"，而是说"我们每一条用例都单独执行，每一条证据都还能归因到具体操作"。

### 第三层：人工验证手册和程序化验证并存，说明这套策略不只是给机器看的

`MANUAL-VERIFY.md` 的存在说明，这套验证体系不只是自动化脚本，也是可交接、可复核、可演示的操作语言。也就是说，它不只是在证明"系统能跑"，还在证明"别人也能按同样方式验证系统能跑"。

这三层合在一起，其实就是在回答一个很关键的项目表达问题：**你凭什么说这个系统是对的？** 当前这套策略给出的答案不是"因为我们测过"，而是"因为我们按证据层级测过，每条结论都还能被追溯到对应层级的证据"。
