...
这也引出热搜链里一个很值得正面讲清的分布式判断：当前系统其实同时维护了两套时间语义。

- Redis 分钟桶 + 实时榜 ZSet 回答的是：**现在什么词正在变热**。
- MySQL `t_hot_search_snapshot` 回答的是：**某个时刻、某一分钟的榜单长什么样**。

这两者不是一个历史版本覆盖另一个实时版本，而是两类不同问题：

- 实时榜服务于当前产品入口和在线展示；
- 快照服务于回看、对账、运营复盘和“那个时刻到底热过什么”的历史追问。

所以热搜当前并不是“一个榜单存两份”，而是**一个实时信号系统 + 一个历史留痕系统**。这一点如果不讲透，后面很容易把 Redis ZSet 当成长期真相，或者反过来把 MySQL 快照当成实时榜。
