...
前面已经讲清楚，当前推荐不是热门榜，而是 5 路召回 + 粗排 + 精排 + 重排的完整 Pipeline。但这并不等于它已经是成熟的学习排序系统。

当前实现真正依赖的仍然是规则化收敛：

- 来源权重由 `SOURCE_WEIGHT` 显式给出，见 `my-xhs-search/src/main/java/com/myxhs/search/service/RecommendService.java:53` 到 `:60`
- 粗排是来源分统一归一化
- 精排是来源权重、偏好、质量、时效的线性融合
- 重排再做列表级约束

这说明当前推荐链的工程上限也很明确：

- 好处是结构清晰、可解释、容易落地；
- 代价是规则一旦增多，权重调参与策略耦合会迅速上升；
- 进一步进入更复杂个性化阶段时，当前 Pipeline 很可能要把“精排”这一层替换成真正的模型服务，而不是继续在线性规则上叠逻辑。

所以这篇在项目表达里最稳的说法不是“我们有成熟推荐引擎”，而是：**当前已经把推荐从热门榜推进到了规则化多路召回 Pipeline，但仍处在可解释规则系统阶段，而不是成熟 ML 排序平台阶段。**
...
但这里还有一个非常重要、也很容易在架构图里被讲漏的工程边界：**闭环存在，不等于闭环天然可靠。**

`RecommendService.reportBehavior()` 在 `my-xhs-search/src/main/java/com/myxhs/search/service/RecommendService.java:176` 到 `:212` 中，已经把这条链的真实工程取舍写出来了：

- 正常路径：行为先发 `RECOMMEND_BEHAVIOR_TOPIC`
- MQ 发送失败：降级同步写 `t_user_behavior`
- 只有连降级写库都失败，才抛出错误

这说明当前实现承认两件事：

1. 推荐行为是高频写，不能默认每次都同步写 DB；
2. 但行为又不能轻易丢掉，否则下一轮召回就会长期失真。

从分布式视角看，这条闭环并不是“读到了推荐 → 自然会反馈回去”的自动神话，而是一条会经过 MQ、降级写库、消费者和离线任务继续收尾的行为样本链。也正因为如此，历史 review 才会出现“行为消费者吞掉写库异常”“推荐离线任务内部失败仍向 XXL-Job 报成功”这类问题：推荐链最危险的不是召回空了，而是**行为样本静默丢失后，系统还以为自己的反馈闭环很完整。**
...
补：多路召回超时后，当前实现会主动取消未完成的 `CompletableFuture`，避免慢召回继续占满 `recallExecutor`。
补：已读去重集合仍用 Redis Set 精确存储，但加了 `MAX_SEEN_SET_SIZE` 上限保护；超过阈值时会主动缩短 TTL，避免长期活跃用户的已读集合无界增长。
