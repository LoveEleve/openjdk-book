...
补：跨实例推送失败时，当前实现只把普通聊天消息降级存离线；已读回执和 TYPING 这类瞬时信号不会离线重放，避免写入一个根本没有持久消息体可回放的伪离线记录。

## 一致性哈希负载均衡：同用户路由到同一 IM 实例

在 Redis Pub/Sub 定向路由之外，IM 还提供了一层更前置的路由能力：`my-xhs-im/src/main/java/com/myxhs/im/loadbalancer/ImConsistentHashLoadBalancer.java:38`。

它解决的是 WebSocket 会话粘性问题：同一 userId 的私信需要始终落在同一 IM 实例上，才能避免消息跨节点转发。

实现要点：

- 每个物理实例生成 150 个虚拟节点，均匀分布在一致性 Hash 环上，减少数据倾斜
- 基于 `X-User-Id` 映射到虚拟节点，选择最近实例——同一 userId 始终命中同一实例
- 实例上线/下线时重建 Hash 环；用 `cachedInstancesHash` 缓存实例列表指纹，仅在实例列表变化时才重建，避免每次请求都重建
- 无法从请求上下文提取 userId 时兜底轮询

需要注意：该 LoadBalancer 需要 `spring.cloud.loadbalancer.configurations=my-xhs-im-consistent-hash` 显式声明为默认 LoadBalancer 才生效，属于与 Redis Pub/Sub 定向投递相互独立的第二层路由机制。
