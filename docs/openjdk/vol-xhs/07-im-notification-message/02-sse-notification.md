...
但这里必须补一个分布式边界：SSE 的跨实例 Pub/Sub 是“即时体验通道”，不是通知持久化通道。`publishCrossInstance()` 返回成功，只能说明消息被 Redis 接受发布；如果目标实例在发布瞬间断开，订阅回调不会执行，而 `SseEmitterManager` 也不会像 IM 一样额外保存一份离线推送消息。当前通知之所以仍然可恢复，是因为通知实体和未读数已经分别落在数据库/Redis 视图里，用户下一次打开列表仍可看见；但这不等于跨实例 SSE 本身具备消息队列式重放能力。
...
如果继续沿通知这条用户感知链往前推，下一个自然的方向就是 `07-im-notification-message/03-message-aggregation.md`，因为 SSE 推送解决的是“怎么立刻把事件推给在线用户”，而通知聚合与未读计数要继续回答“这些提醒怎样长期收敛成用户能理解的列表和红点视图”。