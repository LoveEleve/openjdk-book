...
```
1. 读本交接文档（完整方法论 + 当前状态 + 缺陷档案 + 深度补充清单）
2. 读 00-domain-discovery-v3.md — 48域权威清单 + 源码路径
3. 读 outlines/01-os-abstraction/01-platform-detection.md — v5 标准示范
4. 从 §八 下一步计划 的 P0 项开始
4.1 域40 Launcher — 先补 questions + 自查（KP+2大纲已有）
4.2 域41 ZIP & JIMAGE — 补 02-jimage.md + questions（KP+01已有）
4.3 域42-48 — 按 §八 清单逐域做完整管线
4.4 待深补域 — 按 §八 清单逐域深补
```
...
| 优先级 | 域 | 问题 | 需要做什么 |
|:--:|:--:|------|------|
| P0-1 | **15 C2** | KP+8大纲已写但**03-08全部塌方**(19-23行/0场景/0C++) | 重写03-08全部6篇到v5标准; 01-02深化至60+行 |
| P0-2 | **40 Launcher** | KP+2大纲已有, 02仅25行+缺questions | 深化02(场景+源码+关键设计) + 补questions + 自查 |
| P0-3 | **41 ZIP & JIMAGE** | KP已写+01已有 | 补02-jimage.md + questions + 自查 |

### P1 — 质量偏薄需深化

| 域 | 问题 | 需要做什么 |
|:--:|------|------|
| 37 | 2篇31/36行, 02缺关键设计+跨层 | 深化02: 补关键设计+[C++] |
| 38 | 2篇36/36行, 跨层全0 | 补[C++]标注 |
| 39 | 2篇48/37行, 跨层全0 | 补[C++]标注 |
| 32 JFR | 6篇已修复, 04-06偏薄 | 回顾04/05/06场景+跨层 |
| 26 G1 | 7篇已修复, 02偏薄 | 02(concurrent mark)可深化 |
| 25 GC | 6篇已修复, 06源锚=1 | 补充06源锚 |

### P2 — 新域启动 (Group 10-11)

| 域 | 名称 | 行数 | 预估 |
|:--:|------|:--:|:--:|
| 42 | Core Native | ~15000 | 3-4 |
| 43 | NIO & Net | ~17500 | 3-4 |
| 44 | Class Verification | ~5000 | 2 |
| 45 | Math Library | ~6400 | 2 |
| 46 | SA Postmortem | ~3900 | 1 |
| 47 | Instrumentation | ~5300 | 2 |
| 48 | Utilities | ~25400 | 4-5 |

### 执行顺序建议

```
第一轮: P0-2(域40补全) → P0-3(域41补全) [~1h]
第二轮: P2 域42-48 逐域全管线 [~4h]
第三轮: P0-1 域15 C2 重做03-08 [~4h]
第四轮: P1-1~P1-6 薄域深化 [可选,~2h]
```
...
当全部48域完成后，新AI应执行以下 checklist 验证全量质量：

```
[ ] 48/48 knowledge-planning/*.md 存在，每文件四章完整(grep '^## 0' = 4)
[ ] 48/48 outlines/{域}/ 目录存在
[ ] 所有大纲文件四要素非零(grep '^场景:'/关键设计/[C++:/[x86:')
[ ] 所有大纲有核心悬念+桥(tail -2 → "下一篇"或"下域")
[ ] 所有大纲文字锚=0(grep '\(.*\.(hpp|cpp):(?!\d)')
[ ] 所有KP P1/P2/P3=3 (grep '^### P[123]')
[ ] 所有KP 🔴/🟡/🟢 分级标注存在
[ ] 域15 C2 03-08 全部 v5 达标(场景>0/源码>0/关键设计>0/跨层>0)
[ ] 域37-39 跨层标注已补齐
[ ] 所有 completeness-questions.md 存在，每文件≥5身份/≥8问
[ ] 源锚行号验证: 至少抽样对照源码(grep -n 'methodName' file.cpp 验证行号存在)
[ ] 整个 GROUP 群完成後回归检查初域是否有尾段薄
```
