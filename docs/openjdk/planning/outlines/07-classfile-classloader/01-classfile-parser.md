...
> - **字段排列顺序(写作期补充,易抄反)**: layout_fields 按 FieldsAllocationStyle(globals.hpp:940,默认 **1**)排偏移——**默认顺序是 longs/doubles、ints、shorts/chars、bytes、最后 oops**(注释 :4072);style 0 才是 oops 在前(:4067),硬编码偏移核心类(java.lang.Class/ClassLoader/Reference,:4038-4043)固定 style 0;static 字段偏移另算(fac->count[STATIC_*] → static_field_size,:3966-3975),落在 InstanceMirrorKlass 静态区
