...
> - **"三步走"严重简化**: 真实六步=①dictionary->find(带 protection domain,:653)②非 parallelCapable 拿**类加载器对象锁**(ObjectLocker,:678,与 Java 层 define 同锁)③锁内 find_class 复查(:694)④placeholder super_load_in_progress→handle_parallel_super_load(:701-713)⑤LOAD_INSTANCE 占位(:792)+加载(:819)+check_constraints/record_dependency/update_dictionary(:826-836,见下)⑥清理占位(:859)+protection domain 校验(:883-889,is_valid_protection_domain/validate_protection_domain);查字典共 4 次(:653/:694/:775/:800)
...
- **PlaceholderTable 大纲未提(核心机制)**: Hashtable<Symbol*>(placeholders.hpp:37);LOAD_INSTANCE/LOAD_SUPER 两种占位;bootstrap 不拿对象锁→在 SystemDictionary_lock wait 等第一请求者(:768)/双锁等待(double_lock_wait);check_seen_thread 检测循环→ClassCircularityError(:759,:813);RedefineClasses 靠占位符判断定义中(:734)
- **record_dependency(:836-840)**: 定义 loader 在发起 loader 存活期不被卸载(非双亲委派依赖)——大纲未提
