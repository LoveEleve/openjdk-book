...
- **mirror 可变大小(大纲未提)**: create_mirror(javaClasses.cpp:894+)=InstanceMirrorKlass::instance_size(instanceMirrorKlass.cpp:40-46,=size_helper()+static_field_size(),静态字段住镜像里)+allocate_instance(:48-56);oop_size/static_oop_field_count 注入字段自描述大小(:1279-1291,读回 :58-60);fixup_mirror_list + Universe::fixup_mirrors(systemDictionary.cpp:2023)补齐早期镜像;getClass() 链路=Object.java:72 native→jni_GetObjectClass(jni.cpp:1292-1300)→obj->klass()->java_mirror()(与 as_Klass 反方向)
- **String.value 永远是 byte[](07-03 REVIEW 联动修正)**: 07-03 正文/大纲旧写 "latin1→byte[]、utf16→char[]" 错——basic_create(javaClasses.cpp:252-253)无论编码都是 new_byteArray,UTF-16 长度翻倍(String.java:140);07-03 正文与大纲 ⚠️ 块已同步修正
