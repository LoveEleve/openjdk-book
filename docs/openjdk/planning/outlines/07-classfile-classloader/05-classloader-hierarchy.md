...
> - **CLD 真实生命周期**(大纲简化): _klasses 链表(add_class,06-03/07-01 讲过);ClassLoaderDataGraph::do_unloading(classLoaderData.cpp:1373): is_alive(:696-701,**keep_alive 计数||_holder 弱句柄未清,无 JNI 强引用检查**)→free_deallocate_list+purge_previous_versions/死→**unload()**(:597,标记+清 deallocate 列表+通知 JVMTI,Metaspace 随后整批归还)+链表移除(:1394-1412)
