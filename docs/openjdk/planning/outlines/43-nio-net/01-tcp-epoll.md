...
> - **"PollArrayWrapper.poll(timeout) → wait for EPOLLOUT ready" 编造(重要)**: PollArrayWrapper 是 **windows 类**(src/java.base/windows/classes/sun/nio/ch/);linux 的带超时 connect 用 **poll(2)**——`struct pollfd {fd, POLLOUT}` + `NET_Poll(&pfd, 1, timeout)`(PlainSocketImpl.c:348,EINTR 按 JVM_NanoTime 重算 :358-369,超时 SocketTimeoutException+shutdown :373-382);**POLLOUT 不是 EPOLLOUT**
...
> - **"socketClose0 linger→RST" 编造(JDK8 形态)**: JDK11=**deferred close**(useDeferredClose): marker_fd(getMarkerFD :73,socketpair+shutdown 注释 :68-72)→NET_Dup2(marker_fd, fd)(:783-786)顶替让阻塞 read/write 立即返回;linger 在 socketSetOption0(:826)单独设置
> - **"EPoll.c:40-97/:58-65/:68-80/:82-97" 行号偏 1**: EPoll.c **97 行**;eventSize :41/eventsOffset :47/dataOffset :53(布局三兄弟,大纲漏)/create :59(epoll_create(256),注释 "size hint not used in modern kernels")/ctl :69/wait :83(jlong_to_ptr unsafe 裸内存零拷贝 :86,EINTR→IOS_INTERRUPTED :89-90)
