...
> - **vtable 构建行号漂移**: compute_vtable_size_and_num_mirandas :56-120(get_mirandas :93-94);**initialize_vtable 先复制父表再覆写/追加**: initialize_from_super :138-155(copy_vtable_to,:151,父表整体复制进子表),再逐方法 update_inherited_vtable :368——override 覆写复制来的槽位、父表没有的追加(注释 :205-206 "if override, replace in copy of super vtable, otherwise append to end";private/static/<init> 不占 :397-400;final 不新增 entry :402-406;数组类不引入新槽 :203-204)
