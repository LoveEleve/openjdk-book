...
> - **第 3 轮 REVIEW 补充(2026-08-13)**: ①wide 执行链精确化: _wide(196)模板读 bcp[1]+jump ArrayAddress→_wentry_point(templateTable_x86.cpp:4504-4510,注释 "rbcp increment step is part of the individual wide bytecode implementations");宽模板出口仍走共享 dispatch 表;②iadd 消费精确化: iop2(templateTable_x86.cpp:1337-1340)=pop_i(rdx)+addl(rax,rdx),非"消费 rax 两个操作数";③deopt 三态: reexecute 走 deopt_reexecute_entry(method,bcp)(内部对 _return_register_finalizer 特判走 deopt_reexecute_return_entry,templateInterpreter.cpp:339-352),24-03 的 unpack 三态对应;④字节码表初始化时机: init_globals→bytecodes_init(init.cpp:104)早于模板表(TemplateInterpreter::initialize 内 TemplateTable::initialize :50);⑤dispatch 表二维 _table[10][256] 选行依据=执行器当前栈顶状态(=上一条指令 tos_out)
