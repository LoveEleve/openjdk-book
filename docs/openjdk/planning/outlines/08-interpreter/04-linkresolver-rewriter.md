...
> - **"newarray → fast_newarray" 编造**: 枚举里不存在该字节码;Rewriter 替换指令字节的只有两类=lookupswitch→fast_linearswitch/fast_binaryswitch(scan_method :394-402,按 BinarySwitchThreshold)与 ldc/ldc_w→fast_aldc/fast_aldc_w((*bcp)=_fast_aldc :355,String/MH/MT/引用型 condy);getfield→fast_igetfield 是解释器运行时 patch(02 篇),不是 Rewriter;rewrite 时机=instanceKlass.cpp:851-857 rewrite_class(验证后首次执行前,is_rewritten 只一次)
