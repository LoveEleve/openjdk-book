...
> - **"GuardedMemory canary 0xBAADF00D/0xDEADBEEF" 编造**: 守卫=GUARD_SIZE 16 字节填充 **badResourceValue(0xAB)**(globalDefinitions.hpp:1012,Guard::build/verify guardedMemory.hpp:96-112);**guarded_malloc 函数不存在**——用法=wrap_copy/free_copy(guardedMemory.cpp:31-54)+verify_guards(guardedMemory.hpp:212);GuardedMemory : StackObj(:83);**"只在 debug build 启用" 不准(第 3 轮 REVIEW)**: 真实用户=jniCheck(-Xcheck:jni 在 release 也可用,jniCheck.cpp:384 wrap_copy/:395,422-433 free_copy,get_tag 存原始指针)
