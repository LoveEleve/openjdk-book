...
> - **load0 真实流程**(大纲大幅简化): isBuiltin?procHandle:JVM_LoadLibrary(:354)→findJniFunction(:290-330,**普通库只找 JNI_OnLoad;JNI_OnLoad_<库名> 变体仅 builtin 库**——cname 只传给 isBuiltin 路径 :357-359,buildJniFunctionName=sym+"_"+cname jni_util_md.c:53-59;基础符号 JNI_ONLOAD_SYMBOLS jvm_md.h:40)→无入口 jniVersion=0x00010001(:365)→JVM_IsSupportedJNIVersion 校验(:378-389,失败 UnsatisfiedLinkError "unsupported JNI version 0x%08X")→handle(:400)/jniVersion(:390) 存回(javadoc 用途 ClassLoader.java:2406-2412);execstack 库: dlopen 改变栈可执行属性、栈保护页丢失,修复(逐线程重设 guard_memory)在 safepoint/VMThread 执行(os_linux.cpp:1883-1927,dll_load_in_vmthread :2126-2152,加载本身未必在 VMThread)
