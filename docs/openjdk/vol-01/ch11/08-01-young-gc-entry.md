...
### 1.4 触发 GC——attempt_allocation_slow 的决策循环

前面的三级挽救都失败了（Retained Region 没空间、持锁后 Eden 也拿不到新区、GCLocker 扩展也不行），控制权到达 `attempt_allocation_slow()`（g1CollectedHeap.cpp:410-516）。

这个方法的核心结构是一个 **for 循环**，不是单次尝试。原因：GC 可能因为 GCLocker 被拒绝，也可能因为别的线程抢先了而失败——必须能重试。

**循环里做三件事，循环本身不设退出条件（`for (;;)`），靠内部的 return 退出：**

```
for (;;) {
    // (1) 持锁阶段: 最后试一次持锁分配, 如果成功直接返回
    // (2) GC 阶段: 如果需要 GC, 发起 do_collection_pause
    //    成功+有空间 → 返回对象
    //    成功+没空间 → 返回 NULL (失败)
    //    失败了(被抢先) → 回到循环头重试
    // (3) GCLocker 阶段: 如果需要等 GCLocker, stall 等它释放
    //    等太多次 → 放弃, 返回 NULL
    // (4) 无论走了 (2) 还是 (3), 出锁后先无锁试一次分配
    //    别的线程可能刚做了 GC 释放了空间 → 直接受益 → 返回
}
```

**为什么会有"无锁重试"这一步**——多个线程可能同时走到 slow path。线程 A 抢先执行了 GC（步骤 2），线程 B 在 Heap_lock 上排队等。A 的 GC 完成释放了大量 Eden Region，B 拿到锁后在持锁重试（步骤 1）中就分配成功了。如果 B 没走持锁重试也没走 GC（比如 GCLocker 让它走了 stall），等出锁后在步骤 4 的无锁重试中仍能受益于 A 刚释放的空间。

**GCLocker stall 的终止条件**——不无限等：`gclocker_retry_count > GCLockerRetryAllocationCount` 时放弃（返回 NULL）。防止 JNI critical section 永远不释放导致永久卡死。

**关键：触发 GC ≠ 自己需要 GC**。线程 A 触发 GC，线程 B、C、D 在锁上排队，醒来后分配成功——它们一次 GC 都没做但都受益了。
