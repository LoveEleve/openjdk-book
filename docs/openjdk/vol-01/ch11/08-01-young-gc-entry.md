...
"分配失败"之后，最终的控制权交到了 `attempt_allocation_slow()`。这个方法的结构被误报很多，这里逐行拆开。

```cpp
// g1CollectedHeap.cpp:410-516
HeapWord* G1CollectedHeap::attempt_allocation_slow(size_t word_size) {
    HeapWord* result = NULL;

    // ★ 关键——这是一个 for 循环，不是一次性尝试
    for (uint try_count = 1; /* no condition */; try_count++) {
        ...
        // ── 持锁阶段 ──
        {
            ...
            // 步骤 1: 持锁重试分配
            result = _allocator->attempt_allocation_locked(word_size);  // line 433
            if (result != NULL) return result;

            // 步骤 2: GCLocker 紧急扩展
            if (GCLocker::is_active_and_needs_gc()
                && g1_policy()->can_expand_young_list()) {  // line 441
                result = _allocator->attempt_allocation_force(word_size);  // line 444
                if (result != NULL) return result;
            }

            // 步骤 3: 判断能不能做 GC
            should_try_gc = !GCLocker::needs_gc();          // line 452
            gc_count_before = total_collections();           // line 454
        } // 释放 Heap_lock
        ...
        if (should_try_gc) {
            ...
            result = do_collection_pause(word_size, gc_count_before, &succeeded,
                                         GCCause::_g1_inc_collection_pause);  // line 459
            if (result != NULL) {
                return result;    // GC 成功回收空间并分配了对象
            }
            if (succeeded) {
                return NULL;      // GC 成功执行但无法分配 → 彻底失败
            }
            // GC 没能执行（别的线程抢先做了 GC）→ 继续循环
        } else {
            // GCLocker block → 卡住等
            GCLocker::stall_until_clear();                   // line 488
        }

        // ── 无锁重试 ──
        // 无论走了 GC 还是 GCLocker stall，出锁后都可能已经有空间了
        //（别的线程抢先做了 GC 释放了 Region）
        result = _allocator->attempt_allocation(word_size, word_size, &dummy);  // line 501
        if (result != NULL) {
            return result;        // 别��线程的 GC 帮我们释放了空间
        }
    }
}
```
...
| 路径 | 条件 | 返回值 |
|------|------|--------|
| **持锁分配成功** | `attempt_allocation_locked()` 或 `attempt_allocation_force()` 成功 | 对象地址 |
| **GC 成功且分配成功** | `do_collection_pause()` → `succeeded=true, result!=NULL` | 对象地址 |
| **GC 成功但分配失败** | `do_collection_pause()` → `succeeded=true, result==NULL` | NULL（失败） |
| **别线程的 GC 帮忙** | 循环底部的无锁 `attempt_allocation()` 成功 | 对象地址 |
| **GCLocker stall** | 不退出——卡住直到可以重试 | 继续循环 |
| **GC 没成功（被抢先）** | `do_collection_pause()` → `succeeded=false` | 继续循环 |

**关键——N 个线程可以同时走到 slow path**。先到的线程触发 GC，后到的在 `Heap_lock` 上排队。GC 完成后 Eden 有空闲 Region，后到的线程在持锁重试（步骤 1）或循环底部的无锁重试中直接分配成功——"GC 不是我触发的，但我受益了"。
