...
G1 用 `_refill_waste_limit` 做容差（threadLocalAllocBuffer.hpp:57）。这个值初始等于 `TLAB 大小 / TLABRefillWasteFraction`（默认 64）。如果 TLAB 剩余空间大于这个阈值，**不退休 TLAB**——直接在 Eden Region 上用 CAS 做一次分配（绕过 TLAB 的本��� bump，目标仍是 Eden）。只有剩余空间小于等于阈值时才退休当前 TLAB、申请新的。
...
G1CollectedHeap::attempt_allocation_slow(word_size) {
    ...
    if (result != NULL) return result;   // 持锁后重试成功 → 不触发 GC

    // GCLocker 紧急扩展
    if (GCLocker::is_active_and_needs_gc()
        && g1_policy()->can_expand_young_list()) {                 // line 441
        result = _allocator->attempt_allocation_force(word_size);  // line 443
...
`do_collection_pause()`（g1CollectedHeap.cpp:2501-2511）创建 `VM_G1CollectForAllocation` 操作，提交给 VMThread 执行。VMThread 发起 safepoint——arming polling page、等所有线程停下——进入 `do_collection_pause_at_safepoint()`。

### 1.5 两个线程走到 slow path 的竞争

两个线程可能同时走到 slow path。先到的触发 GC，后到的在 `Heap_lock` 上排队。GC 完成后 Eden 有空闲 Region，后到的线程 `attempt_allocation_locked()` 直接分配成功——这就是 "GC 不是我自己触发的，但我受益了"。
...
|------|----------|-----------|
| 触��� | `_refill_waste_limit` 判断 | Region 无法分配（`par_allocate` 返回 NULL） |
