...
G1 的优化重点在"暂停时间可控"——通过预测（`_analytics` + `_mmu_tracker`，ch11/04）限制 CSet 大小来控制 GC 工作量。但注意 **GC 日志里显示的 `8.234ms` 不包括到达时间**——如果某个线程卡在长循环里，TTSP 可能额外增加数毫秒，GC 日志不会反映这部分。排查"GC 暂停为什么这么长"时先确认 TTSP 是否占了大头（用 §2.4 末尾列出的工具）。
