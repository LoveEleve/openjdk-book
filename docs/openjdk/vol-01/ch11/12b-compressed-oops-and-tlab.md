...
> **本文定位**：`Universe::initialize_heap()`（universe.cpp:764-823）五个阶段中的 ③④⑤——补完 ch11 遗留的缺口。③ 设 TLAB 全局上限、④ 决定压缩指针编码模式、⑤ 启动 TLAB 子系统。ch11 前面的文章讲完 create_heap 和 G1CollectedHeap::initialize()，本文把 initialize_heap() 剩余的三行核心调用讲完。
>
> **前置依赖**：[ch11/12](12-summary.md)（总结章，指出本缺口）；ch11/03（reserve_heap 与压缩指针 base 的预约）。
...
jint Universe::initialize_heap() {
  _collectedHeap = create_heap();                          // ① 已讲（ch11/04）
  jint status = _collectedHeap->initialize();              // ② 已讲（ch11/02-11）
  if (status != JNI_OK) {
    return status;
  }
...
`reserved_region().end()` 是堆预约的**末尾地址**（ch11/03 的 mmap 完成后才有值）。四个组合：
...
第 ④ 步**只负责最终确定 shift/base**——堆落在哪是 ch11/03 的 `Universe::reserve_heap()` 早就决定的。而那里远不是一次 mmap 那么简单：`ReservedHeapSpace` 构造走 `initialize_compressed_heap()`（virtualspace.cpp:490-600），**从最优模式到最差模式逐级尝试**，每级失败才降级到下一级：
...
**2. 堆 ≤ 32GB 也可能落到高位地址**。低 32GB 地址空间被占用时（ASLR 随机化、其他进程/库先 mmap 占了低地址），即使堆只有 8GB，落点在 64GB 处也走 DisjointBase。这正是 ch11/03 里 reserve_heap 优先尝试低地址预留的原因，也是五级降级存在的意义——**尽可能把堆留在可压缩的地址区间**。
...
```
① create_heap()                       ch11/04
② _collectedHeap->initialize()        ch11/02-11
...
此后的流程回到 `universe_init()`（ch10/01 全景）：SystemDictionary::initialize_oop_storage → Metaspace::global_initialize → ... → SymbolTable/StringTable::create_table。
...
> **下一篇**：[ch11/12 总结](12-summary.md)——回顾全部 12 篇文章如何将 `initialize_heap()` 五阶段逐行讲透。
