...
>
> **前置依赖**：ch14/01 SymbolTable 初始化（哈希表与去重的基础概念、对比基线）。
>
...
> **注**：① 的 OopStorage 是什么、为什么需要它（GC 搬对象后，谁来更新你存下的对象地址）——完整设计推演见 [ch10/02 OopStorage](../ch10/02-oopstorage.md)。本文 §2 只展开 StringTable 用到的弱引用语义。
...
- **包装 Handle**：`Handle h_string(THREAD, string)` 将 oop 存入线程 HandleArea 槽位——`intern()` 内部可能触发 GC（分配字符串、去重），裸 oop 会野；Handle 通过槽位间接读写，GC 更新槽位值后仍拿到最新地址（原理见 ch10/02）。这不是多余的来回包装——JNIHandleBlock（jstring 的槽位）和 HandleArea（Handle 的槽位）是两套不同的池子：JNI 管着 JNIHandleBlock 的生命周期（native 返回时自动释放），内部代码管着 HandleArea（HandleMark 按 C++ 作用域释放）；两者不能互用，`intern()` 的 GC 暴露点在 HandleMark 作用域内，必须用自己的池子。**先于 `as_unicode_string` 包装**——因为 `as_unicode_string` 使用 `CHECK_NULL`（内部可能因 OOM 抛异常触发 GC），此时 oop 已在 Handle 保护下
