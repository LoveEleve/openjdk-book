...
>
> **前置依赖**：ch14/01（Hashtable 基础、锁模型）、ch14/02（OopStorage、弱引用概念）；ch13（`method_idnum`、redefine 概念）。
...
> 时序：RMT 的 `create_table()` 在 `universe_init` 第 747 行调用——在 `SystemDictionary::initialize_oop_storage()`（第 692 行）之后，`vm_weak_oop_storage`（ch10/02 讲过的 OopStorage 实例）已经建好。RMT 的条目存弱引用时是从这个已建好的池子里**租槽位**。
...
add_method(method, rmethod_name):
  ...
  ① redefine 检查: method->is_old()？
      是 → 用 method_with_idnum 换新方法（呼应 ch13）
...
两处呼应前文：
- **redefine 检查用 `method_with_idnum`**——正是 ch13 讲的机制：idnum 不变，取到最新版本
...
这保证了缓存里的 Method* 永远是当前版本——正是 ch13 讲的"idnum 不变、方法指针自动跟随"思想在另一处的应用。
