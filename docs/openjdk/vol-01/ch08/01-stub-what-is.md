...
> **前置依赖**：[ch07 VM_Version_init](openjdk/vol-01/ch07/01-cpuid.md)：汇编桩生成基础设施就绪
> → **后续**：[ch08/02 StubRoutines 表](02-stubroutines-table.md)：call_stub 等函数指针的注册表
...
**为什么需要 stub 而不是直接用 C++ 函数调用？** C++ 编译器和 JIT 编译器使用不同的调用约定——C++ 函数调用通过寄存器（x86_64 的 rdi/rsi/rdx/rcx/r8/r9）或栈传参，并且 caller-saved/callee-saved 寄存器划分不同。JIT 编译的 Java 方法可能在任意寄存器中持有活跃的 Java 对象引用——如果 C++ 代码直接 `call` Java 编译代码，C++ 的调用约定可能破坏这些寄存器。stub 在两种调用约定之间插入一层"适配器"——在 C++ 侧按 C++ 约定接收参数，内部切换到 Java 侧约定再跳转。这类似于网络协议栈中 MAC 层和 IP 层之间的帧封装——上层不需要知道下层的帧格式。

**为什么 stub 是汇编而不是 C++ 生成？** C++ 编译器生成的 `call` 指令自动 push 返回地址到栈，自动调整 rsp——程序员无法控制在哪个寄存器中留下什么值。但 stub 需要在跳转到 Java 编译代码之前精确控制每个寄存器——Java 编译代码期望 r15 是 `Thread*`、r13 是 `bcp`（bytecode pointer），这些约定在 C++ 编译器看来是"被破坏的寄存器"。C++ 不可能生成"保留 r15 = Thread* 的同时调用其他函数"的代码——编译器优化会把 r15 当成可用寄存器复用。汇编是唯一能精确控制寄存器布局的方案。
