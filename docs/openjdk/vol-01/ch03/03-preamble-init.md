...
`dlsym` 是动态链接器的查找函数——第一个参数是一个 `dlopen` 返回的句柄，第二个参数是符号名，返回该符号在内存中的地址。第一章用过 `dlsym(libjvm_handle, "JNI_CreateJavaVM")`——在 `libjvm.so` 里找。

这里 `RTLD_DEFAULT` 是一个特殊值，传给 `dlsym` 时表示"在全局符号表中搜索"——即搜索主程序和所有已加载的 `.so`。`glibc` 的 `dlfcn.h` 定义它为 `(void*)0`，但这不是真的空指针——`dlsym` 内部识别它并切换到全局搜索模式。Linux 上 `clock_gettime` 在 glibc 里，所以能直接找到。
