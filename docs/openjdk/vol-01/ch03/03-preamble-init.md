...
第一步，通过 `dlsym` 在当前进程符号表中查找 `clock_gettime`。`NEEDS_LIBRT` 在 Linux 上未定义，直接走 `RTLD_DEFAULT`——不需要 `dlopen`：

```c
// NEEDS_LIBRT 未定义，dlopen 分支不编译
handle = RTLD_DEFAULT;
int (*clock_gettime_func)(clockid_t, struct timespec*) =
    (int(*)(clockid_t, struct timespec*))dlsym(handle, "clock_gettime");
if (clock_gettime_func(CLOCK_MONOTONIC, &tp) == 0) {
    _clock_gettime = clock_gettime_func;    // 存到函数指针
}
```
...
第二步，查找 `pthread_condattr_setclock`，把条件变量的时钟源也设为单调时钟：
...
if (condattr_setclock_func != NULL) {
    if (_pthread_condattr_setclock(_condAttr, CLOCK_MONOTONIC) == 0) {
        ...
        // 降级：不支持 CLOCK_MONOTONIC 条件变量
    }
}
```

最后 `pthread_init_common()` 初始化全局条件变量和互斥锁——HotSpot 线程同步的基础设施。
