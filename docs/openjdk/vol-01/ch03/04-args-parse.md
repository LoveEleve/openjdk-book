...
public:
  bool is_readable() const {                // 是否可读
    return !_internal                       // 非 internal 属性即可读
...
void LogFileOutput::set_file_name_parameters(jlong vm_start_time) {
  ...
  time_t utc_time = vm_start_time / 1000;          // 毫秒转秒
  os::localtime_pd(&utc_time, &local_time);        // UTC转本地时间
...
> **★★★ 核心关注：** ergo 自动补全了哪些关键 flag，以及补全的依据（物理内存 决定 堆大小、CPU 核数 决定 编译器线程数）。不需要逐行阅读源码。
...
- **CompressedOops**：64 位平台上，如果堆大小不超过 32GB，自动启用压缩对象指针（8 字节引用 压缩为 4 字节，节省内存）。
...
37 行 `thread.cpp` 代码驱动了约 3000 行参数解析逻辑。整个 Stage 2 做了一件事：**把用户命令行输入的字符串选项变成 JVM 可读取的 200+ 个全局变量。**

数据流：

```
init_system_properties    -- 初始化 4 个规范属性 + 5 个 OS 属性占位符（链表）
    ↓
JDK_Version_init          -- dlsym(JDK_GetVersionInfo0) 获取 JDK 版本号，用于后续
                            flag 默认值的版本区分
    ↓
init_version_specific_system_properties  -- 追加 3 个版本相关属性到链表
    ↓
LogConfiguration::initialize  -- 统一日志框架初始化（StdoutLog + StderrLog）
    ↓
Arguments::parse           -- 4 路参数源（vm_options/命令行/两个环境变量），按优先级
                            覆盖，把字符串匹配到对应的 flag 全局变量上
    ↓
os::init_before_ergo       -- 计算并保存 9 个 OS 级变量（CPU 核数、大页大小、栈守卫区大小）
    ↓
Arguments::apply_ergo       -- 自动推算用户没指定的 flag：GC 类型、堆大小、编译器线程数等
    ↓
check_ranges / check_constraints / mark_startup  -- 校验 + 快照启动值
```

这些函数**每个都只做变量赋值**，不创建任何 Java 对象、不分配 GC 堆、不加载类。被赋值的变量分布在几类存储位置：

| 存储位置 | 示例 | 赋值来源 |
|---------|------|---------|
| `Arguments` 静态成员 | `_system_properties` 链表 | `init_system_properties` + `os::init_system_properties_values` |
| `SystemProperty` 链表 | 13 个节点（key + value + 权限标记） | 同上 |
| `os::Linux` 静态成员 | `_processor_count`, `_physical_memory`, `_large_page_size` | `os::init` (Stage 1) + `init_before_ergo` (Stage 2) |
| `JavaThread` 静态成员 | `_stack_red_zone_size`, `_stack_yellow_zone_size` 等 | `init_before_ergo` 保存值；`create_stack_guard_pages` 实际用 `mprotect` 保护 |
| `JVMFlag` 全局变量 | `UseG1GC`, `MaxHeapSize`, `CICompilerCount` 等 200+ 个 | `parse`（用户指定）+ `apply_ergo`（自动推算） |

`parse` 之后，200+ 个 flag 已就位——GC 选好了、堆大小算好了、编译器线程数确定了、日志框架初始化了。接下来 `HOTSPOT_VM_INIT_BEGIN` 进入 Stage 3，`init_globals` 开始用这些值搭建 JVM 的运行时结构。
