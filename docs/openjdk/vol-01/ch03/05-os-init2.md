...
`HOTSPOT_VM_INIT_BEGIN()` 是一个 DTrace 探针标记。DTrace 是 Solaris 系统上的动态追踪框架，允许在不重启进程的情况下插入观测点来监控系统行为。Linux 内核不原生支持 DTrace，所以标准 JDK 11 构建（包括本机）都使用 `dtrace_disabled.hpp`，其中定义 `#define HOTSPOT_VM_INIT_BEGIN()` 为空宏——编译后完全不存在。本机 `-XX:+PrintFlagsFinal` 输出中也确认所有 DTrace 相关 flag（`DTraceAllocProbes`、`DTraceMethodProbes` 等）均为 false。生产环境从不开启，仅 Debug 构建或 Solaris 上的特殊 JDK 构建才会启用。
