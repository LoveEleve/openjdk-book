...
`HOTSPOT_VM_INIT_BEGIN()` 是一个 DTrace 探针标记，定义在 `dtrace_disabled.hpp:110`：

```c
#define HOTSPOT_VM_INIT_BEGIN()       // 默认构建：空宏
```

只有在启用 DTrace 编译时才会展开为实际的探针指令。本机 JDK 没有启用 DTrace 支持，这行编译后不存在。
