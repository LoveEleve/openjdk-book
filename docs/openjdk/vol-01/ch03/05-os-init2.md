...
最底层是一个 park/unpark 原语——只做一件事：让当前线程睡眠，或被唤醒。它内部用了 `pthread_mutex_lock` + `pthread_cond_wait`，但 HotSpot 没有把它直接暴露为锁接口，而是在它之上堆了四层才形成最终的全功能 Monitor。`PlatformEvent`（`os_posix.hpp`）的代码非常短：
