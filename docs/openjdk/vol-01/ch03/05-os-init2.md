...
最底层是一个 park/unpark 原语。HotSpot 的锁在**需要让线程睡眠时**最终会掉到 `pthread_mutex_lock` + `pthread_cond_wait`——下面展示的代码也证明了这一点。但 HotSpot 不能把 `pthread_mutex_t` **直接作为 JVM 代码的锁接口**暴露出去，因为加锁和等待期间需要插入自己的逻辑：检查 safepoint、切换 JavaThread 状态、管理自建的等待队列。所以在 `pthread_mutex` 之上包了一层 `PlatformEvent`（`os_posix.hpp`）：
...
Monitor::lock()
  ...
  ├─ TrySpin()                ← 等不到就自旋 20 轮 (指数退避)
  ├─ ThreadBlockInVM           ← Java 线程状态: _thread_in_vm → _thread_blocked
  │                              (让 safepoint 的 VMThread 知道这个线程已停下)
  └─ ILock()                   ← 慢路径
