...
`chunkpool_init()` 创建四个 `ChunkPool`。这是后面 `Thread::Thread()` 构造函数中 `new ResourceArea()` 和 `new HandleArea()` 的前提——没有这四池，线程的临时内存池就分配不了第一个 Chunk。从最底层往上逐层看清整个体系。

#### Chunk —— 可链接的大块内存

`Chunk`（`arena.hpp:45-89`）是一块连续内存，内部切成头部 + 数据区两部分。每个 `Chunk` 通过 `_next` 指针串成链表。

```cpp
class Chunk : CHeapObj<mtChunk> {
  Chunk*       _next;     // 链表指针
  const size_t _len;      // 数据区长度
...
`bottom()` 是数据区起点（跳过了 Chunk 头部），`top()` 是终点。`slack=40` 是 `sizeof(Chunk)+malloc` 头部开销估计——让总分配略小于 2^k，避免 buddy allocator 浪费。
...
│ Chunk头部  │ 数据区 (_len 字节)                 │
│ _next(8B)  │ ████████░░░░░░░░░░░░░░░░░░░░░░   │
│ _len (8B)  │ ↑ _hwm     ↑ _max = top()        │
└───────────┴───────────────────────────────────┘
              bottom()                    top()
```

数据区内 `_hwm` 是撞针（bump pointer）。每次 `Amalloc` 推一下，推到 `_max` 表示这个 Chunk 满了。

#### ChunkPool —— Chunk 对象的缓存池

`ChunkPool` 不存小块，它只缓存 `Chunk` 对象本身——四种标准大小分别对应四个静态池：

```cpp
class ChunkPool : public CHeapObj<mtInternal> {
  Chunk*       _first;        // 空闭 Chunk 链表头（LIFO）
  size_t       _num_chunks;   // 池里多少个空闲块
  size_t       _num_used;     // 当前借出多少块
  const size_t _size;         // 池中所有 Chunk 固定大小——不可跨池
};

static ChunkPool* _large_pool;   // 32728 字节
static ChunkPool* _medium_pool;  // 10200 字节
static ChunkPool* _small_pool;   //   984 字节（Arena 构造时默认取）
static ChunkPool* _tiny_pool;    //   216 字节（HandleArea 取）
```

每个池只存一种大小——`allocate()` 入口有 `assert(bytes == _size)` 防止跨池。分发小块内存是 `Arena::Amalloc()` 的活，ChunkPool 只管大块的借和还。

`allocate()` 从 `_first` 链表摘头（O(1)），池空才 `os::malloc`。`free()` 插回链表头（LIFO），不真释放——后续其他 Arena 可以复用同一个 Chunk。池中超过 5 个空闲块时 `ChunkPoolCleaner`（每 5000ms 醒来一次的周期任务）批量 `os::free` 还给 OS。

#### Arena —— 撞针分配器

`Arena` 是一个 C++ 对象——内部管理 Chunk 链表，对外提供撞针分配接口。构造时自动创建第一个 Chunk（984 字节，从 `small_pool` 取），撞针就位。

```cpp
class Arena : public CHeapObj<mtNone> {
  Chunk* _first;         // 第一个 Chunk（链表头，不变）
  Chunk* _chunk;         // 当前活跃 Chunk（撞针所在块）
  char*  _hwm;           // 撞针——下一个可分配位置
  char*  _max;           // 当前 Chunk 的 top()
};
```

`Amalloc(x)` 两个分支：`_hwm + x <= _max` 就只需要一条指针加法和一条比较（快速路径约 10 条 CPU 指令）。撞针顶到 `_max` 了才调 `grow()`——从 `large_pool`（32728 字节）取一个新 Chunk，旧 Chunk 串上新 Chunk：

```
grow() 前:  _first → Chunk1(init=984, 已满) ← _chunk
grow() 后:  _first → Chunk1(984) → Chunk2(size=32728) ← _chunk
                            旧串新                  撞针跳到这里
```

析构时遍历 Chunk 链表，每个 Chunk 归还到对应大小的 ChunkPool。

#### ResourceArea 与 ResourceMark —— 线程的临时内存栈

`ResourceArea` 就是 Arena 加了一行 `friend class ResourceMark`。`Thread::Thread()` 构造函数中为每个线程创建一个：

```cpp
set_resource_area(new (mtThread) ResourceArea());   // 创建 Arena，init_size=984
set_handle_area(new (mtThread) HandleArea(NULL));   // HandleArea init_size=216
```

`ResourceMark` 是一个 RAII 栈对象——构造时拍下 `(_chunk, _hwm, _max)` 的快照，析构时恢复。一个函数在开头写 `ResourceMark rm(THREAD)`，函数内 100 次 `Amalloc`，函数返回时 `rm` 析构——一次撞针回退清全部临时内存，零次 `free`。

`HandleArea` 与 `HandleMark` 同理——但撞的是 `oop*` 槽位，用于 GC 句柄分配。本质完全相同。

#### 完整依赖链——为什么 chunkpool_init 必须早于 new JavaThread()

```
chunkpool_init()
  → 创建 4 个 ChunkPool（初始全空，零预分配）

Thread::Thread()
  → new ResourceArea()
    → Arena() 构造: new Chunk(init_size=984)
      → Chunk::operator new: length=984 → small_pool
        → ChunkPool::allocate(1000): 池空 → os::malloc(1000)
    → _hwm = chunk->bottom()  撞针就位

  → new HandleArea()
    → Arena(tiny_size=216) 构造 → tiny_pool → os::malloc(232)
    → 撞针就位
```

首次 `new ResourceArea()` 时池是空的，仍走 `os::malloc`。但后续线程创建时 `small_pool` 中可能有归还的 Chunk——`allocate()->get_first()` O(1) 返回，不再 malloc。撞针在私有 Arena 上推、和 ChunkPool 无关——只在 `grow()` 和析构时短暂碰一下共享池。
