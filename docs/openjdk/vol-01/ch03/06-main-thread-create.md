...
`eventlog_init()` 初始化一个固定大小的环形缓冲区，JVM 运行期间自动记录关键事件（线程启动/退出、类加载、GC 操作）。如果 JVM 崩溃，`hs_err_pid<pid>.log` 文件中的 "Event:" 段就是从这个缓冲区 dump 出来的——帮助诊断崩溃前最后几秒发生了什么。初始化这么早是因为崩溃随时可能发生，日志得从第一行代码就开始记录。
