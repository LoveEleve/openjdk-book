...
现在的问题是：LWP-2 这个 OS 线程还没有在 JVM 内部"登记"——JVM 不知道它的栈边界在哪、它是什么状态、它的 ParkEvent 分配了没有。从 `Threads::create_vm` 的 9 阶段骨架（参见 [3.2 Stage 1-9 全貌](#/openjdk/vol-01/ch03/02-threads-create-vm.md)）来看，下一步的使命很明确：给 LWP-2 穿上 JVM 的外衣——创建 JVM 的第一个 `JavaThread` 对象，把当前线程在 JVM 内部完成登记。
...
### SuspendibleThreadSet_init — 并发 GC 线程的协作暂停
...
> **前置概念**：以下几节涉及 HotSpot 中互相独立的三套 Handle 体系——oop Handle（HandleArea/HandleMark/Handle）、Metadata Handle（methodHandle/constantPoolHandle）、JNIHandleBlock。如果直接看这四行代码感觉不知所云，请先在首页目录中阅读**前置概念：三套 Handle 体系**。。
...
>
> 完整的三件套（HandleArea / HandleMark / Handle）背景在 前置概念：三套 Handle 体系 已详细讲过。
...
这 5 个字段全部为 `Thread` 的实例字段（线程私有），此刻初始化为 NULL/0——因为线程尚未加入 `Threads::_thread_list`。它们服务于三个独立子系统：

- `_oops_do_parity` → GC 并行标记的去重锁，见前置概念：_oops_do_parity
- `_threads_hazard_ptr`、`_threads_list_ptr`、`_nested_threads_hazard_ptr_cnt` → Thread-SMR 的 Hazard Pointer + 嵌套遍历，见前置概念：Thread-SMR
- `_rcu_counter` → GlobalCounter（RCU 风格的宽限期等待），见前置概念：GlobalCounter
...
`_SR_lock` 是 HotSpot 的 `Monitor`（第 3.5 节详细讲解的四层锁机制的第 3 层），用于线程的 suspend/resume 操作。完整的协作式暂停机制见前置概念：_SR_lock — 线程自我挂起的信号锁。
...
`ParkEvent::Allocate(this)` 调用 `operator new` 在 C-Heap 上分配一个 256 字节对齐的 `ParkEvent` 对象，并把 `AssociatedWith` 设为自己所在的 `Thread`。四个 ParkEvent 按使用者分为两组：

**应用层**（Java 代码触发）：
| ParkEvent | 触发场景 | 谁在用它 |
|-----------|---------|---------|
| `_ParkEvent` | Java 代码中 `synchronized` 等待锁 | `ObjectMonitor::wait()` |
| `_SleepEvent` | Java 代码中 `Thread.sleep()` 超时等待 | `JVM_Sleep()` |

`_SleepEvent` 不是直接调 OS 的 `sleep()` 系统调用。`JVM_Sleep()` 内部用 `ParkEvent::park(millis)`——底层是 `pthread_cond_timedwait` 带超时。超时到了自动返回；超时没到时被 `Thread.interrupt()` 打断也会立即醒来。所以 sleep 的唤醒有两种途径：超时到期，或被中断。

**VM 内部层**（HotSpot 自身逻辑触发）：
| ParkEvent | 触发场景 | 谁在用它 |
|-----------|---------|---------|
| `_MutexEvent` | HotSpot 内部 `Mutex`/`Monitor` 锁竞争时排队 | `Monitor::ILock()` |
| `_MuxEvent` | 低层线程间简单同步 | `Parker` |

两组共用同一套 `ParkEvent` 机制（底层都是 `pthread_mutex_lock` + `pthread_cond_wait`），分组只是为了**隔离等待队列**——Java 层的锁等待和 VM 内部的锁等待不会互相干扰。
...
`Thread::Thread()` 中还有一些未展开的赋值，它们的归属：

| 字段 | 说明 |
|------|------|
| `_OnTrap`、`_schedctl`、`_Stalled`、`_TypeTag` | Solaris 遗留字段，Linux 上只是初始化为 0，整个 `os_linux.cpp` 无任何读写 |
| `debug_only(...)` / `NOT_PRODUCT(...)` / `#ifdef ASSERT` | 全部是编译期移除的 debug 断言桩，release 构建中不存在 |
| `_jvmti_env_iteration_count` | JVMTI 迭代计数器，等到 JVMTI 章节展开 |
| `set_allocated_bytes(0)` | TLAB 分配统计，等到 GC 章节展开 |
| `_vm_operation_started_count / _completed_count` | VMOperation 统计计数器，等到 VMOperation 章节展开 |
| `_current_pending_monitor` / `_current_waiting_monitor` / `_num_nested_signal` | ObjectMonitor 内部状态，等到锁章节展开 |
| `omFreeList` / `omInUseList` / `omFreeCount` / `omFreeProvision` / `omInUseCount` | 线程本地的 ObjectMonitor 批量分配缓存，等到锁章节展开 |
| `BarrierSet::on_thread_create(this)` | GC 写屏障通知，等到 GC 章节展开 |
| `_in_asgct` | AsyncGetCallTrace 标志，等到诊断章节展开 |

以上字段不是废弃的——它们按主题分别属于 JVMTI/GC/锁/VMOperation/诊断等章节，展开它们等于跳入对应子系统的细节中。本章专注于"把 OS 线程登记为 JavaThread 需要的最小字段集"。
...
`_threadObj` 是 `java.lang.Thread` 对象在 Java 堆中的 oop 引用。此刻设为 NULL——`init_globals()` 还没执行，Java 堆还没初始化，根本没有 Java 对象可以引用。

> **一个 Java 线程到底有多少个"线程对象"？** 当 Java 代码执行 `new Thread().start()` 时，一共创建了 4 层对象：
>
> | 层 | 类 | 内存位置 | 创建时机 |
> |---|------|---------|---------|
> | Java 堆对象 | `java.lang.Thread`（oop） | Java Heap（GC 管理） | `new Thread()` 时分配，`start()` 时通过 `prepare()` 关联到 `JavaThread` |
> | C++ VM 对象 | `JavaThread`（继承 `Thread`） | C-Heap | `new JavaThread()` 在 `JVM_StartThread` 中 |
> | 平台抽象 | `OSThread` | C-Heap | `os::create_thread()` 内部 `new OSThread()` |
> | 内核线程 | `pthread_t` | 内核 | `pthread_create()`，在 `new JavaThread()` 的构造函数里调用 |
>
> `_threadObj` 存的就是第一层的双向关联——`JavaThread._threadObj` 指向 Java 堆上的 oop，而 oop 中的 `eetop` native 字段（通过 `java_lang_Thread::set_thread()` 写入）指向 `JavaThread*`。**主线程走的是特殊路径**：`create_initial_thread()`（`thread.cpp:1090`）直接分配 oop 并调用 `set_threadObj()`，不走 `prepare()`。

后续主线程走完 `init_globals()` → 调用 `create_initial_thread()` 时，JVM 创建 `java.lang.Thread` 实例并通过 `set_threadObj()` 写入此字段。

```cpp
set_entry_point(NULL);
```

`_entry_point` 是一个函数指针（`ThreadFunction`），指向这个线程要执行的入口函数。子线程在 `JVM_StartThread → JavaThread::prepare` 时被设为 `thread_main_inner`（先执行 `<clinit>` 再执行 `run()` 方法）。**主线程永远是 NULL**——因为主线程不走 `prepare()` 路径，它的"入口函数"是 `JLI_Launch → JavaMain → InitializeJVM → Threads::create_vm`，不需要通过 `_entry_point` 来启动。
...
**JNI 函数表是什么？** JNI 规范定义了一套 C 函数接口（`GetFieldID`、`FindClass`、`CallVoidMethod` 等共 200+ 个函数），HotSpot 编译时在 `jni.cpp:3530` 初始化一个全局静态表 `jni_NativeInterface`：

```cpp
struct JNINativeInterface_ jni_NativeInterface = {
    jni_GetVersion,          // 第 7 号槽位
    jni_DefineClass,         // 第 8 号槽位
    jni_FindClass,           // 第 9 号槽位
    // ... 200+ 个函数指针，按 JNI 规范固定的槽位顺序排列
};
```

`jni_functions()`（`jni.cpp:3878`）返回这张全局表的首地址：`return &jni_NativeInterface;`。

**这个表怎么被使用？** 当你在 native 方法中写 `env->GetFieldID(env, cls, name, sig)` 时，实际上执行的是 `env->functions->GetFieldID(env, cls, name, sig)`——通过 `_jni_environment.functions` 指针间接调用。`set_jni_functions()` 就是把这张全局表的指针写入当前 `JavaThread` 对象上的 `_jni_environment.functions` 字段（`thread.hpp:1199-1201`）。

所有线程的 `_jni_environment.functions` 都指向同一张全局表——200+ 个函数指针，编译期确定，不需要每线程复制。JVMTI agent 可以通过 `set_jni_functions` 替换为另一张包装过的表，拦截 JNI 调用（比如调试器监控每次 `FindClass`）。

**JNIEnv 又是什么？** 你在 native 方法中用的 `env` 变量，就是当前线程的 `_jni_environment`。

```cpp
// 一个典型的 JNI native 方法
JNIEXPORT void JNICALL Java_MyClass_myMethod(JNIEnv *env, jclass cls) {
    jfieldID fid = env->GetFieldID(env, cls, "x", "I");  // ← env 来自哪里？
}
```

这个 `env` 指针指向的内存，就是 `JavaThread` 对象内部嵌着的 `_jni_environment` 结构。`_jni_environment` 里面只有一个关键字段 `functions`，它就是指向全局函数表 `jni_NativeInterface` 的指针。当你写 `env->GetFieldID(...)`，实际发生的事情是：

```
env->GetFieldID(env, cls, "x", "I")
      ↓
env->functions->GetFieldID(env, cls, "x", "I")
      ↓
从全局表 jni_NativeInterface 的第 11 号槽位取出 jni_GetFieldID 函数指针
      ↓
调用 jni_GetFieldID(env, cls, "x", "I")
```

JVM 调用 native 方法前，找到当前 `JavaThread` 对象，取 `&(thread._jni_environment)` 作为 `env` 参数传进去。`set_jni_functions` 就是把这个表指针塞进 `_jni_environment.functions`。
...
去优化是 JIT 编译器特有的问题。举个例子：JIT 把 `animal.speak()` 编译为直接跳转到 `Dog::speak()` 的机器码——因为当前只加载了 `Dog` 这一个子类。后来 `Cat` 类被加载了，`animal.speak()` 现在可能调度到 `Cat::speak()`——已编译的机器码错误了。JVM 需要把所有正在执行这段错误机器码的线程的栈帧从编译帧"拆回"解释器帧，这个过程就是去优化。

`_vframe_array_head/last` 链表保存被拆解的帧信息，此刻全部初始化为 NULL——线程刚创建，还没有任何编译帧需要拆解。完整机制在 JIT 章节展开。
...
`_thread_in_native` 和 `_thread_in_vm` 编译后都是机器码，但归属不同：

- **`_thread_in_native`**：线程在执行**你的** JNI 代码（`Java_MyClass_myMethod()` 里写的 C 代码）。JVM 不控制这段代码——它可以调用任意 OS 函数、拿任意锁、阻塞任意时间。GC 不能打断它，只能等它返回。

- **`_thread_in_vm`**：线程在执行 **HotSpot 自己的** C++ 代码（`threadSMR.cpp`、`safepoint.cpp` 里的逻辑）。JVM 完全控制这段代码——在特定位置插入了 safepoint 检查点，GC 可以在这些检查点安全暂停线程。

举个例子：Java 代码调用 `System.arraycopy()` → JVM 内部执行 `jni_SetObjectArrayElement()` → 此时线程就是 `_thread_in_vm`。但如果你的 Java 代码通过 JNI 调了一个自己写的 `void Java_MyClass_doWork()` → 此时线程是 `_thread_in_native`。
...
当 Java 代码抛出异常时，JVM 把异常对象（如 `new NullPointerException()` 的 oop）写入 `_exception_oop`，记录抛出位置（`_exception_pc`）和 catch 块的入口（`_exception_handler_pc`），然后跳转到异常处理逻辑。这三个字段是**每线程私有**的——线程 A 和线程 B 同时抛异常互不影响。

此刻全部初始化为 NULL/0——线程刚创建，没有待处理的异常。完整的异常抛出、栈展开、异常表匹配、去优化回退过程在专门的异常处理章节展开。
...
三个字段共同支撑调试器和 JIT 编译器对线程的控制：

- `_jvmti_thread_state`：JVMTI agent（如调试器、profiler）附加到线程时创建的状态对象，包含断点信息、单步状态、`GetThreadState` 的返回值缓存。没有 agent 附加时保持 NULL。
- `_interp_only_mode`：调试器要求"只走解释器，不走 JIT 编译的机器码"时的开关。设为非 0 后，即使方法有编译好的机器码，线程也会走解释器——这样调试器才能在每行字节码上设断点。
- `_special_runtime_exit_condition`：第 8.1 节 suspend-resume 中讲过的异步退出条件——线程在 VM 中执行时（`_thread_in_vm`），如果这个字段不是 `_no_async_condition`，说明有外部请求（如 suspend、async exception、trace flag）需要处理。`handle_special_runtime_exit_condition()` 在这里触发 `java_suspend_self()` 等操作。

此刻全部为 NULL/0/`_no_async_condition`——线程刚创建，没有调试器附加、没有编译码、没有异步退出条件。JVMTI 和 JIT 编译控制的完整机制在对应章节展开。
...
- `_thread_stat`：每线程的争用统计计数器。`new ThreadStatistics()` 分配在 C-Heap 上，结构如下（`threadService.hpp:132-194`）：

  ```cpp
  class ThreadStatistics : public CHeapObj<mtInternal> {
    jlong        _contended_enter_count;    // 锁竞争次数（synchronized 争锁失败）
    elapsedTimer _contended_enter_timer;    // 锁竞争的累计耗时
    jlong        _monitor_wait_count;       // Object.wait() 调用次数
    elapsedTimer _monitor_wait_timer;       // Object.wait() 的累计耗时
    jlong        _sleep_count;             // Thread.sleep() 次数
    elapsedTimer _sleep_timer;             // Thread.sleep() 的累计耗时
    // 每个计数器都配一个 elapsedTimer——不仅记次数，还记耗时
  };
  ```

  三个统计事件（`contended_enter`、`monitor_wait`、`sleep`）各自有一套 begin/end 方法——`contended_enter_begin()` 启动计时，`contended_enter_end()` 停止计时并累加。JMX `ThreadMXBean.getThreadInfo()` 通过 `ThreadSnapshot` 读取这些数据。
- `_parker`：`Unsafe.park()` / `Unsafe.unpark()` 的底层实现——`LockSupport.park()` 和整个 `j.u.c` 的阻塞都依赖它。和前面的 `ParkEvent` 是两套不同的等待机制：Parker 用于 `LockSupport`（JUC），ParkEvent 用于 `synchronized`/`Object.wait()`。`Parker::Allocate(this)` 在 C-Heap 上分配，完整机制见前置概念：Parker。
- `_safepoint_state`：线程级安全点状态机。当 GC 触发 safepoint 时，VMThread 通过这个对象的三个状态（`_running`/`_at_safepoint`/`_call_back`）协调"你停了吗"。完整的安全点协议见前置概念：ThreadSafepointState。

**其他未展开的字段**（同样是 `initialize()` 中的赋值，但属于高度特化或编译期跳过的分支）：

| 字段 | 说明 |
|------|------|
| `_in_deopt_handler`、`_deopt_mark` | 去优化处理器内部状态，等到 JIT 章节展开 |
| `_vm_result` / `_vm_result_2` | JNI 调用的返回值槽位，等到 JNI 章节展开 |
| `_monitor_chunks` | ObjectMonitor 的批量分配链表，等到锁章节展开 |
| `_suspend_equivalent`、`_privileged_stack_top` | 线程挂起等效条件和特权栈，等到 STW 章节展开 |
| `_jvmci_*` 系列（`_pending_monitorenter`、`_pending_deoptimization` 等 10+ 个字段） | 全部在 `#if INCLUDE_JVMCI` 条件编译内，本仓库的 `INCLUDE_JVMCI=0`，代码被预处理器移除 |
| `_doing_unsafe_access` | 临时标记——JVM 内部调用 `Unsafe` 时置 true，防递归 |
...
### 3.1.5 状态跃迁——`_thread_new → _thread_in_vm`

`JavaThread::initialize()` 把 `_thread_state` 设为 `_thread_new`。`new JavaThread()` 返回后，骨架代码紧接着有一行：

```cpp
main_thread->set_thread_state(_thread_in_vm);
```

**直接从 `_thread_new` 跳到 `_thread_in_vm`——跳过了偶数稳定态之间必须经过的奇数过渡态。** 这在主线程之外是违法的：其他 Java 线程必须通过 `ThreadInVMfromNative` 或 `ThreadInVMfromJava` 等 RAII 包装类来完成状态转换——RAII 的构造函数进入奇数过渡态（`_thread_in_native_trans` 等），检查 safepoint，然后进入偶数稳定态（`_thread_in_vm`）。

主线程可以跳过这些，因为此时 JVM 还在初始化阶段——没有 safepoint 需要检查、没有其他 Java 线程在运行、没有 GC 会触发。主线程自己就是唯一在执行的线程，它已经在 VM 代码中了（`JLI_Launch → JavaMain → InitializeJVM → Threads::create_vm`），无需"从 Java 过渡到 VM"。
...
`set_as_starting_thread()` 把当前 OS 线程（LWP-2）绑定到新创建的 `JavaThread` 对象上——分配 `OSThread`、写入 `pthread_self()`、设置 `RUNNABLE` 状态。和子线程不同：子线程走 `os::create_thread()` → `pthread_create` 创建新线程；主线程已经存在，不需要也不能再创建。完整机制见前置概念：主线程附着。
...
`pthread_create` 时已经为栈预留了 1MB 虚拟地址空间，但内核还没有为这些地址分配物理页——lazy allocation。栈底 guard zone 地址从未被访问过，没有物理页 backing。直接对没有物理页的虚拟地址调 mprotect(PROT_NONE) 会失败。

所以先调 os::create_stack_guard_pages()——用 mmap(MAP_FIXED) 在已有虚拟地址上强制分配物理页，不创建新 VMA。之后 guard zone 既有虚拟地址又有物理页，mprotect 才能正常工作。
...
- `stack_base()` → 栈顶，Shadow Zone 从这开始（80KB）
- Shadow Zone —— R/W，banging 探测区，不保护
- Usable Stack —— 正常 Java 帧使用的空间
- `_reserved_stack_activation` → Reserved Zone 开始（4KB PROT_NONE）
- `stack_red_zone_base()` → Red Zone 开始（4KB PROT_NONE）
- `stack_end()` → 栈底
