...
```
os::malloc(1000) 返回的内存:
┌──────────────────┬───────────────────────────────────────────────┐
│ Chunk 对象头部    │         数据区 (_len = 984 字节)               │
│ _next (8字节)     │                                               │
│ _len  (8字节)     │  ████████████░░░░░░░░░░░░░░░░░░░░░░░░░░░░   │
│ 共 16 字节        │  ↑ _hwm         ↑ _max = top()               │
│                   │  已分配(撞针前)   撞针不能越过这里              │
│                   │ bottom()                           top()     │
└──────────────────┴───────────────────────────────────────────────┘
```
...
new (mtThread) ResourceArea()
  │
  │ ① placement new: 在 C-Heap 上分配 ResourceArea 对象本身（约 48 字节）
  │    (mtThread) → NMT 把这 48 字节标记为 mtThread 类型
  │
  ├─ ② ResourceArea::ResourceArea(flags) → Arena::Arena(flags)
  │
  ├─ ③ Arena 构造函数内:
  ...
  │
  ├─ ④ Chunk::operator new(size=16, length=984):
  │      switch(984) → case Chunk::init_size → ChunkPool::small_pool()
  │                                              └── allocate(1000)
  │
  ├─ ⑤ ChunkPool::small_pool()->allocate(1000):
...
③ Amalloc(256) #2           // 撞针继续前进
   ┌────────────────────────────────────────────────────────────┐
   │ [████████████████████████████░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░]│
   │                          ↑ _hwm = P2 (~520 字节已用)         │
   └────────────────────────────────────────────────────────────┘

④ Amalloc(512) #3           // 512 + 520 = 1032 > 984 → grow()
   ┌───────────────────────┐   ┌────────────────────────────────┐
   │ Chunk1 (984, 已满)     │──→│ Chunk2 (32728)                 │
   │ [█████████████████████]│   │ [████████████████░░░░░░░░░░░░░]│
   └───────────────────────┘   └────────────────────────────────┘
                                                  ↑ _hwm 在新块
...
⑤ ~ResourceMark()           // 析构——回滚
   ...
   第一步: _chunk->next() != NULL → chop 删除 Chunk2（归还 ChunkPool）
   第二步: _area->_chunk = _chunk  → 回滚到保存的 Chunk1
   第三步: _area->_hwm = _hwm      → 撞针回退到 P0
   第四步: _area->_max = _max      → 恢复 Chunk1 的 top()

   最终状态:
   ┌────────────────────────────────────────────────────────────┐
   │ Chunk1 (984 字节)  ← 撞针回到原点，0 字节已用                │
   │ [░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░]│
   │  ↑ _hwm = P0                                               │
   └────────────────────────────────────────────────────────────┘
```
