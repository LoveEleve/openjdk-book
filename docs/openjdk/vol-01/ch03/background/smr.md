...
> **`_thread_list` 还在用吗？** 正在使用，但不是被遍历者读取——它只作为 `Threads::add()`（`thread.cpp:4462-4463`，O(1) 头插）和 `Threads::remove()`（`thread.cpp:4502-4513`，O(n) 遍历摘除）的增删入口。所有外部遍历（GC 根扫描、jstack、JVMTI/JMX）已全部改为走 `_java_thread_list`——`ALL_JAVA_THREADS` 宏（`thread.cpp:3534`）定义为 `DO_JAVA_THREADS(ThreadsSMRSupport::get_java_thread_list(), X)`。`_thread_list` 仍是增删线程的标准入口，但不再是任何遍历的数据源。
