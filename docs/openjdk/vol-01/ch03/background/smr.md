...
本文的推理建立在四个基础操作上。如果你已熟悉它们，可以直接跳到第 1 节。

### CAS（Compare-And-Swap）——无锁并发的原子操作

CAS 是一次不可被 CPU 调度打断的硬件指令。它做三件事：

```
cmpxchg(new_val, &location, expected_val)
  ① 原子地读出 location 的当前值
  ② 如果当前值 == expected_val → 写入 new_val → 成功
  ③ 如果当前值 != expected_val → 不写入（保持旧值不变）→ 失败
```

返回值是旧值。调用者通过比较返回值是否等于 `expected_val` 来判断成功与否（等于 = 成功）。

CAS 在多核并发下有两层语义：**"只有当我预期值仍为旧值时，才写入新值"**和**"写入和期望检查是不可分割的整体"**。JDK 中几乎所有无锁协议——从偏向锁到 Thread-SMR——都建立在 CAS 上。

### Atomic::xchg——原子交换

`Atomic::xchg(new_val, &location)` 把 `new_val` 写入 `location`，返回旧值。比 CAS 更简单——无条件写入，不需要预计旧值。本文中 CoW 的全局指针替换就用 xchg：写者不需要知道当前快照是谁——直接把新的换上去，拿回旧的去回收。

### 内存序（acquire / release / fence）

在多核 CPU 上，一个核心写入的值不立刻对其他核心可见。不加屏障的读写顺序可能被编译器或硬件重排。以下三种操作保证特定顺序：

- **acquire**：此后的读/写不排在此次读之前。用于**读者**——保证读到的数据在后续操作中一致可见。本文 `get_java_thread_list()` 和 `get_threads_hazard_ptr()` 都用了 load_acquire。
- **release**：此前的读/写不排在此次写之后。用于**写者**——保证之前写的数据在被其他人读到时已完全发布。本文 `set_threads_hazard_ptr()` 用了 release_store_fence。
- **fence**（屏障）：acquire + release 的合体——此前此后的操作都不能跨越。本文 `OrderAccess::acquire()` 在 `free_list()` 中保证 hazard ptr 的读取排在引用计数读取之前。

### wait/notify——持锁等待的协作模式

线程 A 在 Monitor 锁上调用 `wait()` → 原子性地释放锁并休眠，直到被 `notify()` 或 `notify_all()` 唤醒 → 被唤醒后重新竞争锁，回到调用点继续执行。本文中 `smr_delete()` 发现线程仍受保护时，在 `delete_lock` 上 wait；读者释放标签后 `notify_all` 唤醒等待者。
...
3. **扫描所有线程的 hazard ptr**：`ScanHazardPtrGatherThreadsListClosure`（`threadSMR.cpp:282-306`）遍历所有 JavaThread，读取其 `_threads_hazard_ptr`——注意即使是 tagged（未验证）标签也收集（保守处理，安全优先）。收集结果存入 `ThreadScanHashtable`（`threadSMR.cpp:169-205`），其哈希函数用黄金比例乘法：`ptr * 2^32 * Phi`（2654435761），将指针均匀分散到哈希槽中，避免碰撞。`OrderAccess::acquire()` 屏障保证 hazard ptr 的读取排在后续读 `_nested_handle_cnt` 之前。
...
> **术语**：oop = ordinary object pointer（普通对象指针），即 HotSpot 内部对堆上 Java 对象的引用。safepoint = Java 线程全部暂停、不再执行字节码的全局同步点——STW GC、偏向锁撤销等操作都在 safepoint 期间进行。safepoint 期间线程不能获取 `Threads_lock`——这是 STW GC 安全性的基础（下面详述）。
...
| Hazard Pointer | RCU (GlobalCounter) | RCU 依赖代际计数器——读者 `critical_section_begin` 标记"我进入了临界区"，写者 `write_synchronize` 等**全体**老读者离开临界区后才回收。线程列表读者持有时间长（几百微秒）→ RCU 退化为同步阻塞。HP 只等**特定快照**的读者释放——粒度优势 |
...
回到起点——`Thread::Thread()` 构造函数（`thread.cpp:241-245`）中初始化的 5 个 SMR 字段：
...
_rcu_counter = 0;                  // GlobalCounter 代际计数——与 Linux RCU 同原理（代际计数器 + 宽限期等待），
                                    // 用于 oopStorage/ConcurrentHashTable 等非线程列表场景，非本文核心
```
