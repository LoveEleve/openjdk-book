...
**需要一种协议——T1 贴在 hazard ptr 上的标签能让写者"看到"，写者又能帮 T1 判断"你是不是贴晚了"。**

解法是加一个 tag bit：T1 先把要保护的版本号打上 **unverified** 标记写入 `_threads_hazard_ptr`，然后重读全局指针验证——如果全局指针已经换成新版本，说明 T2 在自己贴标签之前已经操作过了，T1 的标签是"过期的"，需要重试。如果全局指针没变，去掉 tag 标记为 **verified**——此时标签生效，写者扫到后不会删。

写者在扫描时如果发现 tagged（unverified）的 hazard ptr，会主动把它抢走（设为 NULL）并 notify——帮读者判断"你贴晚了"。这就是两阶段发布协议的核心：tag 给读取加了一个"我还没验证完"的缓冲窗口，untag 是"验证通过，你可以扫我了"。
