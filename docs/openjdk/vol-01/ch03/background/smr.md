...
### 3.1 从非 CoW 方案出发——为什么不行

假设没有 CoW，线程列表是一个简单的全局数组 `JavaThread* threads[100]`：

```
写者：threads[3] = newThread;   // 原地修改
读者：正在遍历 threads[0] 到 threads[10] —— 他能不能安全地读？
```

**不能。** 写者是原地改——读者在遍历过程中，数组的内容可能已经变了。线程 T3 正在读 `threads[3]`，写者恰好把这个槽位改成了 NULL 或新指针——读者悬空。

**本质上，问题出在"写者和读者共享同一块可变内存"。** 只要数组可原地修改，读者就必须和写者同步——锁或 CAS 都要。
