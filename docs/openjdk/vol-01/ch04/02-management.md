...
DCmd 的工作机制（jcmd 怎么通过 attach API 把命令送到 JVM、AttachListener 线程怎么处理、DCmd 框架怎么解析参数和执行、为什么 `jcmd PerfCounter.print` 走的是特殊路径不走 attach API）将在本章后续的 **DCmd 工作机制** 一节单独展开——这一节会作为 ch04 的后续文章（4.6 或独立编号）。本节先讲到这里——management_init 注册了 DCmd 让 jcmd 能工作，具体的调用链路稍后展开。
