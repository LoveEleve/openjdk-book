...
为什么 NMT 要单独注册而不放在 `register_dcmds()` 里？因为 NMT 是独立子系统（声明在 `nmtDCmd.hpp`），和 `diagnosticCommand.hpp` 里那些标准 DCmd 不在同一个文件。`register_dcmds()` 在 `diagnosticCommand.cpp:71-138` 注册的都是 `diagnosticCommand.hpp` 声明的命令；NMTDCmd 单独注册。
