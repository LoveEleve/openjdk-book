...
║                                                                    ║
║  management_init()                                                 ║
║  ├── Management::init()      → 3 计时器 + 9 能力位 + 42 个 DCmd    ║
...
║  /tmp/hsperfdata_.../<pid> DCmd 链表（42 个 DCmdFactory 对象）      ║
║  22 个计数器写到这里       两个都存在 HotSpot C++ 进程的内存里       ║
╚══════════════════════════════════════════════════════════════════╝
       │                              │
...
║  零系统调用            ║  ║   ↓                               ║
║  不需要 JVM 配合       ║  ║  ClassLoadingImpl / ThreadImpl /  ║
...
╚═══════════════════════╝  ╚══════════════════════════════════╝
           ...
           另外：jcmd <其他命令> 走 attach API
                 ...
                 AttachListener 线程 dequeue → DCmd::execute()
                 （这条路径目标 JVM 必须配合）
```

> **`JNI_OnLoad` 是什么？** 这是 JNI 规范定义的一个钩子函数——当 Java 代码第一次通过 `System.loadLibrary("management")` 加载 `libmanagement.so` 时，JVM 会自动调用这个库里的 `JNI_OnLoad` 函数（如果存在）。这是 native 库的"初始化入口"——库可以在这里做一次性初始化，比如拿到 JVM 的函数表指针、注册 native 方法等。`libmanagement.so` 就是在 `JNI_OnLoad` 里调 `JVM_GetManagement(JMM_VERSION)` 拿到 `jmm_interface` 函数表指针，之后所有 MXBean 方法的调用都通过这个指针进 HotSpot。如果 native 库没有导出 `JNI_OnLoad`，JVM 假设它只需要 JNI 1.1 版本，不调用任何初始化函数。
