...
ch10-ch11 讲完了堆初始化。ch13 承接 `universe_init` 中的 6 个 `LatestMethodCache` 创建——位于 `ClassLoaderData::init_null_class_loader_data()` 之后、`MetaspaceShared::initialize_shared_spaces()` 之前（`universe.cpp:713-720`）。
...
```
ch10-ch11 堆初始化 ──→ ch13 LatestMethodCache
                       │
                       ├─ 6 个单槽缓存
                       ├─ 延迟填充 + idnum 安全
                       └─ 1 槽命中的统计证据
