...
init_globals()（runtime/init.cpp:101）:
  10: gc_barrier_stubs_init()     ← ch15
  11: interpreter_init()           ← 本章 §1——TemplateInterpreter::initialize()
  12: invocationCounter_init()     ← ch15
  13: accessFlags_init()           ← ch15
...
`Interpreter::update_cds_entry_table(kind)` 在 CDS 启用时更新归档入口表（ch14 已讲过共享表概念——默认不开启，此行为 no-op）。
