# 12. ch09 总结——200 行 initialize() 逐行讲透之后

> **本文定位**：ch09 全部 12 篇文章（01-11 + 补章 12b）的收官总结。不新增源码分析，只做三件事——把 `initialize()` 的完整执行路径再走一遍、提炼整章反复出现的设计模式、指出后续学习路径。

---

## 1. 整章覆盖范围回顾

ch09 用 12 篇文章拆解了 G1 堆创建的全过程。覆盖的源码范围：

```
Universe::initialize_heap()                ← ch09/01（全景）
└─ create_heap()                           ← ch09/04
   └─ create_heap_with_policy<G1CollectedHeap, G1CollectorPolicy>()
      ├─ new G1CollectorPolicy()           ← ch09/02
      │   ├─ setup_heap_region_size()      ← Region 大小 1/2/4/8/16/32MB
      │   └─ setup_remset_size()           ← RSet 表容量
      └─ new G1CollectedHeap(policy)       ← ch09/04a（75 行构造函数）

└─ _collectedHeap->initialize()            ← 201 行，G1 专属
   ├─ Universe::reserve_heap()             ← ch09/03（两次 mmap）
   ├─ G1CardTable + BarrierSet             ← ch09/05
   ├─ HotCardCache                         ← ch09/05
   ├─ 6 个 G1RegionToSpaceMapper           ← ch09/05
   ├─ HeapRegionManager::initialize()      ← ch09/05
   ├─ G1RemSet                             ← ch09/06
   ├─ G1BlockOffsetTable + CSet fast test  ← ch09/06
   ├─ G1ConcurrentMark (~150 行)           ← ch09/07
   ├─ expand(init_byte_size)               ← ch09/08
   ├─ g1_policy()->init()                  ← ch09/09
   ├─ SATB + 双 DCQ + ConcurrentRefine     ← ch09/10
   ├─ Dummy Region + 分配器就位            ← ch09/11
   ├─ G1MonitoringSupport + StringDedup    ← ch09/11
   ├─ PreservedMarksSet                    ← ch09/11
   └─ G1CollectionSet::initialize()        ← ch09/11

└─ TLAB set_max_size / CompressedOops / TLAB startup   ← ch09/01（概览）
```
...
│ (ch09/03)                    │   │ HeapRegionManager 建 _regions │
└──────────────────────────────┘   │ (ch09/05)                    │
                                   └──────────────────────────────┘
              ↓
...
│ expand() commit 物理页       │ → │ 分配器激活 (ch09/11)         │
│ 队列系统 (ch09/07-10)        │   │ 监控/去重/CSet 就位          │
└──────────────────────────────┘   └──────────────────────────────┘
```
...
Dummy Region 是 `G1AllocRegion` 的 Null Object——用"永远非 NULL 但永远分配失败"的 Region 消除分配路径上的 NULL 检查（ch09/11）。类似的哨兵还有 `FreeRegionList::set_unrealistically_long_length(max_regions() + 1)`——把 free list 长度的"不可能阈值"设在合法操作永远碰不到的值，一旦碰线说明内部状态坏了（ch09/06）。
...
```
这正是 ch09/11 埋下的伏笔——dummy region 如何在实际分配中"被绕过"。
...
ch12 是独立的 Metaspace 诊断章（依赖 ch08/07，非本章），与 G1 堆初始化无直接承接关系。

**回到 initialize_heap() 内部**：ch09 的五阶段中，①② 由 ch09/02-04 讲透，③④⑤ 由[补章 12b](12b-compressed-oops-and-tlab.md)讲透（universe.cpp:764-823）：

```
Universe::initialize_heap()
├─ ① create_heap()                                     ← ch09/04 已讲
├─ ② _collectedHeap->initialize() (201 行)              ← ch09/02-11 已讲
...
③-⑤ 是 ch09 的遗留缺口——原计划在 ch09/06 展开（压缩指针与 TLAB），但 ch09/06 实际写成了 RemSet+BOT。已由[补章 12b](12b-compressed-oops-and-tlab.md)补完（三步合计约 90 行源码，一篇搞定）。
...
├─ ④ Metaspace::global_initialize()                      ← ch08-07
├─ ⑤ MetaspaceCounters::initialize_performance_counters()← ch08-06
├─ ⑥ JVMFlagConstraintList::check_constraints()          ← ch08-05
├─ ⑦ ClassLoaderData::init_null_class_loader_data()      ← ch08-03
├─ ⑧ 6× new LatestMethodCache()                          ← ch10
...
ch09 用了 12 篇文章（01-11 + 补章 12b）、覆盖 200+ 行 initialize()。回到最初的问题：**为什么 G1 的堆初始化这么重？**
...
> **下一篇**：[ch10.1 LatestMethodCache](../ch10/01-latest-method-cache.md)——`universe_init()` 第 ⑧ 步，堆刚建好后的 6 个单槽方法缓存。
