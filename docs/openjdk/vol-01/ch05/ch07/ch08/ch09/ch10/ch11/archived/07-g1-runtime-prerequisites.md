...
> **本文定位**：桥章——连接 ch10 初始化系列（01-06）和运行时系列（08-16）。本文不讲任何具体 GC 流程细节，只搭建后续 8 篇文章共享的**全部概念砖块**。
>
> **前置依赖**：ch10/01-06（知道堆初始化建了哪些数据结构：Region/RemSet/BOT/CSet 测试位图/Mapper/写屏障/卡表）。
>
...
G1 把堆切成等大 Region（大小由 ch10/02 的 `setup_heap_region_size` 确定）。但 Region **不按物理位置分代**——每个 Region 上的"角色"由 `HeapRegionType` 的 **Tag 位**决定：
...
**为什么是全有或全无**——一个 Old Region 里可能并存几十到几百个**互相独立**的对象。线程栈引用 A、JNI 引用 B、静态字段引用 C。A 活着不保证 B 活着——所以 Old Region 里可能出现"10 个对象中 3 个活、7 个死"的**部分存活**状态。并发标记必须逐个对象遍历才能区分谁活谁死。

而 Humongous Region 整个 Region**只存一个对象**——那个巨对象本身。Start+Continues Region 合起来就是这一个对象的空间，没有其他任何对象。所以"这个 Region 里有没有活对象"等价于"这一个 Humongous 对象还活着吗"——只有两个答案：活着（整个 Region 全是活数据）或者死了（整个 Region 全是死数据），不存在中间态。

**为什么不能混合**——一个 Humongous 对象占了 Region 的一半以上，HotSpot 决定**不往剩下的空间分配其他对象**。剩下的（不到一半的）空间直接被浪费了。这不是 bug，是故意为之——为了避免"Region 内既有 Humongous 又有普通小对象"的复杂状态管理。代价是浪费部分内存，换来"一个 Region = 一个对象"的简单性，进而换来 RSet 空就能判断全死的快捷路径。
...
`humongous_reclaim_candidates` 位图在 ch10/06 详细讲解——它缓存了"哪些 Humongous Region 的 RSet 为空"的判断结果，供 GC 暂停时直接使用。
...
> 每个 Region 独立维护自己的 RSet（ch10/06 讲的三层结构）。
...
G1 的优化重点在"暂停时间可控"——通过预测（`_analytics` + `_mmu_tracker`，ch10/04）限制 CSet 大小来控制 GC 工作量。但注意 **GC 日志里显示的 `8.234ms` 不包括到达时间**——如果某个线程卡在长循环里，TTSP 可能额外增加数毫秒，GC 日志不会反映这部分。排查"GC 暂停为什么这么长"时先确认 TTSP 是否占了大头（用 §2.4 末尾列出的工具）。
...
**长循环是主要延迟来源**——JIT 编译器在方法返回和循环回边处插入 safepoint poll（一条 `test` 指令读 polling page）。但编译器为了优化可能会**移走或合并** poll 点。如果一个紧凑循环被优化后 100 万次迭代不插一个 poll，这个线程可能在数毫秒内都无法响应 safepoint 请求——VM Thread 只能干等着。
...
| 方式 | 看到什么 | 启用方法 |
|------|---------|---------|
| `SafepointStats._time_to_sync` | TTSP 总耗时（纳秒） | `-XX:+PrintSafepointStatistics`（输出表格，`sync` 列即 TTSP） |
| UL safepoint 日志 | 各阶段耗时文字输出 | `-Xlog:safepoint=debug`（显示初始化线程数、等待阻塞过程） |
| TTSP 超时检测 | 哪个线程卡住了 | `-XX:+SafepointTimeout -XX:SafepointTimeoutDelay=1000`（超时 1 秒打印线程栈） |
| JFR 事件 | 完整 safepoint 时间线 | `EventSafepointBegin` → `EventSafepointStateSynchronization` → `EventSafepointEnd`（通过 `safepointId` 关联） |

**TTSP 确实是可能的性能瓶颈**——GC 日志里的 `8.234ms` 只算 GC 实际工作的时间，**不包括**从发起 safepoint 到所有线程停下的时间。如果某个线程被卡在长循环里，TTSP 可能额外增加几毫秒甚至几十毫秒，但 GC 日志里看不到这部分。排查 GC 暂停过长时，**先看 TTSP，再看 GC 工作**。
...
两者都通过 `VM_CGC_Operation op(&cl, "Pause Remark")` 或 `"Pause Cleanup"` 提交给 VMThread 执行 STW。详细流程在 ch10/14（CM 后半）展开。
...
ch10/06 讲了 `in_cset_fast_test`：偏置数组 + `InCSetState(-1/0/1/2)` O(1) 判断"某个地址在不在 CSet"。那是 **CSet 的使用阶段**。
...
下一篇：**ch10/08 Young GC Evacuation 周期**——把本节搭好的砖块真正用起来。
