...
```
全局层（所有 CLD 共享）:
  VirtualSpaceList  →  连续的虚拟地址空间链表
  ChunkManager      →  空闲 chunk 的全局回收池
...
`Metaspace::global_initialize()`（`metaspace.cpp:1294`）在 `universe_init` 第 694 行调用。它做四件事：

1. 初始化 GC 水位线（`MetaspaceGC::initialize`）——分配超过阈值触发 GC
2. CDS/压缩类空间初始化——如果开启 `UseCompressedClassPointers`，在堆顶上 mmap 出 Class Space（`CompressedClassSpaceSize` 默认 1GB，但若 `MaxMetaspaceSize` 小于 1GB 会被自动调小——见 `metaspace.cpp:1280-1285` 的 `FLAG_SET_ERGO` 逻辑），创建 `_class_space_list` 和 `_chunk_manager_class`
3. 计算首个 chunk 大小——bootstrap CLD 的初始 chunk = `InitialBootClassLoaderMetaspaceSize / wordSize`
4. 创建 NonClassType 的 `_space_list` 和 `_chunk_manager_metadata`

class space 的特殊初始化路径（`metaspace.cpp:1074`）：`ReservedSpace(compressed_class_space_size, ...)` 在堆顶之上 mmap，然后 `initialize_class_space(rs)` 把预留空间包装为 VirtualSpaceList。AARCH64/PPC64 有额外逻辑——尝试零基址压缩指针。
...
全局层：mmap 预留 → 按需 commit → VirtualSpaceList/VirtualSpaceNode
...
purge → CLD 卸载后 container_count==0 的 Node → munmap 还给 OS
```
...
| VirtualSpaceNode | 链表节点——一段连续虚拟空间，bump pointer 分配 chunk |
