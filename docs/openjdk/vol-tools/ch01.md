...
Use jcmd 3725467 JFR.dump name=1 filename=FILEPATH to copy recording data to file.
```

注意第二行那句"using maxsize=250MB as default"——不指定文件上限时，JFR 用 250MB 环形缓冲兜底，录满了覆盖最旧的数据，保证不撑爆磁盘。30 秒后，`rec-demo.jfr` 出现在当前目录。这是**运行期动态开启**——不用重启进程，跟给一台跑着的服务器装监控一样。生产环境里这是首选：JFR 默认录制开销在 1% 级别，随时可开可关（后面细说）。
...
==================================================================
 jdk.PromoteObjectOutsidePLAB           3,057,467    49,461,931
 jdk.GCPhaseParallel                    2,291,962    59,214,216
 jdk.TenuringDistribution                  40,230       455,392
 jdk.GCPhasePauseLevel1                    10,728       440,494
 jdk.ObjectAllocationSample                 8,908       139,495
 jdk.ExecutionSample                        6,900        74,157
 jdk.SafepointBegin                         2,710        41,134
 jdk.ExecuteVMOperation                     2,709        46,532
 jdk.GCPhasePause                           2,682        62,221
 jdk.ActiveSetting                          3,041        90,792
 ...
```
...
第三，**不同事件的量级差着几个数量级**。`GCPhaseParallel` 229 万条 vs `SafepointBegin` 2710 条——原因不是谁被过滤了，而是它们本来就不是一个量级的事：safepoint 每秒只发生几十次（30 秒 2710 次），而每次 GC 停顿里，每一个并行阶段、每一根并行线程都是一条 `GCPhaseParallel` 事件——一次 GC 就是几千条。量级差，本身就是这两类机制"频率特征"的第一手证据。

但"这个文件里能看到多少条"，不只取决于事件本身的频率，还取决于**配置的阈值**——这是新手录 JFR 最容易踩的坑：
...
profile 配置把 threshold 全压到 0ms，才露出真面目。所以规矩是：**写安全点、锁、IO、采样相关文章，必须用 `settings=profile` 录**。rec-demo.jfr 这份录制里 SafepointBegin 的 threshold 就是 0ms（实测文件里这类短事件全被记下）——如果当初用了默认配置，今天你看到的就不是 2710 条，而是 0 条。
...
`Random.next` 占 66.9%——这是 BusyTask 忙循环里密集调用 `R.nextDouble()`（Demo 里那个 `Random` 实例）的结果。右下的栈显示 `Random.next → Random.nextDouble → Demo$BusyTask.run → Thread.run`，热点链路一眼到底。这个"采样→聚合→排名"的机制，就是 async-profiler、JFR 采样、Arthas `thread -n` 的共同内核。
...
这篇里每一个精确数字，都只对 `rec-demo.jfr` 这个文件成立——它是 2026-08-11 那个下午、那台机器、那一次运行留下的一份固定录音。任何人、任何时候打开这个文件，看到的都和这里一模一样，因为文件没变。

但如果你按下面的配方自己录一份，数字会变，而且**变是正常的**：

```
$ java Demo &    # 一个 BusyTask 烧 CPU + 一个 AllocTask 疯狂分配
$ jcmd <pid> JFR.start name=rec-cpu settings=profile duration=30s
$ jcmd <pid> JFR.dump name=rec-cpu filename=rec-demo.jfr   # 30 秒后
$ jcmd <pid> JFR.stop name=rec-cpu
```

- **会变的**：`ExecutionSample` 6900、`ObjectAllocationSample` 8908 这类**采样事件**是概率抽样，每次跑都有涨落；`GCPhaseParallel` 229 万这类**频率事件**取决于你机器上 GC 的实际节奏。
- **不会变的**：事件类型的构成（还是那 80 多种）、量级的对比关系（GC 百万级 vs 安全点千级）、热点方法的排名（`Random.next` 第一——只要负载没变）。

一句话：**精确值看文件，结论看形状**。写文章引用精确数字，必须声明它来自哪次录制（像这篇开头那样）；引用"形状"，任何一次重录都支持同一个结论。

还有一点要交代：本文引用的 `rec-demo.jfr` 是当时用 jcmd 以 `rec-cpu` 为名录的，那份录制的具体配置文件已经不可考——文件里只留着当时的设置快照（threshold 混合 0ms 和 10ms，SafepointBegin 是 0ms）。所以本文只说"这份文件里事实如此"，不说"我当时用的命令是这样"。如果你要严格复现，就用上面这行 `settings=profile`——它是可验证的配方。
...
素材在哪里：本篇的截图就是上面的配图（原图 14 张在 `vol-tools/assets/`），录制文件在规划库 `outlines/00-jvm-tools/materials/jfr-recordings/rec-demo.jfr`，事件→域索引见规划库 KP 05 节，JMC 74 条规则清单见 KP 06 节。下一篇用 jcmd 把这台机器的"活体解剖"做一遍——49 个子命令，每个都是一扇窗户。
