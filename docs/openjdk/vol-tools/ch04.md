...
- `level='3'`：C1 全优化层——分层编译（域 13/14）的层级标记，level 1/2/3 是 C1 的三种形态，level 4 是 C2 最终优化。
...
**"callee is too large" 一家独大**——这正是 C2 内联策略的核心规则：方法超过大小上限（`MaxInlineSize`/`FreqInlineSize` 两个开关管着）就不内联。写域 15（C2）时，这张"失败原因分布"表就是"内联决策在真实负载下怎么裁"的第一手统计。
