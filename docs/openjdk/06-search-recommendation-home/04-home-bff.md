...
`my-xhs` 当前显然不想让前端承担这些复杂度，这正是 `home` 存在的根本原因。
...
下一步如果继续沿用户可见能力往前推，最自然的是进入 `02-content-feed-interaction`，因为内容、Feed 和互动正是这些流量入口背后真正承载的对象。