...
2. 实现Runnable接口(<font style="color:rgb(216,57,49);">推荐</font>)
...
下面就进入到FutureTask的工作原理讲解：从isDone()可以提出一个问题：如何知道一个任务是否已经完成了呢？ -- **<font style="color:rgb(216,57,49);">状态</font>**「这一点在阅读源码之前就应该知道」
...
直到执行完任务的那个线程(通常是工作线程)来唤醒位于waiters上的所有阻塞线&#x7A0B;**<font style="color:rgb(216,57,49);">「ps:这里是否可以优化?让被唤醒的线程去帮助唤醒,类似共享锁内部的机制,因为由线程池内部的工作线程来唤醒所有的线程,可能会降低线程池的吞吐量」</font>**
