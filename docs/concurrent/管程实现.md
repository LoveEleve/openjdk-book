...
<!-- 这是一张图片，ocr 内容为： -->
![图片1](https://scnjnj9snmp7.feishu.cn/space/api/box/stream/download/asynccode/?code=NjEwYjA4NDlhZWY3NzRlMzhkNzdmZThhNGYyNWNlMWRfVXdqUTE1b0JmRmZLejUycVpsRFlZZTNGNld6MHhtRWNfVG9rZW46UzlwbmJoaGhDb1VMY3V4aHBvVGNsRjJobmJmXzE3Njg2NTU1MTQ6MTc2ODY1OTExNF9WNA "图片链接可能已过期，如无法显示请联系作者")
...
### 类关系图
<!-- 这是一张图片，ocr 内容为： -->
![图片2](https://scnjnj9snmp7.feishu.cn/space/api/box/stream/download/asynccode/?code=NTRjOGI3NWMwZjM3OTdiMTI1NWNkNTZlNjQ5MWQ0ODdfeGZXbzJwcnk3bmE5YkRuemlTSmNxMWM5VmVWSWdvZlpfVG9rZW46UVRmaGJydnhHb3BCZVh4ckt1UWNEeEFabjBkXzE3Njg2NTU1MTQ6MTc2ODY1OTExNF9WNA "图片链接可能已过期，如无法显示请联系作者")
...
<!-- 这是一张图片，ocr 内容为： -->
![图片3](https://scnjnj9snmp7.feishu.cn/space/api/box/stream/download/asynccode/?code=MDE1YzIzMWJiOGQ2MTA1NjNhMGIxNjg5ZjcyZDFjN2JfS2g1WWZuZ1hSc2p3S2R5T1luTERoUXM4emZZN0g3YWlfVG9rZW46TzZ3TWJ6a2R2b25hRnV4cWsyaWNkSDBlblRlXzE3Njg2NTU1MTQ6MTc2ODY1OTExNF9WNA "图片链接可能已过期，如无法显示请联系作者")
...
<!-- 这是一张图片，ocr 内容为： -->
![图片4](https://scnjnj9snmp7.feishu.cn/space/api/box/stream/download/asynccode/?code=NmMyZDQ4Y2IxYTkwZDdmOTlkNTNhNzAzZWIzMDVjODZfdjg2UlpCQURGMFdha3FMcDBiVnJwdXdWS3owRmlqMzBfVG9rZW46VGhTYWJSU1RubzRRSjJ4aW9Sb2NBemRPblFTXzE3Njg2NTU1MTQ6MTc2ODY1OTExNF9WNA "图片链接可能已过期，如无法显示请联系作者")
...
所以在锁的获取过程中,有哪些地方会涉及到竞争呢？ 1.入同步队列的时候(**入队阶段**) 2.获取资源的时候(state) 而资源获取又可以分为两种情况：唤醒时获取，进入时获取 1.唤醒阶段：线程从阻塞中唤醒后,不一定能够获取到锁，需要继续竞争 2._**进入阶段：在调用lock()的时候 -- 这种情况也是我们平常所讨论的地方**_

那么这三个阶段的公平性如何呢？ 1.入队阶段：各个线程通过CAS插入到队列中,_**不具备公平性**_,因为CAS依赖底层实现(os和硬件),后cas的线程可能比先cas的线程更早的成功入队 2.唤醒阶段：_**具有公平性**_,因为同步队列采用的是FIFO队列，也即首先唤醒的永远是头节点(被中断唤醒不考虑在内),非首节点是不会去尝试获取锁的(state) 上面两个阶段,不管是公平还是非公平实现,都是一样的 3.进入阶段：这个阶段就是我们通常所讨论的地方. a.对于公平锁而言,每个线程在进入的时候都会判断是有有线程节点在同步队列中,如果有,那么就会进入到队列中等待「并不会直接阻塞,而是进入到AQS的入队流程」 b.对于非公平锁而言,每个线程在进入的时候不会关系同步队列中是否有节点在等待,而是直接通过cas来竞争锁资源
...
<!-- 这是一张图片，ocr 内容为： -->
![图片5](https://scnjnj9snmp7.feishu.cn/space/api/box/stream/download/asynccode/?code=NTM1YjhkYTZlYzBhMTcwYTFjNjgzN2FiYWQ4NjgwZWJfeFNnalk2VVN5emdlRzRVWklRclNpNUN4ZndlaTZJdUxfVG9rZW46UWlmRmJnZ1hEbzk0MWJ4bmxMUmM0QnI1bkFnXzE3Njg2NTU1MTQ6MTc2ODY1OTExNF9WNA "图片链接可能已过期，如无法显示请联系作者")
...
<!-- 这是一张图片，ocr 内容为： -->
![图片6](https://scnjnj9snmp7.feishu.cn/space/api/box/stream/download/asynccode/?code=MjY4NzJhMDg3MGQzMDVhNjYzNGUwNDY4Yzk5NDUyNzBfdzZucXhEU05ZNU5NdUdadm9kR2tpaWxkN1hTZlR3dG5fVG9rZW46R0ZreGJpcU5ab21tMDZ4eUZWSGN0aGh5bmVoXzE3Njg2NTU1MTQ6MTc2ODY1OTExNF9WNA "图片链接可能已过期，如无法显示请联系作者")
...
<!-- 这是一张图片，ocr 内容为： -->
![图片7](https://scnjnj9snmp7.feishu.cn/space/api/box/stream/download/asynccode/?code=MThjZjM5ZWZmNWNiNzI3ZGMwODcwMjEyYjRiMjFhOTdfRFk5UzhLYmxWTml1OXo2bDhaTFlQdzdmRkwwV2I4UGFfVG9rZW46U05qM2JYMGZTb29iTVJ4Y0VOVGMwSFFSblNOXzE3Njg2NTU1MTQ6MTc2ODY1OTExNF9WNA "图片链接可能已过期，如无法显示请联系作者")
...
否则：经过上面的判断，要么是没有写锁,要么是写锁和读锁重入 。 那么下面按道理来说：看见的代码应该是：compareAndSetState(c, c + SHARED_UNIT)，来竞争锁资源。 但是读写锁必须要考虑的一个问题：那就是写锁饥饿问题。

前面也说过,读写锁是用于读多写少的情况的,提高多线程读的性能,充分利用系统资源，但是一旦当一个线程获取读锁成功后，如果不加以限制,那么会出现什么样的情况？

<!-- 这是一张图片，ocr 内容为： -->
![图片8](https://scnjnj9snmp7.feishu.cn/space/api/box/stream/download/asynccode/?code=ZDIzYTczZWVjZTFjMmM5NTAyMGQxNzgyYjgzOTlkNjdfOEZqYXg2ekJ4S0ViRUxYTUtsd1dwY0VJTXQ5U2tzdlhfVG9rZW46SWRzRGJtWEhibzhsQU94bUw2dGNHYWJFbmdjXzE3Njg2NTU1MTQ6MTc2ODY1OTExNF9WNA "图片链接可能已过期，如无法显示请联系作者")
...
<!-- 这是一张图片，ocr 内容为： -->
![图片9](https://scnjnj9snmp7.feishu.cn/space/api/box/stream/download/asynccode/?code=ZDk5MWNlOWU3NTJhZWY2YTU3ZjZkOGEwYjY5ZDNjMjJfcjN4VE85aWVVOWxXMmVTbllGSDRBWWFMZU1jcmFoQ0tfVG9rZW46UnNOUGI2YTVob1cza2Z4VGdNMWNYWlBRbkxnXzE3Njg2NTU1MTQ6MTc2ODY1OTExNF9WNA "图片链接可能已过期，如无法显示请联系作者")
...
<!-- 这是一张图片，ocr 内容为： -->
![图片10](https://scnjnj9snmp7.feishu.cn/space/api/box/stream/download/asynccode/?code=YTVjODE5ZjMwNzdlMTRiMDBmNTI5NjNkMDkzZWYzMjNfbHloMk5YWFhvdUVNNU1EMFFpNkMzTm5JQlVQd3NFbjNfVG9rZW46WmpwcGJpZ1Yzb1g5dXN4dDhlU2NGTDBrbkpmXzE3Njg2NTU1MTQ6MTc2ODY1OTExNF9WNA "图片链接可能已过期，如无法显示请联系作者")

缓解写锁饥饿问题

这样做就行了吗? ReentrantReadWriteLock是可重入的读写锁,那么看下图情况： 当已经获取了读锁的线程,再次获取读锁,如果只是按照上面的来实现,那么就会出现死锁 _读线程持有读锁，然后想要获取读锁，但是有写线程在等待,那么为了避免饥饿,读线程会去阻塞,而写线程又在等读线程唤醒 - 死锁_

<!-- 这是一张图片，ocr 内容为： -->
![图片11](https://scnjnj9snmp7.feishu.cn/space/api/box/stream/download/asynccode/?code=YzQ1OWUyMmU4NzU3ZDEzZTYwNjQ4OWNjMGE5ZTNlZmZfTGJYcGRsYXprOXU3U3ZwdjdCY1lqZERCeW5ua1ppTTBfVG9rZW46R1R2SmJZWDVwb1JQUHd4TFFwMmNBaUFPbkxkXzE3Njg2NTU1MTQ6MTc2ODY1OTExNF9WNA "图片链接可能已过期，如无法显示请联系作者")
...
<!-- 这是一张图片，ocr 内容为： -->
![图片12](https://scnjnj9snmp7.feishu.cn/space/api/box/stream/download/asynccode/?code=YTY1ODAwNTZkM2NlMDgxY2RhZWQ0MGE3YzYzMzljMzZfR2lZNHFuUGVhYnZkVEtqd1FCaEU5bEVaVVJkd2tHU0hfVG9rZW46RnkzM2JxUGxob1UxNXd4bTdvcmNIRTRBbmZiXzE3Njg2NTU1MTQ6MTc2ODY1OTExNF9WNA "图片链接可能已过期，如无法显示请联系作者")
...
### 类关系图
<!-- 这是一张图片，ocr 内容为： -->
![图片13](https://scnjnj9snmp7.feishu.cn/space/api/box/stream/download/asynccode/?code=ZWEyZWIyNzFhMDQ2YjIxZWE3MDIxM2I5YzRkNzE5NThfcnB4cDdvU1E1RkxRUldBcXd6RjF2QkU1akt6b3NXYVJfVG9rZW46VFhuMGJyTjg2b1ZPZ2N4dHVBMGM0Qk9Wbm1kXzE3Njg2NTU1MTQ6MTc2ODY1OTExNF9WNA "图片链接可能已过期，如无法显示请联系作者")
...
<!-- 这是一张图片，ocr 内容为： -->
![图片14](https://scnjnj9snmp7.feishu.cn/space/api/box/stream/download/asynccode/?code=NjdlMmI0MmJkMzFlMGRkZjhmNzk3MTMxNWY1YTA4YWNfc05ZWW8xSW85TTVRU3dVeDdQcVBjdWVUWm5CRU9IRlRfVG9rZW46WWtFMWJDWFZQbzBwczR4eFk4ZmNQYURnbmtjXzE3Njg2NTU1MTQ6MTc2ODY1OTExNF9WNA "图片链接可能已过期，如无法显示请联系作者")
...
readLock():获取读锁(_**悲观模式)**_ 读锁的获取和AQS的设计一样,也有快速获取和慢速获取之分 1.快速获取：队列为空并且没有写锁时直接尝试去cas竞争,否则进入到2 2.慢速获取: 下面就看慢速获取,因为它包含了快速获取的逻辑
...
<!-- 这是一张图片，ocr 内容为： -->
![图片15](https://scnjnj9snmp7.feishu.cn/space/api/box/stream/download/asynccode/?code