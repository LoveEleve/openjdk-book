...
在这里回顾一下之前介绍过的ThreadLocal和FastThreadLocal
  ...
  ThreadLocal:每个threadLocal对象都有对应的hashcode,(递增,以hash算法来计算位置) FastThreadLocal:每个fastThreadLocal对象都有自己的index(递增,不是以hash算法来计算位置)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/34840193/1768640008223-ff10027c-1963-470b-8181-566e23f5137d.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/34840193/1768640008149-e0258f40-feef-46cd-b8c0-f425384188aa.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/34840193/1768640008214-9444f25a-621a-446f-a8ab-7045a588f6b7.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/34840193/1768640008408-116e3d92-c2c4-49b9-a593-c9477fb908c9.png)
...
每个节点的hash值都被保存起来了,所以是不需要重新计算hash值的 按照一般的思路：新位置 = e.hash & newTab.length - 1 (这没有什么问题,这样做也能实现)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/34840193/1768640008436-8d50b07c-6242-4687-91fa-da2c9efade4d.png)
...
length = 0000 0000 0000 0001 0000 0000 0000 0000 (2^16次方) length - 1 = 0000 0000 0000 0000 1111 1111 1111 1111（计算元素位置时使用的是length - 1）
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/34840193/1768640009226-a6a8a3f2-7a71-40ea-b961-9f4fda5ce5bb.png)
...
fast-fail机制,是Java集合中的一种**<font style="color:rgb(216,57,49);">错误检测机制</font>**,被用来设计在**<font style="color:rgb(216,57,49);">迭代器遍历集合</font>**的过程中,如果集合中的元素被除了迭代器自身的remove()方法之外的任何方式修改，那么就立刻抛出ConcurrentModificationException 异常。
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/34840193/1768640009264-397376d8-0b6c-4253-96cf-956080c81f9b.png)

我感觉在哪里见过？StampedLock(并且StampedLock可以看作是fail-safe机制的一种实现,因为当写线程操作后,读线程没有抛出异常，通常的做法是重新去读取)

<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/34840193/1768640009364-b2b03c69-6514-49c8-b352-c9cad96cdbc0.png)

+ fail-safe机制(安全失败)

这是一种设计哲学,目的是确保即使系统状态发生变化,操作也不会失败或者抛出异常,而是继续以某种安全的方式继续运行。**<font style="color:rgb(216,57,49);">在Java集合中特指那些在迭代过程中能够容忍集合被修改的并发集合类</font>**
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/34840193/1768640009708-7fd9cf30-ef8d-4cbb-8a32-2021fe382b9b.png)
...
那么为什么是0.75呢？ - **<font style="color:rgb(216,57,49);">二项分布的概率公式计算</font>**（不扩展）

+ 链表转为红黑树的阈值为什么是8，而不是其他值呢？(还需要满足数组的长度超过64)

**<font style="color:rgb(216,57,49);">泊松分布</font>**
...
与树化相反的过程叫做反树化，也即从红黑树退化为链表的操作，但是退化操作不是仅仅由红黑树的个数来决定的，而是由**<font style="color:rgb(216,57,49);">个数</font>**和**<font style="color:rgb(216,57,49);">树的结构</font>**一起来决定的
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/34840193/1768640009866-5b3c88d6-441b-4923-837e-3b942db959a7.png)

这么做的意义是什么呢？ 用户传入16,这代表用户想要存储是16个元素,但是如果按照hashMap的做法,内部数组的容量就是16,但是默认的扩容
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/34840193/1768640009819-e6c3890c-13d8-4cd2-86fb-e6c13082cb0f.png)

这里有一个问题：那就是cmp可是线程安全的，先给出Map的一般结构：

<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/34840193/1768640009965-58fb40fc-3e7a-4851-a693-1ee2af19e262.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/34840193/1768640010255-e1a29fc7-6705-44e9-ad2f-2444f6adb116.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/34840193/1768640010613-d5d10666-bc9b-4107-a376-ad22053af8dc.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/34840193/1768640010553-501b8d5a-0a11-4c1c-81e4-29319ee2a0e7.png)
...
1. 首先会为每个数组长度生成唯一标识：通过int rs = resizeStamp(n) << RESIZE_STAMP_SHIFT来实现,该数值可以分为两个部分,高16位和低16位「高16位代表数组长度唯一标识」「低16位代表扩容线程」
...
**/* ------------------------------**<font style="color:rgb(216,57,49);">核心扩容逻辑</font>**------------------------------*/**
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/34840193/1768640010526-a0ed4f28-a169-47cd-ae37-02980e65a4bf.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/34840193/1768640010714-a0210b72-0202-4e50-86fd-fbed834dd776.png)

#2处的代码逻辑：形成高低链表

<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/34840193/1768640010924-108b4dd1-e8cd-4052-a121-80e4ee8277d9.png)

#3处的代码逻辑：处理[槽位节点,lastRun)之间的节点

<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/34840193/1768640011285-765c99c6-8ba6-4755-a5c1-2f24daa00c99.png)
...
跳表是一种概率性的有序数据结构，它通过维护多级索引来实现快速查找，可以看作是多层次的链表。跳表的平均时间复杂度为O(log n)，最坏情况为O(n)，在性能上可以与平衡树相媲美，**<font style="color:rgb(216,57,49);">但实现更为简单</font>**

其中最关键的一点就在于实现简单(虽然最坏情况可能变为O(n)) 因为类似平衡树(AVL树,红黑树,B树)等数据结构的痛点就在于实现非常复杂,维护困难并且并发性能差, 而跳表的核心设计目标就是：简单实现,良好性能,空间效率以及易于并发

+ 单链表的痛点

即使单链表是**<font style="color:rgb(216,57,49);">有序</font>**的,但是想要找到某个具体元素，还是需要遍历整个链表,时间复杂度为O(N)「当然跳表也是基于有序的,和数组的二分查找的条件是一样的(要求数组是有序的)」
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/34840193/1768640011203-78513321-ef7c-44b6-a205-e1d79fd684f3.png)
...
当初始化后,跳表的结构如下：head节点的level默认为MAX_LEVEL,因为每一层都需要有head节点、

<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/34840193/1768640011464-9fff36c2-c2c6-4701-9e1e-f6aaf0c1baa9.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/34840193/1768640011432-b323e2c2-ad67-4257-b5f5-097d971ccaae.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/34840193/1768640011797-df1eb0f4-34fd-4cae-a639-92522eefaa51.png)

再看一下第一个节点的操作结果(一般情况)：

<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/34840193/1768640011752-1f7e7c1b-5d6d-4255-8be4-aeabe2f564d6.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/34840193/1768640012195-9a9efa50-057d-421a-9904-30c4c7931b08.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/34840193/1768640012075-b8c06629-88a2-46fb-8e64-554d181d8307.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/34840193/1768640012364-11b7a9cf-f2ae-4797-ad88-3741d0ac4ff4.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/34840193/1768640012374-0c7de90a-11b7-4d43-9316-b8975b6e51f4.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/34840193/1768640012506-414c2f2d-3f61-470b-8743-f5caefaedaa1.png)

先回顾下目前的整体架构：重点关注q和x

<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/34840193/1768640012850-38ef8ecb-8c9d-4d60-9a6b-df72982897ca.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/34840193/1768640013022-03783eca-eb1e-438f-8580-64bb6ff53e98.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/34840193/1768640012922-5cce0ec8-b973-4498-8e22-992f80837028.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/34840193/1768640013042-1c2b70fd-379c-4b73-acc1-dd4f1e898b0d.png)

当调用的是无参构造或者是0时,<u>按道理来说容量为0,那么其含义和无参构造是一样的</u>，但是在ArrayList中并没有这样实现,这两者在进行操作时会有不一样的行为.
...
2. 无参构造和0采用的是懒加载的
3. 无参构造的扩容原理和有参一样：新数组的容量 = 旧数组容量 * 1.5 但是0的扩容原理则是每次+1(也即按需使用,好处是空间利用率高，缺点就是频繁的扩容)
...
介绍：CopyOnWriteArrayList 是 java.util.concurrent 包下的一个**<font style="color:rgb(216,57,49);">线程安全</font>**的 List 实现。它是 ArrayList 的线程安全版本，但其**<font style="color:rgb(216,57,49);">线程安全的实现机制与传统的同步方式</font>**（如使用 `synchronized` 关键字）有**<font style="color:rgb(216,57,49);">根本性的不同</font>**。
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/34840193/1768640013119-a1067871-7364-4480-a42e-d37b639cc7a6.png)
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/34840193/1768640013575-5174ddc6-e3d5-4755-9a17-5038a8bb9c41.png)
...
1. CopyOnWriteArrayList集合的使用场景为并发操作，并且是**<font style="color:rgb(216,57,49);">读多写少</font>**
...
<!-- 这是一张图片，ocr 内容为： -->
![](https://cdn.nlark.com/yuque/0/2026/png/34840193/1768640013694-256eca06-01cb-4e58-a3b0-af16b636292f.png)
