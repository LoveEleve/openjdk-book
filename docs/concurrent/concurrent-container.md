...
1. key的hash值为什么不是直接使用hashCode()返回的值,而是要再与自己的高16位做异或运算呢？「**这可以称为扰动函数**」
...
fast-fail机制,是Java集合中的一种**错误检测机制**,被用来设计在**迭代器遍历集合**的过程中,如果集合中的元素被除了迭代器自身的remove()方法之外的任何方式修改，那么就立刻抛出ConcurrentModificationException 异常。
...
这是一种设计哲学,目的是确保即使系统状态发生变化,操作也不会失败或者抛出异常,而是继续以某种安全的方式继续运行。**在Java集合中特指那些在迭代过程中能够容忍集合被修改的并发集合类**
...
与树化相反的过程叫做反树化，也即从红黑树退化为链表的操作，但是退化操作不是仅仅由红黑树的个数来决定的，而是由**个数**和**树的结构**一起来决定的
...
跳表是一种概率性的有序数据结构，它通过维护多级索引来实现快速查找，可以看作是多层次的链表。跳表的平均时间复杂度为O(log n)，最坏情况为O(n)，在性能上可以与平衡树相媲美，**但实现更为简单**

其中最关键的一点就在于实现简单(虽然最坏情况可能变为O(n)) 因为类似平衡树(AVL树,红黑树,B树)等数据结构的痛点就在于实现非常复杂,维护困难并且并发性能差, 而跳表的核心设计目标就是：简单实现,良好性能,空间效率以及易于并发

+ 单链表的痛点

即使单链表是**有序**的,但是想要找到某个具体元素，还是需要遍历整个链表,时间复杂度为O(N)「当然跳表也是基于有序的,和数组的二分查找的条件是一样的(要求数组是有序的)」
...
介绍：CopyOnWriteArrayList 是 java.util.concurrent 包下的一个**线程安全**的 List 实现。它是 ArrayList 的线程安全版本，但其**线程安全的实现机制与传统的同步方式**（如使用 `synchronized` 关键字）有**根本性的不同**。
...
1. CopyOnWriteArrayList集合的使用场景为并发操作，并且是**读多写少**
